...
namespace StarNight.Map.Tests.EditMode.WorldGeneration.SectorPlanning
{
    [TestFixture]
    [Category("MAP14_06")]
    public sealed class SectorQuietActivityEventPlannerTests
    {
        [Test]
        public void FillPublishesQuietAndBufferForRemainingReferenceCanvasSpace()
        {
            var fixture = Fixture.Create();
            var fill = fixture.Fill();
            Assert.That(fill.Success, Is.True, Errors(fill.Errors));
            AssertLowerSha(fill.CanonicalDigest);
            Assert.That(fill.Plan.SectorCount, Is.EqualTo(9));
            Assert.That(fill.Plan.ClassifiedCellCount, Is.EqualTo(9 * 48 * 32));
            Assert.That(fill.Plan.QuietFillCellCount, Is.GreaterThan(0));
            Assert.That(fill.Plan.BufferCellCount, Is.GreaterThan(0));
            Assert.That(fill.Plan.UnclassifiedRemainderCount, Is.Zero);

            var built = fixture.Place(fill.Plan);
            Assert.That(built.Success, Is.True, Errors(built.Errors));
            var plan = built.Plan;
            TestContext.WriteLine("MAP14_06_METRICS sectors={0} classified={1} quiet={2} buffer={3} reserved={4} protected={5} pattern={6} unclassified={7}",
                plan.QuietFillPlan.SectorCount, plan.QuietFillPlan.ClassifiedCellCount,
                plan.QuietFillPlan.QuietFillCellCount, plan.QuietFillPlan.BufferCellCount,
                plan.QuietFillPlan.ReservedCoordinateCount, plan.QuietFillPlan.ProtectedCoordinateCount,
                plan.QuietFillPlan.PatternRenderedCoordinateCount, plan.QuietFillPlan.UnclassifiedRemainderCount);
            TestContext.WriteLine("MAP14_06_ACTIVITY opportunities={0} compatible={1} selected={2} rejected={3} strong={4} rngStreams={5} rngDraws={6}",
                plan.ActivityOpportunityCount, plan.CompatibleActivityCandidateCount,
                plan.ActivitySelectedCount, plan.ActivityRejectedCount, plan.StrongSelectedCount,
                plan.ActivityMap12RngStreamCount, plan.ActivityMap12RngDrawCount);
            TestContext.WriteLine("MAP14_06_EVENT opportunities={0} nonEmptyCompatible={1} emptyCompatible={2} assigned={3} empty={4} cooldownExclusions={5} cooldownViolations={6} rngStreams={7} rngDraws={8}",
                plan.EventOpportunityCount, plan.EventNonEmptyCompatibleCandidateCount,
                plan.EventEmptyCompatibleCandidateCount, plan.EventAssignedNonEmptyCount,
                plan.EventAssignedEmptyCount, plan.CooldownExclusionCount, plan.CooldownViolationCount,
                plan.EventMap12RngStreamCount, plan.EventMap12RngDrawCount);
            TestContext.WriteLine("MAP14_06_DIGEST quiet={0} activityEvent={1} activityAuthority={2} eventAuthority={3}",
                plan.QuietFillPlan.CanonicalDigest, plan.CanonicalDigest,
                plan.ActivityFrequencyPlanDigestBefore, plan.EventAssignmentPlanDigestBefore);
        }

        [Test]
        public void QuietFillAvoidsProtectedOpenAnchorsSpecialShellsAndPatternCells()
        {
            var fixture = Fixture.Create();
            var fill = fixture.Fill().Plan;
            Assert.That(fill, Is.Not.Null);
            Assert.That(fill.ProtectedCoordinateCount, Is.GreaterThan(0));
            Assert.That(fill.ReservedCoordinateCount, Is.GreaterThan(0));
            Assert.That(fill.PatternRenderedCoordinateCount, Is.EqualTo(fixture.RenderPlan.RenderTargetCellCount));
            Assert.That(fill.ProtectedIntrusionCount, Is.Zero);
            Assert.That(fill.ReservedIntrusionCount, Is.Zero);
            Assert.That(fill.PatternOverwriteCount, Is.Zero);
            Assert.That(fill.Cells.Where(value => value.ProtectedNoWrite || value.ReservedNoWrite || value.PatternRendered)
                .Any(value => value.ActivityEligible), Is.False);
        }

        [Test]
        public void QuietFillIsInBoundsUniqueAndDoesNotMutatePatternRender()
        {
            var fixture = Fixture.Create();
            var before = fixture.RenderPlan.CanonicalDigest;
            var fill = fixture.Fill();
            Assert.That(fill.Success, Is.True, Errors(fill.Errors));
            Assert.That(fill.Plan.Cells.All(value => value.Coordinate.X >= 0 && value.Coordinate.X < 48 &&
                                                     value.Coordinate.Y >= 0 && value.Coordinate.Y < 32), Is.True);
            Assert.That(fill.Plan.Cells.Select(Key).Distinct(StringComparer.Ordinal).Count(),
                Is.EqualTo(fill.Plan.Cells.Count));
            Assert.That(fixture.RenderPlan.CanonicalDigest, Is.EqualTo(before));
            Assert.That(fill.Plan.PatternRenderPlanDigestBefore, Is.EqualTo(before));
            Assert.That(fill.Plan.PatternRenderPlanDigestAfter, Is.EqualTo(before));
        }

        [Test]
        public void ActivityOpportunitiesUseMap12CompatibilityFrequencyAndCaps()
        {
            var fixture = Fixture.Create();
            var fill = fixture.Fill().Plan;
            var package = fixture.CreateAuthorities(fill);
            var result = fixture.Place(fill, package);
            Assert.That(result.Success, Is.True, Errors(result.Errors));
            var plan = result.Plan;
            Assert.That(plan.Map12ActivityCandidateCompilerType,
                Is.EqualTo(typeof(ActivityCandidateIndexCompiler).FullName));
            Assert.That(plan.Map12ActivityFrequencyPlannerType,
                Is.EqualTo(typeof(ActivityFrequencyPlanner).FullName));
            Assert.That(plan.ActivityOpportunityCount, Is.EqualTo(9));
            Assert.That(plan.CompatibleActivityCandidateCount, Is.EqualTo(9));
            Assert.That(plan.ActivitySelectedCount, Is.EqualTo(1));
            Assert.That(plan.ActivityRejectedCount, Is.EqualTo(8));
            Assert.That(plan.StrongSelectedCount, Is.EqualTo(1));
            Assert.That(plan.ActivityDecisions.Single(value => value.State == SectorActivityEventPlacementState.Selected)
                .WorldStrongAfter, Is.EqualTo(1));
        }

        [Test]
        public void ActivityMarkersPreserveRemovalSafetyAndStaticRouteIdentity()
        {
            var fixture = Fixture.Create();
            var result = fixture.Place(fixture.Fill().Plan);
            Assert.That(result.Success, Is.True, Errors(result.Errors));
            Assert.That(result.Plan.RemovalSafetyIdentityPreserved, Is.True);
            Assert.That(result.Plan.QuietFillPlan.RouteAccessIdentityBefore,
                Is.EqualTo(result.Plan.QuietFillPlan.RouteAccessIdentityAfter));
            Assert.That(result.Plan.ActivityMarkerMutationCount, Is.Zero);
            Assert.That(result.Plan.ActivityRuntimeSpawnCount, Is.Zero);
            Assert.That(result.Plan.ActivityMap12RngDrawCount, Is.GreaterThan(0));
            Assert.That(result.Plan.Map14RngDrawCount, Is.Zero);
        }

        [Test]
        public void EventMarkerAssignmentUsesMap12CooldownAndExplicitEmpty()
        {
            var fixture = Fixture.Create();
            var result = fixture.Place(fixture.Fill().Plan);
            Assert.That(result.Success, Is.True, Errors(result.Errors));
            var plan = result.Plan;
            Assert.That(plan.Map12EventCandidateCompilerType,
                Is.EqualTo(typeof(EventOverlayCandidateIndexCompiler).FullName));
            Assert.That(plan.Map12EventAssignmentPlannerType,
                Is.EqualTo(typeof(EventOverlayAssignmentPlanner).FullName));
            Assert.That(plan.EventOpportunityCount, Is.EqualTo(9));
            Assert.That(plan.EventNonEmptyCompatibleCandidateCount, Is.EqualTo(9));
            Assert.That(plan.EventEmptyCompatibleCandidateCount, Is.EqualTo(9));
            Assert.That(plan.EventAssignedNonEmptyCount, Is.EqualTo(1));
            Assert.That(plan.EventAssignedEmptyCount, Is.EqualTo(8));
            Assert.That(plan.CooldownViolationCount, Is.Zero);
            Assert.That(plan.EventDecisions.Count, Is.EqualTo(9));
        }

        [Test]
        public void SpecialVillageBossForgeMarkersRemainNonOwning()
        {
            var fixture = Fixture.Create();
            var result = fixture.Place(fixture.Fill().Plan);
            Assert.That(result.Success, Is.True, Errors(result.Errors));
            var special = result.Plan.Request.EventOpportunities
                .Where(value => value.MarkerKind == SectorActivityEventMarkerKind.EventSpecial).ToArray();
            Assert.That(special.Length, Is.EqualTo(5));
            Assert.That(special.Any(value => value.OwnerIdentity == "REGION_VILLAGE"), Is.True);
            Assert.That(special.Any(value => value.OwnerIdentity == "REGION_CORE"), Is.True);
            Assert.That(special.Any(value => value.OwnerIdentity == "REGION_FORGE"), Is.True);
            Assert.That(special.Any(value => value.OwnerIdentity == "REGION_BOSS"), Is.True);
            Assert.That(special.Any(value => value.OwnerIdentity == "REGION_MERCHANT"), Is.True);
            Assert.That(result.Plan.SpecialPersistenceMutationCount, Is.Zero);
            Assert.That(result.Plan.RewardExecutionCount, Is.Zero);
            Assert.That(result.Plan.CombatExecutionCount, Is.Zero);
            Assert.That(result.Plan.CraftingExecutionCount, Is.Zero);
        }

        [Test]
        public void NoFinalOwnershipRetryTilePhysicsOrSceneMutation()
        {
            var fixture = Fixture.Create();
            var result = fixture.Place(fixture.Fill().Plan);
            Assert.That(result.Success, Is.True, Errors(result.Errors));
            var plan = result.Plan;
            Assert.That(plan.FinalCanvasOwnershipWriteCount, Is.Zero);
            Assert.That(plan.LayerConflictResolutionCount, Is.Zero);
        ...
        [Test]
        public void InvalidProtectedOverlapMissingAuthorityDuplicateAndMutationClaimsFailAtomically()
        {
            var fixture = Fixture.Create();
            var invalidFill = SectorQuietFillPlanner.Fill(new SectorQuietActivityEventBuildRequest(
                fixture.Input, fixture.Assignments, fixture.AnchorPlan, fixture.PlacementPlan,
                fixture.SpineEnvelopePlan, fixture.RolePlan, fixture.RenderPlan,
                SectorQuietFillPlanner.ReferencePublicationLabel,
                referenceFaults: new[]
                {
                    SectorQuietActivityEventErrorCode.DuplicateQuietCell,
                    SectorQuietActivityEventErrorCode.QuietCellTouchesProtectedOpen,
                },
                patternCanvasMutationClaim: true,
                ...
                retryCount: 1,
                tileWriteCount: 1));
            AssertAtomic(invalidFill,
                SectorQuietActivityEventErrorCode.DuplicateQuietCell,
                SectorQuietActivityEventErrorCode.QuietCellTouchesProtectedOpen,
                SectorQuietActivityEventErrorCode.PatternCanvasMutationClaim,
                SectorQuietActivityEventErrorCode.OwnershipMutationClaim);

            var fill = fixture.Fill().Plan;
            var missing = SectorActivityEventPlacementPlanner.Place(new SectorActivityEventPlacementRequest(
                fill, Array.Empty<SectorActivityOpportunityProjection>(), null, null,
                Array.Empty<SectorEventMarkerOpportunityProjection>(), null, null,
                SectorActivityEventPlacementPlanner.ReferencePublicationLabel));
            AssertAtomic(missing,
                SectorQuietActivityEventErrorCode.MissingActivityAuthority,
                SectorQuietActivityEventErrorCode.MissingEventAuthority);

            var authority = fixture.CreateAuthorities(fill);
            var mutations = SectorActivityEventPlacementPlanner.Place(authority.Request(fill,
                referenceFaults: new[]
                {
                    SectorQuietActivityEventErrorCode.ActivityOpportunityOverlapsProtected,
                    SectorQuietActivityEventErrorCode.EventOpportunityOverlapsProtected,
                },
                activityMarkerMutationClaim: true,
                eventMarkerMutationClaim: true,
                specialPersistenceMutationClaim: true,
                ownershipMutationClaim: true,
                solverInvocationCount: 1,
                map14RngDrawCount: 1,
                retryCount: 1,
                tileWriteCount: 1));
            AssertAtomic(mutations,
                SectorQuietActivityEventErrorCode.ActivityOpportunityOverlapsProtected,
                SectorQuietActivityEventErrorCode.EventOpportunityOverlapsProtected,
                SectorQuietActivityEventErrorCode.ActivityMarkerMutationClaim,
                SectorQuietActivityEventErrorCode.EventMarkerMutationClaim,
                SectorQuietActivityEventErrorCode.SpecialPersistenceMutationClaim,
                SectorQuietActivityEventErrorCode.OwnershipMutationClaim);
        }
        ...
        public void PublicationIsDeterministicAcrossRepeatReverseAndTurkishCulture()
        {
            ...
            try
            {
                ...
                var firstFill = firstFixture.Fill().Plan;
                var first = firstFixture.Place(firstFill);
                var repeated = firstFixture.Place(firstFill);
                var reverseFixture = Fixture.Create(true);
                var reverseFill = reverseFixture.Fill().Plan;
                var reverse = reverseFixture.Place(reverseFill, reverseFixture.CreateAuthorities(reverseFill, true));
                CultureInfo.CurrentCulture = new CultureInfo("tr-TR");
                CultureInfo.CurrentUICulture = new CultureInfo("tr-TR");
                var turkishFixture = Fixture.Create();
                var turkishFill = turkishFixture.Fill().Plan;
                var turkish = turkishFixture.Place(turkishFill);
                Assert.That(new[] { first.Success, repeated.Success, reverse.Success, turkish.Success }, Is.All.True);
                Assert.That(repeated.CanonicalDigest, Is.EqualTo(first.CanonicalDigest));
                Assert.That(reverseFill.CanonicalDigest, Is.EqualTo(firstFill.CanonicalDigest));
                ...
                Assert.That(Join(reverse.Plan.ActivityDecisions.Select(value => value.OpportunityId + ":" + value.ActivityId.Value)),
                    Is.EqualTo(Join(first.Plan.ActivityDecisions.Select(value => value.OpportunityId + ":" + value.ActivityId.Value))));
                Assert.That(Join(reverse.Plan.EventDecisions.Select(value => value.OpportunityId + ":" + value.EventId.Value)),
                    Is.EqualTo(Join(first.Plan.EventDecisions.Select(value => value.OpportunityId + ":" + value.EventId.Value))));
            }
            finally
            {
        ...
        private static void AssertAtomic(
            SectorQuietFillBuildResult result,
            params SectorQuietActivityEventErrorCode[] expected)
        {
        ...
        private static void AssertAtomic(
            SectorQuietActivityEventBuildResult result,
            params SectorQuietActivityEventErrorCode[] expected)
        {
            Assert.That(result.Success, Is.False);
            Assert.That(result.Plan, Is.Null);
        ...
        private static string Key(SectorQuietFillCell value) =>
        ...
        private static string Errors(IEnumerable<SectorQuietActivityEventError> errors) =>
            string.Join(";", (errors ?? Array.Empty<SectorQuietActivityEventError>()).Select(value => value.ToString()));
