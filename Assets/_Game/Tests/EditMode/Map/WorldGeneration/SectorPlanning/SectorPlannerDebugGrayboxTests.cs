...
namespace StarNight.Map.Tests.EditMode.WorldGeneration.SectorPlanning
{
    [TestFixture]
    [Category("MAP14_09")]
    public sealed class SectorPlannerDebugGrayboxTests
    {
        ...
        public void BuildReferencePacket()
        {
            ...
            var canvas = fixture.BuildCanvas(false);
            var retry = SectorPlannerRetryExecutor.Execute(RetryRequest(canvas, MetricsInputs()));
        ...
        [Test]
        public void DebugExportPublishesSuccessfulPlanSectionsTokensAndDigest()
        {
            ...
            TestContext.WriteLine("DEBUG_EXPORTS=1;SUCCESS_SECTIONS=" + export.Export.SectionCount +
                ";TOKENS=" + export.Export.TokenCount + ";GRID_PAYLOADS=" + export.Export.TextGridPayloadCount +
                ";DIGEST=" + export.Export.CanonicalDigest);
        }

        [Test]
        public void FailureRingExportsCenterAndAvailableNeighborContextWithoutRepair()
        {
        ...
        [Test]
        public void GrayboxCatalogCoversEveryRouteTypeInOneAndThreeSectorFixtures()
        {
            AssertCoverage(SectorPlannerGrayboxCoverageKind.RouteType, 7);
            Assert.That(catalog.CoverageAudit.CoveredIn(SectorPlannerGrayboxCoverageKind.RouteType, SectorPlannerGrayboxFixtureKind.OneSector).Count, Is.EqualTo(7));
            Assert.That(catalog.CoverageAudit.CoveredIn(SectorPlannerGrayboxCoverageKind.RouteType, SectorPlannerGrayboxFixtureKind.ThreeSector).Count, Is.EqualTo(7));
        ...
        [Test]
        public void GrayboxCatalogCoversEveryBiomeInOneAndThreeSectorFixtures()
        {
            AssertCoverage(SectorPlannerGrayboxCoverageKind.Biome, 4);
            Assert.That(catalog.CoverageAudit.CoveredIn(SectorPlannerGrayboxCoverageKind.Biome, SectorPlannerGrayboxFixtureKind.OneSector).Count, Is.EqualTo(4));
            Assert.That(catalog.CoverageAudit.CoveredIn(SectorPlannerGrayboxCoverageKind.Biome, SectorPlannerGrayboxFixtureKind.ThreeSector).Count, Is.EqualTo(4));
        }

        [Test]
        public void GrayboxCatalogCoversEveryBoundaryPairInOneAndThreeSectorFixtures()
        {
            AssertCoverage(SectorPlannerGrayboxCoverageKind.BoundaryPair, 6);
            Assert.That(catalog.CoverageAudit.CoveredIn(SectorPlannerGrayboxCoverageKind.BoundaryPair, SectorPlannerGrayboxFixtureKind.OneSector).Count, Is.EqualTo(6));
            Assert.That(catalog.CoverageAudit.CoveredIn(SectorPlannerGrayboxCoverageKind.BoundaryPair, SectorPlannerGrayboxFixtureKind.ThreeSector).Count, Is.EqualTo(6));
        }

        [Test]
        public void GrayboxCatalogCoversEverySpecialConditionInOneAndThreeSectorFixtures()
        {
            AssertCoverage(SectorPlannerGrayboxCoverageKind.SpecialRegion, 6);
            Assert.That(catalog.CoverageAudit.CoveredIn(SectorPlannerGrayboxCoverageKind.SpecialRegion, SectorPlannerGrayboxFixtureKind.OneSector).Count, Is.EqualTo(6));
            Assert.That(catalog.CoverageAudit.CoveredIn(SectorPlannerGrayboxCoverageKind.SpecialRegion, SectorPlannerGrayboxFixtureKind.ThreeSector).Count, Is.EqualTo(6));
        ...
        [Test]
        public void CoverageIncludesOwnershipPlanesAndRetryStages()
        {
            ...
            Assert.That(catalog.CoverageAudit.ZeroCountEvidence, Is.Not.Empty);
            ...
            var ownerSection = export.Export.Sections.Single(value => value.Kind == SectorPlannerDebugSectionKind.OwnershipPlanes);
            Assert.That(ownerSection.Facts.Count(value => value.Key.StartsWith("PLANE_", StringComparison.Ordinal)), Is.EqualTo(5));
            Assert.That(ownerSection.Facts.Count(value => value.Key.StartsWith("OWNER_", StringComparison.Ordinal)), Is.EqualTo(11));
            TestContext.WriteLine("FIXTURES_ONE=9;FIXTURES_THREE=9;FIXTURES_FAILURE_RING=1;ROUTE=7/7/0;BIOME=4/4/0;BOUNDARY=6/6/0;SPECIAL=6/6/0;PACING_ROLE=" + CoverageCounts(SectorPlannerGrayboxCoverageKind.PacingRole) +
                ";ACCESS_CLASS=" + CoverageCounts(SectorPlannerGrayboxCoverageKind.AccessClass) +
                ";OWNERSHIP_PLANE=5/5/0;RETRY_STAGE_TERMINAL=8/8/0;COVERAGE_DIGEST=" + catalog.CoverageAudit.CanonicalDigest);
            TestContext.WriteLine("OWNERSHIP_PLANES=" + string.Join(",", Enum.GetValues(typeof(SectorCanvasOwnershipPlane)).Cast<SectorCanvasOwnershipPlane>()
                .Select(value => value + "=" + request.RetryPlan.Request.OwnershipPlan.CountOwned(value))) +
                ";OWNERS=" + string.Join(",", Enum.GetValues(typeof(SectorCanvasOwnerKind)).Cast<SectorCanvasOwnerKind>()
                    .Select(value => value + "=" + request.RetryPlan.Request.OwnershipPlan.CountWinners(value))) +
                ";RETRY_STAGES=" + string.Join(",", Enum.GetValues(typeof(SectorPlannerRetryStage)).Cast<SectorPlannerRetryStage>()
                    .Select(value => value + "=" + request.RetryPlan.Count(value))) +
                ";TERMINAL=" + request.RetryPlan.TerminalDecision);
        }
        ...
        [Test]
        public void DebugExportPreservesAllUpstreamIdentityAndDoesNotDrawRng()
        {
            var retry = request.RetryPlan;
            Assert.That(retry.AllUpstreamIdentitiesPreserved, Is.True);
            Assert.That(retry.PlannerInputDigestBefore, Is.EqualTo(retry.PlannerInputDigestAfter));
            Assert.That(retry.PacingAssignmentDigestBefore, Is.EqualTo(retry.PacingAssignmentDigestAfter));
            Assert.That(retry.FixedAnchorPlanDigestBefore, Is.EqualTo(retry.FixedAnchorPlanDigestAfter));
            Assert.That(retry.ClusterPlacementPlanDigestBefore, Is.EqualTo(retry.ClusterPlacementPlanDigestAfter));
            Assert.That(retry.SpineEnvelopePlanDigestBefore, Is.EqualTo(retry.SpineEnvelopePlanDigestAfter));
            Assert.That(retry.RolePatternPlanDigestBefore, Is.EqualTo(retry.RolePatternPlanDigestAfter));
            Assert.That(retry.PatternRenderPlanDigestBefore, Is.EqualTo(retry.PatternRenderPlanDigestAfter));
            Assert.That(retry.QuietActivityEventPlanDigestBefore, Is.EqualTo(retry.QuietActivityEventPlanDigestAfter));
            Assert.That(retry.CanvasOwnershipPlanDigestBefore, Is.EqualTo(retry.CanvasOwnershipPlanDigestAfter));
            Assert.That(export.NewRngDrawCount + export.RetryExecutionCount, Is.Zero);
            Assert.That(export.Export.MutationProof.NewRngDrawCount + export.Export.MutationProof.RetryExecutionCount, Is.Zero);
        }

        [Test]
        public void InvalidMissingSourceDuplicateTokenMissingCoverageAndFileWriteClaimsFailAtomically()
        {
        ...
        [Test]
        public void NoTilePhysicsScenePreviewGameplayOrExitApprovalMutation()
        {
        ...
        private static SectorPlannerRetryBuildRequest RetryRequest(
            ...
            new SectorPlannerRetryBuildRequest(canvas, SectorPlannerRetryPolicy.CreateDefault(), RetryRngFactory(),
                0x14090001UL, new SectorCoord(2, 2), sourceAttemptInputs: inputs,
