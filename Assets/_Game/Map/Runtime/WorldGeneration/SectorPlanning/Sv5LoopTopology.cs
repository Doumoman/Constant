...
namespace StarNight.Map.WorldGeneration.SectorPlanning
{
    ...
    {
        public Sv5LoopOccupancyCell(RmapSpecialWorldPoint world, Sv5InfillCellValue value,
    ...
    public sealed class Sv5LoopTopologyEvaluation
    {
        internal Sv5LoopTopologyEvaluation(int baselineCost,int finalCost,int baselineContacts,int newlyCarvedAir,
            ...
            IReadOnlyCollection<RmapSpecialWorldPoint> finalFootNodes,IEnumerable<string> errors)
        {
        ...
        public IReadOnlyCollection<RmapSpecialWorldPoint> FinalFootNodes { get; }
    ...
    {
        internal Sv5LoopTopologyPlan(IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> baseline,
            IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> final,
            IReadOnlyCollection<RmapSpecialWorldPoint> baselineFeet,
            IReadOnlyCollection<RmapSpecialWorldPoint> finalFeet,IEnumerable<Sv5LoopTopologyEdge> baselineEdges,
            ...
            Sv5LoopTopologyMetrics after,IEnumerable<Sv5LoopTopologyProof> proofs,IEnumerable<string> diagnostics)
        {
            ...
            Diagnostics=Array.AsReadOnly((diagnostics ?? Array.Empty<string>()).Distinct(StringComparer.Ordinal)
                .OrderBy(v=>v,StringComparer.Ordinal).ToArray());
            Digest=StarNight.Map.WorldGeneration.WorldData.RmapWorldDefinition.Hash(
        ...
        public IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> FinalOccupancy { get; }
        public IReadOnlyCollection<RmapSpecialWorldPoint> BaselineFootNodes { get; }
        public IReadOnlyCollection<RmapSpecialWorldPoint> FinalFootNodes { get; }
    ...
    {
        private readonly HashSet<RmapSpecialWorldPoint> baselineFootSet;
        private readonly ReadOnlyDictionary<RmapSpecialWorldPoint,IReadOnlyList<RmapSpecialWorldPoint>> footAdjacency;
        private readonly ReadOnlyDictionary<RmapSpecialWorldPoint,int> footComponents;
        ...
        internal Sv5LoopTopologyContext(Sv5SpaceGraphPlan plan,
            IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> baseline)
        {
            Plan=plan ?? throw new ArgumentNullException(nameof(plan));
            BaselineOccupancy=baseline ?? throw new ArgumentNullException(nameof(baseline));
            baselineFootSet=new HashSet<RmapSpecialWorldPoint>(Sv5LoopTopology.SupportedFootNodes(baseline));
            BaselineFootNodes=Array.AsReadOnly(baselineFootSet.OrderBy(v=>v).ToArray());
            var adjacency=baselineFootSet.OrderBy(v=>v).ToDictionary(v=>v,v=>(IReadOnlyList<RmapSpecialWorldPoint>)
                Array.AsReadOnly(Sv5LoopTopology.FootNeighbors(v).Where(baselineFootSet.Contains).OrderBy(n=>n).ToArray()));
            footAdjacency=new ReadOnlyDictionary<RmapSpecialWorldPoint,IReadOnlyList<RmapSpecialWorldPoint>>(adjacency);
            var components=new Dictionary<RmapSpecialWorldPoint,int>(); int component=0;
            foreach(var start in baselineFootSet.OrderBy(v=>v))
            {
                if(components.ContainsKey(start)) continue;
                var queue=new Queue<RmapSpecialWorldPoint>(); queue.Enqueue(start); components[start]=component;
                while(queue.Count!=0)
                {
            ...
            footComponents=new ReadOnlyDictionary<RmapSpecialWorldPoint,int>(components);
        ...
        public IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> BaselineOccupancy { get; }
        public IReadOnlyCollection<RmapSpecialWorldPoint> BaselineFootNodes { get; }
        public IReadOnlyDictionary<RmapSpecialWorldPoint,IReadOnlyList<RmapSpecialWorldPoint>> FootAdjacency => footAdjacency;
        public IReadOnlyDictionary<RmapSpecialWorldPoint,int> FootComponents => footComponents;
        public IReadOnlyList<Sv5LoopTopologyEdge> BaselineEdges { get; }
        public Sv5LoopTopologyMetrics Before { get; }
        public int BaselineShortestCacheEntries => baselineShortestCosts.Count;

        public bool IsBaselineFoot(RmapSpecialWorldPoint point) => baselineFootSet.Contains(point);
        public bool SameFootComponent(RmapSpecialWorldPoint first,RmapSpecialWorldPoint second)
        ...
        public int BaselineShortestCost(RmapSpecialWorldPoint start,RmapSpecialWorldPoint goal)
        {
        ...
        private static string PointPair(RmapSpecialWorldPoint first,RmapSpecialWorldPoint second)
    ...
    {
        private sealed class OverlayOccupancy : IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>
        {
            private readonly IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> baseline;
            private readonly IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> overlay;
            internal OverlayOccupancy(IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> sourceBaseline,
                IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> sourceOverlay)
            { baseline=sourceBaseline; overlay=sourceOverlay; }
            public int Count => baseline.Count+overlay.Keys.Count(v=>!baseline.ContainsKey(v));
            public IEnumerable<RmapSpecialWorldPoint> Keys => baseline.Keys.Concat(overlay.Keys.Where(v=>!baseline.ContainsKey(v)));
            public IEnumerable<Sv5LoopOccupancyCell> Values => Keys.Select(v=>this[v]);
            public Sv5LoopOccupancyCell this[RmapSpecialWorldPoint key] => overlay.TryGetValue(key,out Sv5LoopOccupancyCell value) ? value : baseline[key];
            public bool ContainsKey(RmapSpecialWorldPoint key) => overlay.ContainsKey(key) || baseline.ContainsKey(key);
            public bool TryGetValue(RmapSpecialWorldPoint key,out Sv5LoopOccupancyCell value)
            { if(overlay.TryGetValue(key,out value)) return true; return baseline.TryGetValue(key,out value); }
            public IEnumerator<KeyValuePair<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>> GetEnumerator()
            {
                foreach(var pair in baseline) yield return overlay.TryGetValue(pair.Key,out Sv5LoopOccupancyCell value) ?
                    new KeyValuePair<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>(pair.Key,value) : pair;
                foreach(var pair in overlay) if(!baseline.ContainsKey(pair.Key)) yield return pair;
            }
            System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator() => GetEnumerator();
        }

        private sealed class OverlayFootNodes : IReadOnlyCollection<RmapSpecialWorldPoint>
        {
            private readonly IReadOnlyCollection<RmapSpecialWorldPoint> baseline;
            private readonly HashSet<RmapSpecialWorldPoint> affected;
            private readonly Func<RmapSpecialWorldPoint,bool> predicate;
            internal OverlayFootNodes(IReadOnlyCollection<RmapSpecialWorldPoint> sourceBaseline,
                HashSet<RmapSpecialWorldPoint> sourceAffected,Func<RmapSpecialWorldPoint,bool> sourcePredicate)
            { baseline=sourceBaseline;affected=sourceAffected;predicate=sourcePredicate; }
            public int Count => this.Count(v=>true);
            public IEnumerator<RmapSpecialWorldPoint> GetEnumerator()
            {
        ...
        public static IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> CaptureBaseline(
            Sv5SpaceGraphPlan plan)
        {
            if(plan==null || plan.Infill==null) throw new ArgumentException("An infill plan is required.",nameof(plan));
            var cells=new Dictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>();
            ...
            return new ReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>(cells);
        }
        ...
        public static Sv5LoopTopologyEvaluation Evaluate(Sv5LoopTopologyContext context,string loopId,
            string fromRoomId,string toRoomId,IReadOnlyList<RmapSpecialWorldPoint> footPath,
            IEnumerable<Sv5LoopCell> cells)
        {
            if(context==null) throw new ArgumentNullException(nameof(context));
            var errors=new List<string>(); var path=(footPath ?? Array.Empty<RmapSpecialWorldPoint>()).ToArray();
            var source=(cells ?? Array.Empty<Sv5LoopCell>()).ToArray();
            var overlay=new Dictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>();
            ...
            var affected=new HashSet<RmapSpecialWorldPoint>();
            foreach(var point in overlay.Keys) for(int dy=-1;dy<=1;dy++)
                affected.Add(new RmapSpecialWorldPoint(point.X,point.Y+dy));
            bool IsFinalFoot(RmapSpecialWorldPoint point)
            {
                ...
                return point.Y>0 && point.Y+1<Sv5SpaceGraphPlanner.WorldHeight &&
                    ...
                    OverlayValue(new RmapSpecialWorldPoint(point.X,point.Y+1))==Sv5InfillCellValue.Air &&
                    OverlayValue(new RmapSpecialWorldPoint(point.X,point.Y-1))==Sv5InfillCellValue.Solid;
            }
            Sv5InfillCellValue OverlayValue(RmapSpecialWorldPoint point)
            ...
            IReadOnlyDictionary<string,Sv5LoopTopologyEvaluation> suppliedEvaluations=null,bool validateStored=true)
        {
            ...
            var final=new Dictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>(context.BaselineOccupancy);
            var affected=new HashSet<RmapSpecialWorldPoint>(); var diagnostics=new List<string>();
            foreach(var link in links) foreach(var cell in link.Cells)
            {
                ...
                for(int dy=-1;dy<=1;dy++) affected.Add(new RmapSpecialWorldPoint(cell.World.X,cell.World.Y+dy));
            }
            var finalOccupancy=new ReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>(final);
            var finalFeet=new HashSet<RmapSpecialWorldPoint>(context.BaselineFootNodes);
        ...
        public static IReadOnlyCollection<RmapSpecialWorldPoint> SupportedFootNodes(
            IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> occupancy)
        {
            var result=new HashSet<RmapSpecialWorldPoint>();
            foreach(var pair in occupancy.Where(v=>v.Value.Value==Sv5InfillCellValue.Air))
            {
                ...
                if(p.Y>0 && p.Y+1<Sv5SpaceGraphPlanner.WorldHeight &&
                    ValueAt(occupancy,new RmapSpecialWorldPoint(p.X,p.Y+1))==Sv5InfillCellValue.Air &&
                    ValueAt(occupancy,new RmapSpecialWorldPoint(p.X,p.Y-1))==Sv5InfillCellValue.Solid) result.Add(p);
            }
            return Array.AsReadOnly(result.OrderBy(v=>v).ToArray());
        }

        private static bool IsSupportedFoot(
            IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> occupancy,RmapSpecialWorldPoint point)
            => point.Y>0 && point.Y+1<Sv5SpaceGraphPlanner.WorldHeight &&
               ValueAt(occupancy,point)==Sv5InfillCellValue.Air &&
               ValueAt(occupancy,new RmapSpecialWorldPoint(point.X,point.Y+1))==Sv5InfillCellValue.Air &&
               ValueAt(occupancy,new RmapSpecialWorldPoint(point.X,point.Y-1))==Sv5InfillCellValue.Solid;

        private static int ShortestCostOverlay(RmapSpecialWorldPoint start,RmapSpecialWorldPoint goal,
            Sv5LoopTopologyContext context,Func<RmapSpecialWorldPoint,bool> isFoot)
        {
            if(!isFoot(start) || !isFoot(goal)) return -1;
            var queue=new Queue<RmapSpecialWorldPoint>();
            var distance=new Dictionary<RmapSpecialWorldPoint,int>{{start,1}}; queue.Enqueue(start);
            while(queue.Count!=0)
            {
        ...
        public static int ShortestCost(RmapSpecialWorldPoint start,RmapSpecialWorldPoint goal,
            IEnumerable<RmapSpecialWorldPoint> sourceNodes)
        {
            var nodes=sourceNodes as ISet<RmapSpecialWorldPoint> ?? new HashSet<RmapSpecialWorldPoint>(sourceNodes);
            if(!nodes.Contains(start) || !nodes.Contains(goal)) return -1;
            var queue=new Queue<RmapSpecialWorldPoint>(); var distance=new Dictionary<RmapSpecialWorldPoint,int>{{start,1}};
            queue.Enqueue(start);
            while(queue.Count!=0)
            {
        ...
        public static IReadOnlyCollection<RmapSpecialWorldPoint> ReachableFootNodes(
            IEnumerable<RmapSpecialWorldPoint> sourceNodes,RmapSpecialWorldPoint start)
        {
            var nodes=new HashSet<RmapSpecialWorldPoint>(sourceNodes ?? Array.Empty<RmapSpecialWorldPoint>());
        ...
        private static void CarveHostComponentNetworks(Sv5SpaceGraphPlan plan,
            IDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> cells)
        {
            ...
            var initialView=new ReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>(
                (Dictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>)cells);
            var initialFeet=new HashSet<RmapSpecialWorldPoint>(SupportedFootNodes(initialView));
            var componentByFoot=new Dictionary<RmapSpecialWorldPoint,int>();
            var componentCells=new Dictionary<int,HashSet<RmapSpecialWorldPoint>>(); int componentId=0;
            ...
            foreach(string host in roomHosts.Values.Distinct(StringComparer.Ordinal).OrderBy(v=>v,StringComparer.Ordinal))
            {
                ...
                var network=new HashSet<RmapSpecialWorldPoint>(componentCells[ids[0]]);
                foreach(int id in ids.Skip(1))
                {
                    var view=new ReadOnlyDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>(
                        (Dictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell>)cells);
                    var existing=new HashSet<RmapSpecialWorldPoint>(SupportedFootNodes(view));
                    ...
                    IReadOnlyList<RmapSpecialWorldPoint> path=Array.AsReadOnly(Array.Empty<RmapSpecialWorldPoint>());
                    foreach(int margin in new[]{4,8,16,32,64})
                    {
                        ...
                        var possible=new HashSet<RmapSpecialWorldPoint>(existing);
                    ...
                    {
                        SetBackbone(new RmapSpecialWorldPoint(p.X,p.Y-1),Sv5InfillCellValue.Solid,
                            "HOST_COMPONENT_SUPPORT",host,cells);
                        SetBackbone(p,Sv5InfillCellValue.Air,"HOST_COMPONENT_AIR",host,cells);
                        SetBackbone(new RmapSpecialWorldPoint(p.X,p.Y+1),Sv5InfillCellValue.Air,
        ...
        private static bool CanPlaceFootAnywhere(RmapSpecialWorldPoint p,
            IDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> cells)
            => CanAssignAnywhere(p,Sv5InfillCellValue.Air,cells) &&
               CanAssignAnywhere(new RmapSpecialWorldPoint(p.X,p.Y+1),Sv5InfillCellValue.Air,cells) &&
               CanAssignAnywhere(new RmapSpecialWorldPoint(p.X,p.Y-1),Sv5InfillCellValue.Solid,cells);

        private static bool CanAssignAnywhere(RmapSpecialWorldPoint p,Sv5InfillCellValue value,
            IDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> cells)
        {
        ...
        private static void SetBackbone(RmapSpecialWorldPoint p,Sv5InfillCellValue value,string provenance,
            string owner,IDictionary<RmapSpecialWorldPoint,Sv5LoopOccupancyCell> cells)
        {
        ...
        private static HashSet<RmapSpecialWorldPoint> Reachable(ISet<RmapSpecialWorldPoint> nodes,
            IEnumerable<RmapSpecialWorldPoint> starts)
        {
            var reached=new HashSet<RmapSpecialWorldPoint>(starts.Where(nodes.Contains));
            var queue=new Queue<RmapSpecialWorldPoint>(reached);
            while(queue.Count!=0)
            {
                var p=queue.Dequeue();
        ...
        private static IReadOnlyList<RmapSpecialWorldPoint> FindPath(ISet<RmapSpecialWorldPoint> nodes,
            IEnumerable<RmapSpecialWorldPoint> starts,ISet<RmapSpecialWorldPoint> targets)
        {
            var queue=new Queue<RmapSpecialWorldPoint>();
            var parent=new Dictionary<RmapSpecialWorldPoint,RmapSpecialWorldPoint>();
            ...
            RmapSpecialWorldPoint found=default(RmapSpecialWorldPoint); bool success=false;
            ...
            if(!success) return Array.AsReadOnly(Array.Empty<RmapSpecialWorldPoint>());
            var result=new List<RmapSpecialWorldPoint>{found};
        ...
        internal static IEnumerable<RmapSpecialWorldPoint> FootNeighbors(RmapSpecialWorldPoint p)
        {
            foreach(int dx in new[]{-1,1}) foreach(int dy in new[]{-1,0,1})
                yield return new RmapSpecialWorldPoint(p.X+dx,p.Y+dy);
        }
