...
namespace StarNight.Map.WorldGeneration.SectorPlanning
{
    ...
    public sealed class Sv5HubProfile
    {
        ...
        public string Digest => Sv5WorldDefinition.Hash("SV5_HUB_SHELL_PROFILE_V1|624|416|12|30|24|40|4|3|3|6|7|"+
            TargetConnections+"|"+MinimumConnections+"|"+MaximumConnections);
    }

    public sealed class Sv5HubCell : IComparable<Sv5HubCell>
    {
        internal Sv5HubCell(string hubId, Sv5SpecialWorldPoint world, Sv5HubCellRole role,
    ...
    {
        internal Sv5HubSocket(string id, Sv5HubSide side, Sv5HubTier tier, Sv5SpecialWorldPoint anchor,
        ...
        public Sv5SpecialWorldPoint Anchor { get; }
    ...
    {
        internal Sv5HubPort(string id, string socketId, Sv5SpecialWorldPoint anchor)
    ...
    public sealed class Sv5HubConnection : IComparable<Sv5HubConnection>
    {
        ...
        private readonly ReadOnlyCollection<Sv5MovementEdge> movementWitness;
        ...
        internal Sv5HubConnection(string id, string portId, string socketId, string externalRoomId,
            string externalSpaceGroupId, Sv5SpecialWorldPoint portAnchor, Sv5SpecialWorldPoint externalAnchor,
        ...
        public Sv5SpecialWorldPoint PortAnchor { get; }
        public Sv5SpecialWorldPoint ExternalAnchor { get; }
        public IReadOnlyList<Sv5SpecialWorldPoint> Centerline => centerline;
        public IReadOnlyList<Sv5MovementEdge> MovementWitness => movementWitness;
        ...
        public string DigestToken => Id+"|"+PortId+"|"+ExternalRoomId+"|"+ExternalSpaceGroupId+"|"+
            ...
            string.Join(";",MovementWitness.Select(edge=>edge.DigestToken))+"|"+
    ...
    {
        private readonly ReadOnlyCollection<Sv5SpecialWorldPoint> cells;
        internal Sv5HubConstraintSet(string category,IEnumerable<Sv5SpecialWorldPoint> source)
        {Category=category;cells=Array.AsReadOnly(source.Distinct().OrderBy(value=>value).ToArray());}
        public string Category { get; }
        public IReadOnlyList<Sv5SpecialWorldPoint> Cells => cells;
    ...
    public sealed class Sv5HubShellPlan
    {
            ...
            IEnumerable<Sv5HubConstraintSet> constraintSets, IEnumerable<string> diagnostics, Sv5HubPerformance performance)
        {
            ...
            Digest=Sv5WorldDefinition.Hash("SV5_HUB_SHELL_ACTUAL_CELL_V1\n"+BaselineDigest+"\n"+Profile.Digest+"\n"+
        ...
        public bool Success => Diagnostics.Count==0 && HubId.Length!=0 && Sockets.Count==6 && Ports.Count>=4 && Ports.Count<=6 &&
            ...
            Connections.Select(v=>v.ExternalAnchor).Distinct().Count()==Connections.Count &&
    ...
    public static class Sv5HubShell
    {
        ...
        {
            public Sv5HubSide Side; public Sv5HubTier Tier; public Sv5SpecialWorldPoint Anchor; public int Height; public int ApertureStart;
        }
        private sealed class ExternalTarget : IComparable<ExternalTarget>
        {
            ...
            public Sv5SpecialWorldPoint World; public int ComponentId;
        ...
        {
            public HashSet<Sv5SpecialWorldPoint> ProtectedBody;
            public HashSet<Sv5SpecialWorldPoint> Type0;
            public HashSet<Sv5SpecialWorldPoint> Progression;
            public HashSet<Sv5SpecialWorldPoint> EndpointForbidden;
            public HashSet<Sv5SpecialWorldPoint> FootprintBlocked;
        ...
        public static Sv5HubShellPlan Build(Sv5SpaceGraphPlan plan, Sv5HubProfile profile = null)
        {
            ...
            if(chosen==null)
            {
                ...
                var empty=new ReadOnlyDictionary<Sv5SpecialWorldPoint,Sv5LoopOccupancyCell>(new Dictionary<Sv5SpecialWorldPoint,Sv5LoopOccupancyCell>(baseline));
            ...
            string hubId="SV5_HUB_"+Sv5WorldDefinition.Hash("SV5_HUB|"+plan.Seed+"|"+chosen.Candidate.Bounds+"|"+
            ...
            var externalAnchors=new HashSet<Sv5SpecialWorldPoint>(assignments.Select(value=>value.Endpoint.World));
            ...
            var final=new Dictionary<Sv5SpecialWorldPoint,Sv5LoopOccupancyCell>(baseline);
            ...
            foreach(var cell in connections.SelectMany(connection=>connection.Cells).Where(cell=>cell.Changed||
                cell.Role==Sv5HubConnectionCellRole.Support))
            ...
            return new Sv5HubShellPlan(plan.Digest,profile,attempts.Select(v=>v.Candidate),hubId,chosen.Candidate.Bounds,
                cells,sockets,ports,connections,baseline,new ReadOnlyDictionary<Sv5SpecialWorldPoint,Sv5LoopOccupancyCell>(final),
        ...
        private static EvaluationBatch Enumerate(Sv5SpaceGraphPlan plan,Sv5HubProfile profile,
            ...
            IReadOnlyDictionary<Sv5SpecialWorldPoint,int> closedComponents)
        {
            ...
            {
                string id="HUB_CANDIDATE_"+Sv5WorldDefinition.Hash(plan.Seed+"|"+bounds).Substring(0,16);
                ...
                void Search(int optionIndex,List<Assignment> current,HashSet<string> rooms,HashSet<string> groups,
                    HashSet<Sv5SpecialWorldPoint> anchors,HashSet<Sv5SpecialWorldPoint> reserved,int? component)
                {
                    ...
                    {
                        if(rooms.Contains(endpoint.RoomId)||groups.Contains(endpoint.SpaceGroupId)||
                            anchors.Contains(endpoint.World))continue;
                        ...
                        var occupied=selected.Route.Cells.Select(cell=>cell.World).Where(cell=>!bounds.Contains(cell)&&
                            !cell.Equals(endpoint.World)).Distinct().ToArray();
                        current.Add(selected);rooms.Add(endpoint.RoomId);groups.Add(endpoint.SpaceGroupId);
                        anchors.Add(endpoint.World);reserved.UnionWith(occupied);
                        Search(optionIndex+1,current,rooms,groups,anchors,reserved,component??endpointComponent);
                        reserved.ExceptWith(occupied);anchors.Remove(endpoint.World);
                        groups.Remove(endpoint.SpaceGroupId);rooms.Remove(endpoint.RoomId);
                    ...
                    Search(optionIndex+1,current,rooms,groups,anchors,reserved,component);
                }
                Search(0,new List<Assignment>(),new HashSet<string>(StringComparer.Ordinal),
                    new HashSet<string>(StringComparer.Ordinal),new HashSet<Sv5SpecialWorldPoint>(),
                    new HashSet<Sv5SpecialWorldPoint>(),null);
            ...
            IEnumerable<ExternalTarget> endpoints)
        {
            ...
            int Fallback(ExternalTarget endpoint)=>endpoint.Id.StartsWith("COMPONENT_",StringComparison.Ordinal)?1:0;
            var groups=endpoints.Where(endpoint=>!bounds.Contains(endpoint.World))
                .Where(endpoint=>!endpoint.SpaceGroupId.EndsWith("RETURN_TO_START",StringComparison.Ordinal))
                ...
                {
                    Id=group.Key,
                    Endpoints=group.OrderBy(Fallback).ThenBy(Score)
                        .ThenBy(endpoint=>endpoint.World).ThenBy(endpoint=>endpoint.RoomId,StringComparer.Ordinal)
                        .ThenBy(endpoint=>endpoint.Id,StringComparer.Ordinal).ToArray()
                })
                .OrderBy(group=>group.Endpoints.Length==0?int.MaxValue:Score(group.Endpoints[0]))
                .ThenBy(group=>group.Id,StringComparer.Ordinal).ToArray();
            var result=new List<ExternalTarget>();var usedWorlds=new HashSet<Sv5SpecialWorldPoint>();
            for(int round=0;round<4;round++)
            {
                foreach(var group in groups)
                {
                    var endpoint=group.Endpoints.Skip(round).FirstOrDefault(value=>!usedWorlds.Contains(value.World));
                    if(endpoint==null)continue;
                    result.Add(endpoint);usedWorlds.Add(endpoint.World);
                }
            }
            return result;
        }
            ...
            IReadOnlyDictionary<Sv5SpecialWorldPoint,int> closedComponents)
        {
            ...
            bool Supported(Sv5SpecialWorldPoint point)=>point.Y>0&&point.Y+1<Sv5SpaceGraphPlanner.WorldHeight&&
                Sv5LoopTopology.ValueAt(occupancy,point)==Sv5InfillCellValue.Air&&
                Sv5LoopTopology.ValueAt(occupancy,new Sv5SpecialWorldPoint(point.X,point.Y+1))==Sv5InfillCellValue.Air&&
                Sv5LoopTopology.ValueAt(occupancy,new Sv5SpecialWorldPoint(point.X,point.Y-1))==Sv5InfillCellValue.Solid;
            bool Ingress(Sv5SpecialWorldPoint end,Sv5SpecialWorldPoint point)
            {
                ...
                var head=new Sv5SpecialWorldPoint(point.X,point.Y+1);if(head.Equals(end))return true;
        ...
        private static SocketSpec[] Specs(Sv5SpaceBounds bounds)
        {
            ...
            var result=new List<SocketSpec>();foreach(var side in new[]{Sv5HubSide.Left,Sv5HubSide.Right})for(int i=0;i<3;i++)
                result.Add(new SocketSpec{Side=side,Tier=tiers[i],Height=heights[i],ApertureStart=bounds.Y+starts[i],
                    Anchor=new Sv5SpecialWorldPoint(side==Sv5HubSide.Left?bounds.X:bounds.MaxXExclusive-1,bounds.Y+starts[i]+heights[i]/2)});
            return result.ToArray();
        }
        private static bool Same(SocketSpec first,SocketSpec second)=>first.Side==second.Side&&first.Tier==second.Tier;
        private static bool FootprintBlocked(Sv5SpaceBounds bounds,ISet<Sv5SpecialWorldPoint> blocked)
        {
            for(int y=bounds.Y;y<bounds.MaxYExclusive;y++)for(int x=bounds.X;x<bounds.MaxXExclusive;x++)
                if(blocked.Contains(new Sv5SpecialWorldPoint(x,y)))return true;return false;
        }
        private static ConstraintIndex BuildConstraints(Sv5SpaceGraphPlan plan)
        {
            var protectedCells=new HashSet<Sv5SpecialWorldPoint>(Sv5SpaceSidepaths.ProtectedCells(plan));
            var type0=new HashSet<Sv5SpecialWorldPoint>(plan.Core.RouteSource.Secrets.SelectMany(secret=>secret.Chunks)
            ...
            var progression=new HashSet<Sv5SpecialWorldPoint>(plan.Gates.SelectMany(gate=>gate.BlockingCells)
                .Concat(plan.Gates.SelectMany(gate=>gate.BlockingFaces).SelectMany(face=>new[]{face.First,face.Second})));
            var core=new HashSet<Sv5SpecialWorldPoint>(plan.Core.CoreCells.Select(cell=>cell.World)
                .Concat(plan.Core.RouteCells.Select(cell=>cell.World)));
            var infill=new HashSet<Sv5SpecialWorldPoint>(plan.Infill.Cells.Select(cell=>cell.World));
            var loops=new HashSet<Sv5SpecialWorldPoint>(plan.Loops.Cells.Select(cell=>cell.World));
            var sidepaths=new HashSet<Sv5SpecialWorldPoint>(plan.Sidepaths.Cells.Select(cell=>cell.World));
            var reservations=new HashSet<Sv5SpecialWorldPoint>(plan.Reservations
            ...
            var protectedBody=new HashSet<Sv5SpecialWorldPoint>(protectedCells);protectedBody.UnionWith(core);
            ...
            var endpointForbidden=new HashSet<Sv5SpecialWorldPoint>(protectedCells);endpointForbidden.UnionWith(core);
            endpointForbidden.UnionWith(type0);endpointForbidden.UnionWith(progression);
            var footprint=new HashSet<Sv5SpecialWorldPoint>(protectedBody);footprint.UnionWith(type0);footprint.UnionWith(progression);
        ...
        private static IReadOnlyDictionary<Sv5SpecialWorldPoint,int> ClosedComponents(Sv5SpaceGraphPlan plan)
        {
            var physical=Sv5SpaceLoops.ApplyPhysicalCells(plan.Connections,plan.Loops);
            physical=Sv5SpaceSidepaths.ApplyPhysicalCells(physical,plan.Sidepaths);
            var passage=new HashSet<Sv5SpecialWorldPoint>(physical.SelectMany(connection=>connection.Centerline.Concat(connection.ApertureCells)));
            var blockedCells=new HashSet<Sv5SpecialWorldPoint>(plan.Gates.SelectMany(gate=>gate.BlockingCells));
            ...
            passage.ExceptWith(blockedCells);var result=new Dictionary<Sv5SpecialWorldPoint,int>();int component=0;
            foreach(var start in passage.OrderBy(value=>value))
            {
                if(result.ContainsKey(start))continue;var queue=new Queue<Sv5SpecialWorldPoint>();queue.Enqueue(start);result[start]=component;
        ...
        private static string Face(Sv5SpecialWorldPoint first,Sv5SpecialWorldPoint second)=>first.CompareTo(second)<=0?first+">"+second:second+">"+first;
        ...
        private static IReadOnlyList<Sv5HubCell> BuildCells(string hubId,Sv5SpaceBounds bounds,
            IEnumerable<Sv5HubSocket> sockets,IReadOnlyDictionary<Sv5SpecialWorldPoint,Sv5LoopOccupancyCell> baseline)
        {
            var roles=new Dictionary<Sv5SpecialWorldPoint,Sv5HubCellRole>();
            for(int y=bounds.Y;y<bounds.MaxYExclusive;y++)for(int x=bounds.X;x<bounds.MaxXExclusive;x++)
                roles[new Sv5SpecialWorldPoint(x,y)]=Sv5HubCellRole.Solid;
            for(int y=bounds.Y+5;y<bounds.Y+35;y++)for(int x=bounds.X+6;x<bounds.X+18;x++)
                roles[new Sv5SpecialWorldPoint(x,y)]=Sv5HubCellRole.Air;
            for(int y=bounds.Y+8;y<bounds.Y+32;y++)for(int x=bounds.X+10;x<bounds.X+14;x++)
                roles[new Sv5SpecialWorldPoint(x,y)]=Sv5HubCellRole.ReservedTree;
            foreach(var socket in sockets.Where(v=>v.Active))
            {
                ...
                for(int y=start;y<start+socket.ApertureHeight;y++)for(int x=minX;x<=maxX;x++)
                    roles[new Sv5SpecialWorldPoint(x,y)]=Sv5HubCellRole.PortNeck;
            }
        ...
        {
            var passable=new HashSet<Sv5SpecialWorldPoint>(cells.Where(v=>v.Role!=Sv5HubCellRole.Solid).Select(v=>v.World));
            var anchors=ports.Select(v=>v.Anchor).ToArray();if(anchors.Length==0||anchors.Any(v=>!passable.Contains(v)))return false;
            var visited=new HashSet<Sv5SpecialWorldPoint>{anchors[0]};var queue=new Queue<Sv5SpecialWorldPoint>();queue.Enqueue(anchors[0]);
