...
namespace StarNight.Map.WorldGeneration.SectorPlanning
{
    ...
    {
        public Sv5InfillCell(RmapSpecialWorldPoint world, Sv5InfillCellValue value, string owner,
    ...
    public sealed class Sv5InfillPattern
    {
        public Sv5InfillPattern(ushort mask, IEnumerable<Sv5InfillCellValue> values)
        {
            ...
            Digest = RmapWorldDefinition.Hash(mask + "|" + string.Join(",", cells.Select(v => (int)v)));
            Id = "INFILL_" + Digest;
        }
    ...
    {
        public Sv5InfillInstance(Sv5InfillPattern pattern, RmapSpecialWorldPoint origin,
    ...
    {
        internal Sv5InfillStaticProof(IEnumerable<RmapSpecialWorldPoint> approach,
            IEnumerable<RmapSpecialWorldPoint> recovery, string reason)
        { Approach = Array.AsReadOnly(approach.ToArray()); Return = Array.AsReadOnly(recovery.ToArray()); Reason = reason; }
        public IReadOnlyList<RmapSpecialWorldPoint> Approach { get; }
        public IReadOnlyList<RmapSpecialWorldPoint> Return { get; }
    ...
    public static class Sv5InfillPatterns
    {
        ...
        public static IReadOnlyList<Sv5InfillCell> Recipe(string recipe, RmapSpecialWorldPoint origin,
            bool mirror = false, string owner = "FIXTURE", string parent = "", string host = "", bool childDoor = false)
        {
            ...
            for (int x = 0; x < bounds.Width; x++)
            {
                ...
                cells.Add(new Sv5InfillCell(new RmapSpecialWorldPoint(origin.X+(mirror ? bounds.Width-1-x : x),origin.Y+y),
        ...
        public static IReadOnlyList<Sv5InfillInstance> Split(IEnumerable<Sv5InfillCell> source, bool mirror = false)
        {
                         ...
                         .OrderBy(g => g.Key.Owner,StringComparer.Ordinal).ThenBy(g => g.Key.Y).ThenBy(g => g.Key.X))
            {
                ...
                result.Add(new Sv5InfillInstance(canonical,new RmapSpecialWorldPoint(group.Key.X,group.Key.Y),
                    first.Owner,first.Recipe,first.Parent,first.Host,mirror));
            }
            return Array.AsReadOnly(result.ToArray());
        }

        public static IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5InfillCellValue> Reconstruct(IEnumerable<Sv5InfillInstance> instances)
        {
            var result = new Dictionary<RmapSpecialWorldPoint,Sv5InfillCellValue>();
            foreach (var instance in instances)
            for (int i = 0; i < 16; i++)
            {
                if (((instance.Pattern.Mask >> i) & 1) == 0) continue;
                var point = new RmapSpecialWorldPoint(instance.Origin.X+i%4,instance.Origin.Y+i/4);
            ...
            return new ReadOnlyDictionary<RmapSpecialWorldPoint,Sv5InfillCellValue>(result);
        }

        public static IReadOnlyList<RmapSpecialWorldPoint> SolidWindows(
            IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5InfillCellValue> cells, int width = 624, int height = 416)
        {
            ...
            for (int y = 0; y < height; y++) for (int x = 0; x < width; x++)
                sums[x+1,y+1] = sums[x,y+1]+sums[x+1,y]-sums[x,y]+(cells.TryGetValue(new RmapSpecialWorldPoint(x,y),out var v) && v == Sv5InfillCellValue.Solid ? 1 : 0);
            var failures = new List<RmapSpecialWorldPoint>();
            for (int y = 0; y <= height-6; y++) for (int x = 0; x <= width-6; x++)
                if (sums[x+6,y+6]-sums[x,y+6]-sums[x+6,y]+sums[x,y] == 36) failures.Add(new RmapSpecialWorldPoint(x,y));
            return Array.AsReadOnly(failures.ToArray());
        }

        public static Sv5InfillStaticProof Screen(IReadOnlyDictionary<RmapSpecialWorldPoint,Sv5InfillCellValue> cells,
            RmapSpecialWorldPoint start, RmapSpecialWorldPoint goal)
        {
            bool Is(int x,int y,Sv5InfillCellValue value) => cells.TryGetValue(new RmapSpecialWorldPoint(x,y),out var v) && v == value;
            bool Stand(RmapSpecialWorldPoint p) => Is(p.X,p.Y-1,Sv5InfillCellValue.Solid) &&
            ...
            bool Sweep(RmapSpecialWorldPoint a,RmapSpecialWorldPoint b)
            {
            ...
            RmapSpecialWorldPoint[] Search(RmapSpecialWorldPoint from,RmapSpecialWorldPoint to)
            {
                if (!Stand(from) || !Stand(to)) return Array.Empty<RmapSpecialWorldPoint>();
                var queue = new Queue<RmapSpecialWorldPoint>(); var previous = new Dictionary<RmapSpecialWorldPoint,RmapSpecialWorldPoint>();
                ...
                while (queue.Count > 0 && !previous.ContainsKey(to))
                {
                    ...
                    {
                        var b = new RmapSpecialWorldPoint(a.X+dx,a.Y+dy);
                        if (previous.ContainsKey(b) || !Stand(b) || !Sweep(a,b)) continue;
                        previous.Add(b,a); queue.Enqueue(b);
                    }
                }
                if (!previous.ContainsKey(to)) return Array.Empty<RmapSpecialWorldPoint>();
                var path = new List<RmapSpecialWorldPoint> { to };
