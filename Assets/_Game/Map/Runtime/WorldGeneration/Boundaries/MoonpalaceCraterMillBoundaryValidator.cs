...
{
    public sealed class MoonpalaceCraterMillBoundaryValidator
    {
        public MoonpalaceCraterMillBoundaryContentReport Validate(
            MoonpalaceCraterMillBoundaryAuthoringData data)
        {
            if (data == null) throw new ArgumentNullException(nameof(data));

            var issues = new List<string>();
            var expectedCandidateIds = MoonpalaceCraterMillBoundaryAuthoringContract.CandidateIds;
            var expectedMicrochunkIds = MoonpalaceCraterMillBoundaryAuthoringContract.MicrochunkIds;
            ...
            foreach (var profile in MoonpalaceCraterMillBoundaryAuthoringContract.ProfileIds)
            {
            ...
            for (var index = 0; index < candidates.Count; index++)
            {
                ...
                var expectedProfile = MoonpalaceCraterMillBoundaryAuthoringContract.ProfileIds[expectedIndex / 2];
                ...
                Require(row.BiomeAId == MoonpalaceCraterMillBoundaryAuthoringContract.BiomeAId, row.CandidateId + ": biome A mismatch.", issues);
                Require(row.BiomeBId == MoonpalaceCraterMillBoundaryAuthoringContract.BiomeBId, row.CandidateId + ": biome B mismatch.", issues);
                ...
                Require(row.ToolRequirement == MoonpalaceCraterMillBoundaryAuthoringContract.NoToolRequirement,
                    row.CandidateId + ": tool requirement must be NONE.", issues);
            }

            foreach (var row in microchunks)
            {
                if (!MoonpalaceCraterMillBoundaryAuthoringContract.IsOwnedMicrochunk(row.MicrochunkId)) continue;
                ...
                Require(row.BiomeIds == "BIO_MOON_CRATER|BIO_ABANDONED_MILL", row.MicrochunkId + ": biome IDs mismatch.", issues);
            ...
            foreach (var microchunkId in expectedMicrochunkIds)
            {
                ...
                Require(ownedTiles.Count == MoonpalaceCraterMillBoundaryAuthoringContract.CellsPerMicrochunk,
                ...
                Require(coordinateCount == MoonpalaceCraterMillBoundaryAuthoringContract.CellsPerMicrochunk,
            ...
            return new MoonpalaceCraterMillBoundaryContentReport(
                candidateIds,
                microchunkIds,
                candidates.Count,
                matrixComplete,
