...
{
    public sealed class MoonpalaceMillDoughBoundaryValidator
    {
        public MoonpalaceMillDoughBoundaryContentReport Validate(
            MoonpalaceMillDoughBoundaryAuthoringData data)
        {
            if (data == null) throw new ArgumentNullException(nameof(data));

            var issues = new List<string>();
            var expectedCandidateIds = MoonpalaceMillDoughBoundaryAuthoringContract.CandidateIds;
            var expectedMicrochunkIds = MoonpalaceMillDoughBoundaryAuthoringContract.MicrochunkIds;
            ...
            for (var index = 0; index < candidates.Count; index++)
            {
                ...
                var expectedCandidate = MoonpalaceMillDoughBoundaryCandidateMatrix.Canonical.Candidates
                ...
                Require(row.BiomeAId == MoonpalaceMillDoughBoundaryAuthoringContract.BiomeAId, row.CandidateId + ": biome A mismatch.", issues);
                Require(row.BiomeBId == MoonpalaceMillDoughBoundaryAuthoringContract.BiomeBId, row.CandidateId + ": biome B mismatch.", issues);
                ...
                Require(row.ToolRequirement == MoonpalaceMillDoughBoundaryAuthoringContract.NoToolRequirement,
                    row.CandidateId + ": tool requirement must be NONE.", issues);
            }

            foreach (var row in microchunks)
            {
                if (!MoonpalaceMillDoughBoundaryAuthoringContract.IsOwnedMicrochunk(row.MicrochunkId)) continue;
                ...
                Require(row.BiomeIds == "BIO_ABANDONED_MILL|BIO_MOON_DOUGH", row.MicrochunkId + ": biome IDs mismatch.", issues);
            ...
            foreach (var microchunkId in expectedMicrochunkIds)
            {
                ...
                Require(ownedTiles.Count == MoonpalaceMillDoughBoundaryAuthoringContract.CellsPerMicrochunk,
                ...
                Require(coordinateCount == MoonpalaceMillDoughBoundaryAuthoringContract.CellsPerMicrochunk,
                    microchunkId + ": tile coordinates must cover 12x8 exactly.", issues);

                var tileEvidence = ownedTiles.Any(row => row.GroundCode == "G_MILL_METAL") &&
                                   ownedTiles.Any(row => row.GroundCode == "G_DOUGH_SOLID");
                var backgroundEvidence = ownedTiles.Any(row => row.DecorBackCode == "DB_MILL") &&
            ...
            Require(data.RootDoughRowsModified == 0, "Root/Dough rows must not be modified.", issues);
            ...
            return new MoonpalaceMillDoughBoundaryContentReport(
                candidateIds,
                microchunkIds,
                candidates.Count,
                matrixComplete,
