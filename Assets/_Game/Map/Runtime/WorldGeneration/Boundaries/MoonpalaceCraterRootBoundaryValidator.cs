...
{
    public sealed class MoonpalaceCraterRootBoundaryValidator
    {
        public MoonpalaceCraterRootBoundaryContentReport Validate(
            MoonpalaceCraterRootBoundaryAuthoringData data)
        {
            if (data == null) throw new ArgumentNullException(nameof(data));

            var issues = new List<string>();
            var expectedCandidateIds = MoonpalaceCraterRootBoundaryAuthoringContract.CandidateIds;
            var expectedMicrochunkIds = MoonpalaceCraterRootBoundaryAuthoringContract.MicrochunkIds;
            ...
            foreach (var profile in MoonpalaceCraterRootBoundaryAuthoringContract.ProfileIds)
            {
            ...
            for (var index = 0; index < candidates.Count; index++)
            {
                ...
                var expectedProfile = MoonpalaceCraterRootBoundaryAuthoringContract.ProfileIds[expectedIndex / 2];
                ...
                Require(row.BiomeAId == MoonpalaceCraterRootBoundaryAuthoringContract.BiomeAId, row.CandidateId + ": biome A mismatch.", issues);
                Require(row.BiomeBId == MoonpalaceCraterRootBoundaryAuthoringContract.BiomeBId, row.CandidateId + ": biome B mismatch.", issues);
                ...
                Require(row.ToolRequirement == MoonpalaceCraterRootBoundaryAuthoringContract.NoToolRequirement,
                    row.CandidateId + ": tool requirement must be NONE.", issues);
            }

            foreach (var row in microchunks)
            {
                if (!MoonpalaceCraterRootBoundaryAuthoringContract.IsOwnedMicrochunk(row.MicrochunkId)) continue;
                ...
                Require(row.BiomeIds == "BIO_MOON_CRATER|BIO_CASSIA_ROOT", row.MicrochunkId + ": biome IDs mismatch.", issues);
            ...
            foreach (var microchunkId in expectedMicrochunkIds)
            {
                ...
                Require(ownedTiles.Count == MoonpalaceCraterRootBoundaryAuthoringContract.CellsPerMicrochunk,
                ...
                Require(coordinateCount == MoonpalaceCraterRootBoundaryAuthoringContract.CellsPerMicrochunk,
                ...
                var tileEvidence = ownedTiles.Any(row => row.GroundCode == "G_MOON_ROCK") &&
                                   ownedTiles.Any(row => row.GroundCode == "G_CASSIA_WOOD");
                var backgroundEvidence = ownedTiles.Any(row => row.DecorBackCode == "DB_CRATER") &&
                                         ownedTiles.Any(row => row.DecorBackCode == "DB_ROOT");
            ...
            return new MoonpalaceCraterRootBoundaryContentReport(
                candidateIds,
                microchunkIds,
                candidates.Count,
                matrixComplete,
