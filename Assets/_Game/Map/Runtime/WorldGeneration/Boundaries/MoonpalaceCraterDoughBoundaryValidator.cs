...
{
    public sealed class MoonpalaceCraterDoughBoundaryValidator
    {
        public MoonpalaceCraterDoughBoundaryContentReport Validate(
            MoonpalaceCraterDoughBoundaryAuthoringData data)
        {
            if (data == null) throw new ArgumentNullException(nameof(data));

            var issues = new List<string>();
            var expectedCandidateIds = MoonpalaceCraterDoughBoundaryAuthoringContract.CandidateIds;
            var expectedMicrochunkIds = MoonpalaceCraterDoughBoundaryAuthoringContract.MicrochunkIds;
            ...
            var expectedMatrix = new HashSet<string>(
                MoonpalaceCraterDoughBoundaryAuthoringContract.ProfileOrientationKeys,
            ...
            for (var index = 0; index < candidates.Count; index++)
            {
                ...
                var expectedOrientation =
                    MoonpalaceCraterDoughBoundaryAuthoringContract.CandidateOrientations[expectedIndex];
                var expectedProfile =
                    MoonpalaceCraterDoughBoundaryAuthoringContract.CandidateProfileIds[expectedIndex];
                ...
                Require(row.BiomeAId == MoonpalaceCraterDoughBoundaryAuthoringContract.BiomeAId, row.CandidateId + ": biome A mismatch.", issues);
                Require(row.BiomeBId == MoonpalaceCraterDoughBoundaryAuthoringContract.BiomeBId, row.CandidateId + ": biome B mismatch.", issues);
                ...
                Require(row.ToolRequirement == MoonpalaceCraterDoughBoundaryAuthoringContract.NoToolRequirement,
                    row.CandidateId + ": tool requirement must be NONE.", issues);
            }

            foreach (var row in microchunks)
            {
                if (!MoonpalaceCraterDoughBoundaryAuthoringContract.IsOwnedMicrochunk(row.MicrochunkId)) continue;
                ...
                Require(row.BiomeIds == "BIO_MOON_CRATER|BIO_MOON_DOUGH", row.MicrochunkId + ": biome IDs mismatch.", issues);
            ...
            foreach (var microchunkId in expectedMicrochunkIds)
            {
                ...
                Require(ownedTiles.Count == MoonpalaceCraterDoughBoundaryAuthoringContract.CellsPerMicrochunk,
                ...
                Require(coordinateCount == MoonpalaceCraterDoughBoundaryAuthoringContract.CellsPerMicrochunk,
                    microchunkId + ": tile coordinates must cover 12x8 exactly.", issues);

                var tileEvidence = ownedTiles.Any(row => row.GroundCode == "G_MOON_ROCK") &&
                                   ownedTiles.Any(row => row.GroundCode == "G_DOUGH_SOLID");
                var backgroundEvidence = ownedTiles.Any(row => row.DecorBackCode == "DB_CRATER") &&
            ...
            for (var index = 0; index < expectedMicrochunkIds.Count; index++)
            {
                ...
                var horizontal = MoonpalaceCraterDoughBoundaryAuthoringContract.CandidateOrientations[index] ==
            ...
            var invalidLayerHorizontalCandidateCount = candidates.Count(row =>
                row.ProfileId == "BOUND_LAYER" &&
                row.Orientation == MoonpalaceBoundaryOrientation.Horizontal);
            Require(invalidLayerHorizontalCandidateCount == 0,
                "BOUND_LAYER/HORIZONTAL candidates are forbidden.", issues);
            ...
            return new MoonpalaceCraterDoughBoundaryContentReport(
                candidateIds,
                microchunkIds,
                candidates.Count,
                matrixComplete,
                ...
                invalidLayerHorizontalCandidateCount,
                issues);
        }
