...
{
    public sealed class Sv5PatternMaskCandidate
    {
        internal Sv5PatternMaskCandidate(
        ...
        {
            Mask = mask;
            CandidateId = "SV5_MP_" + mask.ToString("X4", CultureInfo.InvariantCulture);
        ...
        internal Sv5PatternMaskCandidate WithRank(int rank) => new Sv5PatternMaskCandidate(
    ...
    public sealed class Sv5PatternMaskCandidateSet
    {
        internal Sv5PatternMaskCandidateSet(int eligibleCount, IEnumerable<Sv5PatternMaskCandidate> candidates,
            IReadOnlyDictionary<string, int> rejectionCounts, string digest)
        {
            EligibleMaskCount = eligibleCount;
            Candidates = new ReadOnlyCollection<Sv5PatternMaskCandidate>(candidates.ToList());
            RejectionCounts = new ReadOnlyDictionary<string, int>(new Dictionary<string, int>(rejectionCounts, StringComparer.Ordinal));
            CanonicalDigest = digest;
        }

        public int RawMaskCount => Sv5PatternMaskLibrary.RawMaskCount;
        public int EligibleMaskCount { get; }
        public IReadOnlyList<Sv5PatternMaskCandidate> Candidates { get; }
        public IReadOnlyDictionary<string, int> RejectionCounts { get; }
        public string CanonicalDigest { get; }
    }

    public static class Sv5PatternMaskLibrary
    {
        public const int RawMaskCount = 65536;
        public const int AcceptedCandidateCount = 500;

        public static Sv5PatternMaskCandidateSet Build()
        {
            var eligible = new List<Sv5PatternMaskCandidate>();
            ...
            for (var value = 0; value < RawMaskCount; value++)
            {
                ...
                eligible.Add(new Sv5PatternMaskCandidate(mask, open, largest, components.Count, north, south, west,
            ...
            return new Sv5PatternMaskCandidateSet(eligible.Count, ranked, rejections, digest);
        }

        private static List<Sv5PatternMaskCandidate> SelectDiverse(IReadOnlyList<Sv5PatternMaskCandidate> eligible)
        {
            var selected = new List<Sv5PatternMaskCandidate>();
            var masks = new HashSet<ushort>();
            var mirrors = new Dictionary<string, int>(StringComparer.Ordinal);

            Func<Sv5PatternMaskCandidate, bool>[] routeCapabilities =
            {
        ...
        private static bool TryAdd(Sv5PatternMaskCandidate candidate, ICollection<Sv5PatternMaskCandidate> selected,
        ...
        private static string DiversityKey(Sv5PatternMaskCandidate candidate) => string.Join("|",
        ...
        private static string CanonicalLine(Sv5PatternMaskCandidate c) => string.Join("|",
        ...
        private static bool ColumnOpen(Sv5PatternMaskCandidate candidate, int x)
        {
        ...
        {
            public Bucket(string key, List<Sv5PatternMaskCandidate> values) { Key = key; Values = values; }
            public string Key { get; }
            public List<Sv5PatternMaskCandidate> Values { get; }
