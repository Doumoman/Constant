...
public class PlayerFSM : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        ResolveOverlaps(); // 지형과 겹쳐 있으면 먼저 밀어낸 뒤 감지를 시작한다
        ApplyPlatformDelta();
        CheckGround();
    ...
    // ── 겹침 해소 ──
    // 스폰 오차·코너 클리핑 등으로 콜라이더가 지형 안에 파고들면 레이캐스트가
    // 내부에서 시작돼 이동이 막힌 채 끼어버린다. 최소 이동 벡터로 밀어내 복구한다.
    // 움직이는 플랫폼(Rigidbody2D 부착)은 통과/탑승 로직과 충돌하지 않게 제외.
    ...
    private void ResolveOverlaps()
    {
        ...
        filter.SetLayerMask(_playerData.collisionLayer);

        for (int iter = 0; iter < 3; iter++)
        {
            Vector2 center = (Vector2)transform.position + Bc.offset;
            int count = Physics2D.OverlapBox(center, Bc.size, 0f, filter, _overlapResults);
            bool moved = false;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                // 정적 지형만 대상 — Composite 타일은 Static Rigidbody2D 를 달고 있으므로
                // "RB 존재" 가 아니라 bodyType 으로 움직이는 플랫폼만 제외한다
                Rigidbody2D rb = other.attachedRigidbody;
                if (rb != null && rb.bodyType != RigidbodyType2D.Static) continue;

                ColliderDistance2D d = other.Distance(Bc);
                if (!d.isValid || !d.isOverlapped) continue;

                transform.position += (Vector3)(d.pointA - d.pointB);
                moved = true;
            }
