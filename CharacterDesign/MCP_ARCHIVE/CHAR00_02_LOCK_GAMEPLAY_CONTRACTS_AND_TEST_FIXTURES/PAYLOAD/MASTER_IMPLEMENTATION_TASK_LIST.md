...
| Phase | Task 수 | 기준 상태 |
|---|---:|---|
| CHAR00 기준선·하네스 | 3 | 1 COMPLETE / 1 CURRENT / 1 LOCKED |
| CHAR01 핵심 이동 | 4 | LOCKED |
| CHAR02 이동 문법 검증 | 3 | LOCKED |
| CHAR03 MAP·방 전환 | 3 | LOCKED |
| CHAR04 상호작용·접촉 전투 | 4 | LOCKED |
| CHAR05 장비·생존·런 | 5 | LOCKED |
| CHAR06 생성 맵·최종 검증 | 4 | LOCKED |
| **합계** | **26** | **1 COMPLETE / 1 CURRENT / 24 LOCKED** |
...
- [>] `CHAR00_02_LOCK_GAMEPLAY_CONTRACTS_AND_TEST_FIXTURES` — CURRENT: 게임플레이 계약·소유권·고정 테스트룸을 확정한다.
- [ ] `CHAR00_03_CHAR00_BASELINE_EXIT_AUDIT` — 기준선과 하네스의 완전성·모순·미해결 의존성을 감사한다.

## CHAR01 — 핵심 이동

- [ ] `CHAR01_01_IMPLEMENT_INPUT_SNAPSHOT_AND_PLAYER_STATES` — 논리 입력 스냅샷·버퍼와 플레이어 상태를 구현한다.
- [ ] `CHAR01_02_IMPLEMENT_COLLISION_QUERIES_AND_GROUND_MOTOR` — 충돌 질의·지지체·걷기·달리기·가감속을 구현한다.
- [ ] `CHAR01_03_IMPLEMENT_JUMP_AIR_CONTROL_AND_LANDING` — 점프·가변 높이·코요테·공중 제어·착지를 구현한다.
- [ ] `CHAR01_04_CHAR01_CORE_MOVEMENT_EXIT_AUDIT` — 핵심 이동 구현과 회귀를 감사한다.

## CHAR02 — 이동 문법 검증

- [ ] `CHAR02_01_VALIDATE_TWO_CELL_HEIGHT_AND_GAP_RULES` — 2셀 높이와 동일 높이 2셀 틈 통과를 검증한다.
