...
```text
TASKS/MAP03_06_IMPLEMENT_RESERVATION_BACKTRACKING.md
```
...
| MAP03_06_IMPLEMENT_RESERVATION_BACKTRACKING | CURRENT |
...
현재는 MAP03의 여섯 번째 Task에서 Start, Boss, Forge, CoreResource 3개 option groups를 exact search order로 선택하고, collision·distance·three-Core cluster hard rejection 뒤 후보 고갈 시 직전 선택으로 backtrack한다. RNG_WORLD_SITE fresh stream은 equal-cost tie-break만 소유하며 failed combination 최대 200에서 retry-required를 반환한다. 실제 Core capacity flood, Village, final reservation snapshot/pass adapter는 수행하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP03_06_IMPLEMENT_RESERVATION_BACKTRACKING` 하나다.
...
MAP03_06이 PASS하면 STATUS FINALIZE는 MAP03_06만 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다. `MAP03_07_IMPLEMENT_CORE_CAPACITY_FLOOD_CHECK`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
