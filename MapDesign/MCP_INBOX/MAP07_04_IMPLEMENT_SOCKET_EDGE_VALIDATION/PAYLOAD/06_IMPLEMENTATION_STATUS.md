...
```text
TASKS/MAP07_04_IMPLEMENT_SOCKET_EDGE_VALIDATION.md
```
...
| MAP07_04_IMPLEMENT_SOCKET_EDGE_VALIDATION | CURRENT |
...
```text
MAP07_03_IMPLEMENT_MICROCHUNK_TRANSFORMS
```

## Last Result

```text
REPORTS/MAP07_03_IMPLEMENT_MICROCHUNK_TRANSFORMS_RESULT.md
STATUS: PASS
...
현재는 MAP07_04_IMPLEMENT_SOCKET_EDGE_VALIDATION 하나만 CURRENT다. MAP07_03은 STATUS `PASS`, SHA-256 `062206bf753f1dce3a9c6a43107e24090bf9abdc253fc9e69eec478a2fafa505`로 검수됐고 microchunk transform layer가 완료됐다.

이번 Task는 socket side, band, traversal kind, edge signature와 실제 outer tile opening/clearance를 대조하는 deterministic socket-edge validator까지만 수행한다. `EDGE_SOLID`은 socket row가 없어야 하며, L/R socket은 horizontal band, U/D socket은 vertical band와 일치해야 한다.

Object slot semantic validation, standalone 96-cell validator pass, reachability probe, authoring grid/window, CSV import/export, preview/report는 MAP07_05~MAP07_13의 소유다. MAP07_04에서 구현하거나 선행 생성하지 않는다.
...
MAP07_04가 PASS하면 MAP07_04만 COMPLETE, Current Task NONE으로 finalize한다. `MAP07_05_IMPLEMENT_OBJECT_SLOT_VALIDATION`은 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
