# 《별을 물어오는 밤 / 마펠렁키》 MAP09 이후 구현 Task Backlog v2.2 Compact

> 기준일: 2026-08-27
>
> 기준선: `MAP00_01 ~ MAP08_14 COMPLETE`, `MAP08 PHASE EXIT: APPROVED`
>
> 변경 목적: V2 구조 전환 Task 1개를 먼저 수행한 뒤, v2.0의 217개 신규 Task를 구조와 검증 안전성을 보존한 108개 구현 Task로 진행
...
```text
COMPLETE: MAP00_01 ~ MAP08_14
CURRENT : MAP09_00_CREATE_V2_MODULE_STRUCTURE
...
`MAP09_00`은 MAP09 기능 구현이 아니라 V2 기능 루트를 안전하게 추가하는 구조 전환 Task다. 기존 파일·폴더를 삭제·이동·이름 변경하지 않으며, 이 Task가 PASS/finalize되기 전에는 `MAP09_01`을 열지 않는다.
...
|---|---|---:|---|
| MAP09_00 | V2 additive 모듈 구조 전환 | 1 | CURRENT |
...
| **신규 합계** |  | **109** | **1 CURRENT / 108 LOCKED** |

기존 완료 Task 105개를 포함한 전체 재기준 총계는 `214 Task = 105 COMPLETE + 1 CURRENT + 108 LOCKED`다.
...
- [ ] `MAP09_00_CREATE_V2_MODULE_STRUCTURE` — 기존 36개 승인 루트와 후속 `Microchunks`·`Boundaries` 구조를 보존하고, Pipeline/MicroPatterns/TerrainClusters/Activities/EventOverlays/SpecialRegions/SectorPlanning/Baking/RuntimeState 기능 루트와 V2 Authoring/Generated 루트를 추가한다.
...
2. 각 Task는 별도 MCP patch와 Result를 가진다.
...
`MAP09_00`은 구조-only 전환 Task이며, `MAP09_01`은 새 solver 구현이 아니라 MAP08 승인 결과와 V2 pass 경계를 설치하는 기준선 Task다.
