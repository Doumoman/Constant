# MAP04_11 — MAP04 Batch and Exit Tests (Repair v1.5 Runtime Scene Adapter Split)

```yaml
status_control:
  task_key: MAP04_11_MAP04_BATCH_AND_EXIT_TESTS
  result_file: REPORTS/MAP04_11_MAP04_BATCH_AND_EXIT_TESTS_RESULT.md
  repair_contract: VARIABLE_COUNT_OVERLAY_AND_RUNTIME_SAFE_SCENE_ADAPTER
```
...
variable overlay source, Editor harness, Scenes folder와 scene asset은 생성됐다. 이 resume은 Unity가 Editor-assembly `MonoBehaviour`를 scene에 붙이지 못하는 충돌을 runtime-safe adapter + Editor custom inspector 분리로 해결하고 남은 tests/exit gate를 끝낸다.
...
```text
Current Task SHA:
bdf27df70d15d8040b0c9c36e538da39dc97d1f2dfc84d91201e35de2ba2f623

Current BLOCKED Result SHA:
409ec72e04acc17fd314c85f0a270ff0c23ab9898035e13983e598ba3cb9252c
...
Compiled Editor harness SHA: d2e49370967a1b6622de0c0b8e30a307755fa2d12a3cd91c6a6ba79e90e53c5b
Scene SHA/meta SHA: c1a093211bbbbd63db0a1d0a37dd45f064ae1ef063eb407ab6a06ec871b51ca3 / 57798017bf5c765d3c09f5414535ec03338ee05d761b3b4922d8485806b10f45
Scenes folder meta SHA: d39cc37491dc1dd85be0ea30fc07e6d4ca8d01ae3db93f8c01e2bc3fcd28c5e2
Existing overlay tests: 150/150 PASS
Assets meta: 3151
Scenes directory/scene: present/present
Scene root harness component: absent; Unity rejected Editor script attachment
```
...
```text
Assets/_Game/Map/Runtime/WorldGeneration/Diagnostics/MapGenerationProgressSceneAdapter.cs
Assets/_Game/Map/Runtime/WorldGeneration/Diagnostics/MapGenerationProgressSceneAdapter.cs.meta
...
current Result와 Phase C Status finalize만 추가 허용한다. harness/folder/scene matching metas는 보존한다. `Map04ExitTests`, repaired cleanup/producer files, other public models/API shape, asmdef, CSV, other Scene/Prefab, Packages, ProjectSettings는 수정하지 않는다.

## Runtime Adapter Split Contract

apply 후 execution 시작 시 아래 baseline을 exact 확인한다.

```text
Scenes folder/meta and MapGenerationProgressTest scene/meta present with frozen hashes/GUIDs
MapGenerationProgressSceneHarness.cs/meta present and compiling in Editor assembly
scene root has topology/site/biome overlays but no attachable fifth harness component
Assets meta 3151
```

신규 `MapGenerationProgressSceneAdapter`는 existing Runtime `Diagnostics` folder와 `Game.Map.Runtime` assembly에 둔다. UnityEditor/editor namespace, fixture construction, generator execution, file I/O를 참조하지 않는다.
...
adapter는 serialized seed text, attempt `0..99`, selected tab과 three overlay component references만 가진다. public/internal surface는 input read, status publish, `ShowTopology/Sites/Biomes`, `Clear`에 한정한다. `Awake/OnEnable/Update` generation, automatic retry, UnityEditor reference는 `0`이다.

existing Editor harness는 `MonoBehaviour`가 아니어야 한다. same file에서 `CustomEditor(typeof(MapGenerationProgressSceneAdapter))`와 private fixture/action runner를 제공한다. Inspector button만 public pipeline을 실행하고 adapter/overlay에 transient snapshot/status를 주입한다.

scene을 Unity Editor API로 열어 root에 runtime adapter를 exact once 추가하고 three overlay references를 연결한 뒤 같은 path에 저장한다. Editor harness type 부착 시도 `0`; scene/folder meta GUID는 변경하지 않는다. 신규 item은 adapter C# + matching meta exact `1/1`뿐이다.
...
BiomePatchOverlay
MapGenerationProgressSceneAdapter
```

`MapGenerationProgressSceneAdapter`는 attachable state/tab bridge다. 기존 `MapGenerationProgressSceneHarness.cs`는 Editor-only custom inspector와 private fixture builder/action runner다. 새 production root/pass/retry adapter가 아니다.
...
```text
Assets meta 3151 -> 3152
new Runtime adapter C#/meta = 1/1
modified existing Editor harness/scene = 1/1
folder/scene meta GUID and hashes unchanged
original pre-v1.3 baseline부터 cumulative new Runtime C#/Editor C#/scene/meta = 1/1/1/4
modified existing C# = overlay allowlist + Editor harness only
