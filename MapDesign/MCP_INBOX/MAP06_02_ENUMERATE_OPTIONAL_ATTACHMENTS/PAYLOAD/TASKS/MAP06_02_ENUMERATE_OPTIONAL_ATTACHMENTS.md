...
### 기존 boundary test 수정 — exact up to 6 C# files
...
신규 C# 7개와 matching `.cs.meta` 7개, 위 boundary test C# 최대 6개 수정, Result 1만 허용한다. existing production graph/CSV/SectorCell/Authoring CSV/asmdef/Scene/Prefab/Packages/ProjectSettings는 수정하지 않는다. 기존 approved directory를 재사용하고 folder meta를 만들지 않는다.
...
Actually executed required total >=2313 PASS
failed/skipped 0/0
