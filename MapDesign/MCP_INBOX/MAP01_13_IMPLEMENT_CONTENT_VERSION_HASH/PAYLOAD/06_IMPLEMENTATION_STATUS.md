...
```text
TASKS/MAP01_13_IMPLEMENT_CONTENT_VERSION_HASH.md
```
...
| MAP01_13_IMPLEMENT_CONTENT_VERSION_HASH | CURRENT |
...
현재는 immutable Registry와 matching parsed source/schema를 canonical semantic byte stream으로 변환하여 file/row order-independent SHA-256 `ContentVersionHash`를 계산한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_13_IMPLEMENT_CONTENT_VERSION_HASH` 하나다.

Global singleton install, atomic replacement/publish, import report/window, domain validation은 구현하지 않는다. 기존 CSV·loader·definitions·FK/Registry·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_13이 PASS해도 `MAP01_14_IMPLEMENT_ATOMIC_PUBLISH_AND_IMPORT_REPORT`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_13을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
