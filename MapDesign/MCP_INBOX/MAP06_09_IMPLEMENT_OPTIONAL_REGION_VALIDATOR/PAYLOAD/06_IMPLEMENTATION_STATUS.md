...
```text
TASKS/MAP06_09_IMPLEMENT_OPTIONAL_REGION_VALIDATOR.md
```
...
| MAP06_09_IMPLEMENT_OPTIONAL_REGION_VALIDATOR | CURRENT |
...
```text
MAP06_07_IMPLEMENT_RETURN_POLICY
```

## Last Result

```text
REPORTS/MAP06_07_IMPLEMENT_RETURN_POLICY_RESULT.md
STATUS: PASS
...
현재는 MAP06_09_IMPLEMENT_OPTIONAL_REGION_VALIDATOR 하나만 CURRENT다. MAP06_08은 PASS 및 COMPLETE 확정 기준으로만 상태 전환됐다. MAP06_10 이후 Task body는 읽거나 생성하거나 실행하지 않는다.

이번 Task는 MAP06_01~08 optional source-chain을 결합해 optional region snapshot, Type0 route masks, access/clue, reward tier, return policy, inactive buffer accounting을 하나의 immutable validation report로 검증하는 것까지만 수행한다.

검증 항목은 mandatory graph identity, Type0 `!(L&&R)`, returnability, visible clue presence, mandatory reward 금지, inactive full accounting, approved reserved-adapter overlap, digest/source identity, zero RNG/source mutation이다. boundary profile/recipe/microchunk/tile/socket/edge/overlay/generated CSV는 만들지 않는다. MAP06_09 symbols만 허용하고 MAP06_10+ future symbols는 계속 금지한다.
...
MAP06_09가 PASS하면 MAP06_09만 COMPLETE, Current Task NONE으로 finalize한다. `MAP06_10_MAP06_OVERLAY_AND_EXIT_TESTS`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
