...
```yaml
patch_id: MAP01_03_IMPLEMENT_RFC4180_READER
patch_version: 1.0
requires_status:
  current_task: NONE
  task_states:
...
requires_result:
  path: MapDesign/MCP/REPORTS/MAP01_02_IMPLEMENT_CSV_SCHEMA_CATALOG_RESULT.md
  exact_status: PASS
requires_project_baseline:
  schema_catalog_files: 60
  schema_catalog_columns: 679
  schema_tests: 30/30
  architecture_tests: 10/10
  targeted_editmode: 40/40
  compile_errors: 0
...
forbids_started_task_prefixes:
  ...
  - MAP14
  - MAP15
sets_current_task: TASKS/MAP01_03_IMPLEMENT_RFC4180_READER.md
copy_operations:
  ...
  - source: PAYLOAD/06_IMPLEMENTATION_STATUS.md
    ...
    mode: replace
  - source: PAYLOAD/TASKS/MAP01_03_IMPLEMENT_RFC4180_READER.md
    destination: MapDesign/MCP/TASKS/MAP01_03_IMPLEMENT_RFC4180_READER.md
    mode: create
...
- 205개 상태 행에서 MAP00_01~10과 MAP01_01~02는 `COMPLETE`, MAP01_03 이후는 `LOCKED`다.
- MAP01_02 Result는 exact `STATUS: PASS`이고 schema catalog `60/679`, schema tests `30/30`, architecture `10/10`, targeted `40/40`, compile error 0이다.
- 기존 Authoring CSV/meta는 `50/50`이며 MAP01_02에서 변경되지 않았다.
- 새 Master는 205개 순서를 유지하고 MAP01_02 COMPLETE, MAP01_03 exact next를 기록한다.
- 새 status는 `12 COMPLETE / MAP01_03 CURRENT / 192 LOCKED`다.
- Current Task와 Task 파일이 정확히 일치하고 Task 파일은 비어 있지 않다.
- PATCH APPLY 단계에서는 `Assets/**`, CSV, C#, test, asmdef를 변경하지 않는다.

Task destination이 이미 존재하면 payload와 바이트 단위로 동일할 때만 재사용한다. 다르면 덮어쓰기·병합하지 않고 `BLOCKED`다.
