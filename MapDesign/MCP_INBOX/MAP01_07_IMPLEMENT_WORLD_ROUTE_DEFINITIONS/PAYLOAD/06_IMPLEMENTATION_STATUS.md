...
```text
TASKS/MAP01_07_IMPLEMENT_WORLD_ROUTE_DEFINITIONS.md
```
...
```text
MAP01_06_IMPLEMENT_SCALAR_AND_LIST_PARSERS
```
...
현재는 successful typed parse 결과에서 world/generation/RNG, route mask, socket/edge signature, sector recipe 계열 13개 CSV를 immutable typed definition set으로 변환한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_07_IMPLEMENT_WORLD_ROUTE_DEFINITIONS` 하나다.

FK resolution, biome/boundary/special/village/microchunk/population/item definition, StaticDataRegistry는 구현하지 않는다. 기존 CSV·reader·validator·PK·parser·schema catalog/importer·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_07이 PASS해도 `MAP01_08_IMPLEMENT_BIOME_BOUNDARY_DEFINITIONS`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_07을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
