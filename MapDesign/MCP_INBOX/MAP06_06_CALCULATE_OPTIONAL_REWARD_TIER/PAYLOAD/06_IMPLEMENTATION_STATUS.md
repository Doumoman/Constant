...
```text
TASKS/MAP06_06_CALCULATE_OPTIONAL_REWARD_TIER.md
```
...
| MAP06_06_CALCULATE_OPTIONAL_REWARD_TIER | CURRENT |
...
```text
MAP06_05_ASSIGN_ACCESS_RULES_AND_CLUES
```

## Last Result

```text
REPORTS/MAP06_05_ASSIGN_ACCESS_RULES_AND_CLUES_RESULT.md
STATUS: PASS
...
현재는 MAP06_06_CALCULATE_OPTIONAL_REWARD_TIER 하나만 CURRENT다. MAP06_05는 PASS 및 COMPLETE 확정 기준으로만 상태 전환됐다. MAP06_07 이후 Task body는 읽거나 생성하거나 실행하지 않는다.

이번 Task는 MAP06_04 Type0 snapshot과 MAP06_05 access/clue assignment의 digest chain을 결합해 각 optional region의 depth/tool/fuel/clue score component와 existing OptionalRewardTier Low/Medium/High/Unique reservation을 immutable하게 계산하는 것까지만 수행한다. approved formula는 `depth*2 + tool tier + fuel/10 + hidden difficulty`, tier minimum은 `0/4/8/12`다.

actual reward ID/item/pool/quantity/spawn, mandatory/core/unique reward, return policy/device, inactive buffer, validator, overlay, generated CSV는 만들지 않는다. MAP06_06에서 새 `OptionalRewardTier*` production symbol이 생기므로 phase-boundary negative assertions는 MAP06_06 symbols를 허용하고 MAP06_07+ future symbols만 금지하도록 필요한 기존 boundary test 파일만 교정할 수 있다.

Mandatory route graph 기준은 고정이다: graph nodes/directed/undirected/route cells `47/96/48/47`, mask counts `20/4/4/17/0/0/2`, validation `12/12/0/0/0`. MAP05 Type4 규칙은 계속 보존한다: U+D mandatory, L/R actual adjacency preserved, UD/LUD/RUD/LRUD all legal. MAP06_04 attachment base-closed `12`와 MAP06_05 access/clue/cost/preview identity도 변경하지 않는다.

MAP06_06이 PASS하면 MAP06_06만 COMPLETE, Current Task NONE으로 finalize한다. `MAP06_07_IMPLEMENT_RETURN_POLICY`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
