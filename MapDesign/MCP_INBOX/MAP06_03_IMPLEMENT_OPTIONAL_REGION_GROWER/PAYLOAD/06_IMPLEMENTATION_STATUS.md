...
```text
TASKS/MAP06_03_IMPLEMENT_OPTIONAL_REGION_GROWER.md
```
...
```text
MAP06_02_ENUMERATE_OPTIONAL_ATTACHMENTS
```

## Last Result

```text
REPORTS/MAP06_02_ENUMERATE_OPTIONAL_ATTACHMENTS_RESULT.md
STATUS: PASS
...
현재는 MAP06_03_IMPLEMENT_OPTIONAL_REGION_GROWER 하나만 CURRENT다. MAP06_02는 PASS 및 COMPLETE 확정 기준으로만 상태 전환됐다. MAP06_04 이후 Task body는 읽거나 생성하거나 실행하지 않는다.

이번 Task는 MAP06_02 optional attachment candidate를 입력으로 사용해 단일 mandatory attachment를 가진 깊이 1~4의 connected non-through optional region 군집을 deterministic하게 성장시키는 것까지만 수행한다. Type0 route mask assignment, access/clue assignment, reward tier calculation algorithm, return device placement, inactive buffer assignment, validator, overlay, generated CSV writer는 구현하지 않는다.

MAP06_03에서 새 `OptionalRegionGrowth*`/`OptionalRegionGrower` production symbol이 생기므로, phase-boundary negative assertions는 MAP06_03 symbol을 허용하고 MAP06_04+ future symbols만 금지하도록 필요한 기존 boundary test 파일만 교정할 수 있다. 그 외 MAP05 production graph/CSV/SectorCell, MAP06_01/02 production source, Authoring CSV/meta와 asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다.
...
MAP06_03이 PASS하면 MAP06_03만 COMPLETE, Current Task NONE으로 finalize한다. `MAP06_04_ASSIGN_TYPE0_ROUTE_MASKS`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
