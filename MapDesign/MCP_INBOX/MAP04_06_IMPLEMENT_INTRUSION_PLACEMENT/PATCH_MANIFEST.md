...
```yaml
patch_id: MAP04_06_IMPLEMENT_INTRUSION_PLACEMENT
patch_version: 1.0
requires_status:
  current_task: NONE
  task_states:
...
requires_result:
  path: MapDesign/MCP/REPORTS/MAP04_05_IMPLEMENT_MULTI_SEED_BIOME_GROWER_RESULT.md
  exact_status: PASS
  sha256: ab23a2d0e30cb21df7fca6f098607cf20ccd5a3cc9a9da4f43f8fdb344ba6e2f
...
forbids_started_task_prefixes:
  ...
  - MAP14
  - MAP15
sets_current_task: TASKS/MAP04_06_IMPLEMENT_INTRUSION_PLACEMENT.md
copy_operations:
  ...
  - source: PAYLOAD/06_IMPLEMENTATION_STATUS.md
    ...
    mode: replace
  - source: PAYLOAD/TASKS/MAP04_06_IMPLEMENT_INTRUSION_PLACEMENT.md
    destination: MapDesign/MCP/TASKS/MAP04_06_IMPLEMENT_INTRUSION_PLACEMENT.md
    mode: create
...
- 205개 상태 행에서 MAP00_01~10, MAP01_01~17, MAP02_01~08, MAP03_01~11, MAP04_01~05는 `COMPLETE`, MAP04_06 이후는 `LOCKED`다.
- MAP04_05 Result는 exact `STATUS: PASS`이고 SHA-256은 `ab23a2d0e30cb21df7fca6f098607cf20ccd5a3cc9a9da4f43f8fdb344ba6e2f`다.
- MAP04_05 focused `164/164`, required regressions `478/478`, actually executed `642/642`, compile/Console/relevant warning `0/0/0`을 통과했다.
- targeted/full은 사용자 지정 감축 profile에 따라 executed PASS가 아니라 discovery/arithmetic `4509/4577`로 분리 기록됐다.
- MAP04_05 attempt-0은 capacity `165/161/4`, RNG `13->13` retry-required를 정확히 냈다.
- viable attempt `24`는 `14 patches / 165 assigned / 4 reserved-unassigned / RNG 17->1907`, overlap/disconnected/source mutation `0/0/0`이다.
- MAP04_05는 exact 8 Runtime + 1 test + 9 matching meta만 만들었고 existing/unexpected Assets modification은 `0/0`이다.
- current Assets meta baseline은 `3110`, Authoring CSV/meta `50/50`, accepted legacy Editor folder meta `6/6`, duplicate GUID group은 `0`이다.
- 새 Master는 MAP04_05 COMPLETE와 MAP04_06 exact next를 기록한다.
- 새 status는 `51 COMPLETE / MAP04_06 CURRENT / 153 LOCKED`다.
