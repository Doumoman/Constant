...
```text
TASKS/MAP01_01_INSTALL_CSV_AUTHORING_BASELINE.md
```
...
```text
REPORTS/MAP00_10_MAP00_EXIT_AUDIT_RESULT.md
STATUS: PASS
MAP00 EXIT: APPROVED
```
...
현재는 정본 CSV 입력을 프로젝트의 확정된 WorldGeneration Authoring 폴더에 바이트 그대로 설치하고 검증한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 상태만 CURRENT로 바뀌고 Task/입력 파일이 누락된 half-applied 상태에서는 exact 205-row CURRENT 계약을 사용하는 v2.3 recovery package만 사용한다.

프로덕션/Editor/Test C#, asmdef, ScriptableObject, Scene, Prefab, Package, ProjectSettings는 생성하거나 수정하지 않는다.

MAP01_01이 PASS해도 `MAP01_02_IMPLEMENT_CSV_SCHEMA_CATALOG`를 자동 구현하지 않는다. STATUS FINALIZE는 MAP01_01을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
