...
```yaml
patch_id: MAP01_01_INSTALL_CSV_AUTHORING_BASELINE
patch_version: 2.3
requires_status:
  current_task: TASKS/MAP01_01_INSTALL_CSV_AUTHORING_BASELINE.md
  task_states:
    ...
    MAP01_02_IMPLEMENT_CSV_SCHEMA_CATALOG: LOCKED
    MAP01_03_IMPLEMENT_RFC4180_READER: LOCKED
    MAP01_04_IMPLEMENT_HEADER_AND_FIELD_VALIDATION: LOCKED
    MAP01_05_IMPLEMENT_PRIMARY_KEY_INDEX: LOCKED
    MAP01_06_IMPLEMENT_SCALAR_AND_LIST_PARSERS: LOCKED
    MAP01_07_IMPLEMENT_WORLD_ROUTE_DEFINITIONS: LOCKED
    MAP01_08_IMPLEMENT_BIOME_BOUNDARY_DEFINITIONS: LOCKED
    MAP01_09_IMPLEMENT_SPECIAL_VILLAGE_DEFINITIONS: LOCKED
    MAP01_10_IMPLEMENT_MICROCHUNK_POPULATION_ITEM_DEFINITIONS: LOCKED
    MAP01_11_IMPLEMENT_FOREIGN_KEY_RESOLVER: LOCKED
    MAP01_12_IMPLEMENT_STATIC_DATA_REGISTRY: LOCKED
    MAP01_13_IMPLEMENT_CONTENT_VERSION_HASH: LOCKED
    MAP01_14_IMPLEMENT_ATOMIC_PUBLISH_AND_IMPORT_REPORT: LOCKED
    MAP01_15_CREATE_CSV_IMPORT_WINDOW: LOCKED
    MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS: LOCKED
    MAP01_17_MAP01_EXIT_AUDIT: LOCKED
    MAP02_01_IMPLEMENT_GENERATED_WORLD_DATA: LOCKED
    MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS: LOCKED
    MAP02_03_IMPLEMENT_GRID_INITIALIZATION_PASS: LOCKED
    MAP02_04_IMPLEMENT_WORLD_GENERATION_ROOT: LOCKED
    MAP02_05_IMPLEMENT_PASS_EXECUTION_RECORDS: LOCKED
...
recovery_contract:
  state: HALF_APPLIED_STATUS_ONLY
  task_destination: ABSENT_OR_BYTE_IDENTICAL_TO_SOURCE
  input_destination: ABSENT_OR_TREE_BYTE_IDENTICAL_TO_SOURCE
  applied_marker: ABSENT
  authoring_destination_csv_count: 0
requires_result:
  path: MapDesign/MCP/REPORTS/MAP00_10_MAP00_EXIT_AUDIT_RESULT.md
  exact_status: PASS
forbids_started_task_prefixes:
  ...
  - MAP14
  - MAP15
sets_current_task: TASKS/MAP01_01_INSTALL_CSV_AUTHORING_BASELINE.md
copy_operations:
  ...
  - source: PAYLOAD/06_IMPLEMENTATION_STATUS.md
    ...
    mode: replace
  - source: PAYLOAD/TASKS/MAP01_01_INSTALL_CSV_AUTHORING_BASELINE.md
    destination: MapDesign/MCP/TASKS/MAP01_01_INSTALL_CSV_AUTHORING_BASELINE.md
    mode: create_or_reuse_if_byte_identical
  - source: PAYLOAD/INPUTS/MAP01_01_CSV_PACKAGE/
    destination: MapDesign/MCP/INPUTS/MAP01_01_CSV_PACKAGE/
    mode: create_directory_tree_or_reuse_if_byte_identical
forbidden_operations:
  - delete
  - overwrite_existing_input_tree
  - overwrite_different_task_file
  - manual_status_rollback
  ...
  - modify_csv_contents_during_patch_apply
  - modify_asmdef
  - run_superseded_map01_package
  - git_commit
  - git_push
```
...
- 기존 Master가 205개 Task, MAP00_01~10 COMPLETE, MAP01_01 exact next를 기록하는지 확인한다.
- `MAP00_10_MAP00_EXIT_AUDIT_RESULT.md`에 `STATUS: PASS`와 `MAP00 EXIT: APPROVED`가 있는지 확인한다.
- 이 recovery 패치는 적용 전 status가 205개 Task를 모두 개별 행으로 기록하고, MAP00_01~10은 `COMPLETE`, MAP01_01은 `CURRENT`, MAP01_02 이후는 모두 `LOCKED`, Current Task는 `TASKS/MAP01_01_INSTALL_CSV_AUTHORING_BASELINE.md`인지 확인한다.
- 실제 Task destination과 입력 destination은 없거나 이 payload와 바이트 단위로 완전히 같아야 한다. `.APPLIED`는 없어야 하고 Assets Authoring CSV는 0개여야 한다.
- 이 패치의 Master 사본은 MAP00 10/10 COMPLETE와 MAP01_01 recovery/resume 상태를 반영한 승인 갱신본이므로 `replace`한다.
- `06_IMPLEMENTATION_STATUS.md`의 Current Task가 `TASKS/MAP01_01_INSTALL_CSV_AUTHORING_BASELINE.md`다.
- 상태 표에서 `MAP01_01_INSTALL_CSV_AUTHORING_BASELINE`이 `CURRENT`다.
- 적용 후에도 상태 표는 205개 개별 Task 행을 유지하고 exact `MAP01_01 = CURRENT`, MAP01_02 이후 `LOCKED`여야 한다.
- Task 파일이 존재하고 비어 있지 않다.
- 입력 트리에 파일이 정확히 64개 있다.
- 입력 트리에 Authoring CSV 49개, Generated schema CSV 11개, 데이터 사전 1개, 검증기 1개, 매핑 1개, 검증 기준 1개가 있다.
- 입력 root 기준 relative path로 정규화한 sorted `sha256sum` manifest의 SHA-256이 `2b0d40ea2d67173168b452b722bc6af91268c28636abda6ae3a6a63457e7109e`다.
- validator 결과가 exit 0, `ERROR 0`, `WARNING 10`이고 baseline과 일치한다.
- PATCH APPLY 단계에서는 `Assets/**`가 변경되지 않는다.

입력 트리가 이미 존재하면 파일 단위 병합이나 덮어쓰기를 하지 않는다. 전체 트리가 이 패치와 바이트 단위로 동일할 때만 재사용하고, 하나라도 다르면 `BLOCKED`로 종료한다.
