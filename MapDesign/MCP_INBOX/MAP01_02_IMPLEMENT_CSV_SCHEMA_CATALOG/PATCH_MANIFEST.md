...
```yaml
patch_id: MAP01_02_IMPLEMENT_CSV_SCHEMA_CATALOG
patch_version: 1.0
requires_status:
  current_task: NONE
  task_states:
...
requires_result:
  path: MapDesign/MCP/REPORTS/MAP01_01_INSTALL_CSV_AUTHORING_BASELINE_RESULT.md
  exact_status: PASS
requires_project_baseline:
  ...
  dictionary_csv: 1
  csv_meta_total: 50
  dictionary_path: Assets/_Game/Map/Data/WorldGeneration/Authoring/CSV_DATA_DICTIONARY.csv
  source_destination_hash_match: 50/50
  utf8_bom_present: 50/50
  compile_errors: 0
...
forbids_started_task_prefixes:
  ...
  - MAP14
  - MAP15
sets_current_task: TASKS/MAP01_02_IMPLEMENT_CSV_SCHEMA_CATALOG.md
copy_operations:
  ...
  - source: PAYLOAD/06_IMPLEMENTATION_STATUS.md
    ...
    mode: replace
  - source: PAYLOAD/TASKS/MAP01_02_IMPLEMENT_CSV_SCHEMA_CATALOG.md
    destination: MapDesign/MCP/TASKS/MAP01_02_IMPLEMENT_CSV_SCHEMA_CATALOG.md
    mode: create
...
- 적용 전 Current Task는 정확히 `NONE`이다.
- 상태 표는 Master와 같은 205개 개별 Task 행을 가지며 MAP00_01~10과 MAP01_01은 `COMPLETE`, MAP01_02 이후는 `LOCKED`다.
- `MAP01_01_INSTALL_CSV_AUTHORING_BASELINE_RESULT.md`는 exact `STATUS: PASS`다.
- 설치된 Authoring CSV는 static 49개와 dictionary 1개, 합계 50개이며 대응 `.csv.meta`도 50개다.
- MAP01_01 Result의 hash/BOM은 `50/50`, mismatch/missing은 0이다.
- 새 Master는 205개 순서를 유지하고 MAP01_01 COMPLETE, MAP01_02 exact next를 기록한다.
- 새 status는 205개 행을 유지하고 `MAP01_02_IMPLEMENT_CSV_SCHEMA_CATALOG = CURRENT`, MAP01_03 이후 `LOCKED`다.
- Current Task는 `TASKS/MAP01_02_IMPLEMENT_CSV_SCHEMA_CATALOG.md`다.
- Task 파일이 존재하고 비어 있지 않다.
- PATCH APPLY 단계에서는 `Assets/**`, CSV, C#, test, asmdef를 변경하지 않는다.

Task destination이 이미 존재하면 payload와 바이트 단위로 동일할 때만 재사용한다. 하나라도 다르면 덮어쓰기·병합하지 않고 `BLOCKED`로 종료한다.
