...
```text
TASKS/MAP04_08_EXPORT_BIOME_PATCH_RESULTS.md
```
...
| MAP04_08_EXPORT_BIOME_PATCH_RESULTS | CURRENT |
...
```text
MAP04_07_IMPLEMENT_PATCH_CLEANUP
```

## Last Result

```text
REPORTS/MAP04_07_IMPLEMENT_PATCH_CLEANUP_RESULT.md
STATUS: PASS
```
...
현재는 MAP04_07의 immutable cleanup publication과 같은 seed의 `GeneratedWorldData`를 결합해 biome/patch가 반영된 새 world snapshot, exact `generated_biome_patches.csv`, exact `generated_world_sectors.csv` bytes를 deterministic publish한다. source는 mutate하지 않고 filesystem에는 쓰지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP04_08_EXPORT_BIOME_PATCH_RESULTS` 하나다.

실제 Authoring/generated CSV와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. approved Runtime/Test `Generation` 폴더에 export production C# 6개, focused test C# 1개와 matching meta 7개만 추가한다. MAP04_01~07 existing files는 수정하지 않는다. final validator, overlay, root/file writer는 만들지 않는다.

MAP04_08이 PASS하면 MAP04_08만 COMPLETE, Current Task NONE으로 finalize한다. `MAP04_09_IMPLEMENT_BIOME_PATCH_VALIDATOR`는 LOCKED로 유지한다.
