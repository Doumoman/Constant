...
```text
TASKS/MAP07_12_CREATE_MICROCHUNK_PREVIEW_AND_REPORT.md
```
...
| MAP07_12_CREATE_MICROCHUNK_PREVIEW_AND_REPORT | CURRENT |
...
```text
REPORTS/MAP07_11_IMPLEMENT_MICROCHUNK_CSV_EXPORT_RESULT.md
STATUS: PASS
...
현재는 MAP07_12_CREATE_MICROCHUNK_PREVIEW_AND_REPORT 하나만 CURRENT다. MAP07_11은 STATUS `PASS`, SHA-256 `340cbed5424208ebeef144028c1806ea6a9039e8a6c14a5f39a824b042b062c6`로 검수됐고 Editor-only selected microchunk CSV exporter가 완료됐다.

이번 Task는 selected microchunk ID의 transform preview, validation report, and reachability heatmap을 만드는 Editor-only diagnostics layer까지만 수행한다. 기존 MAP07 runtime model/rules/validators/probe/transformer와 MAP07_08~11 editor states는 read-only input gate로 보존한다.

Starter catalog full round-trip, MAP07 phase exit, sector assembly, world-level traversal, generated CSV writer는 MAP07_13 이후 소유다. MAP07_12에서 구현하거나 선행 생성하지 않는다.

Mandatory route graph, MAP06 optional-region source chain, and MAP07_01~MAP07_11 production artifacts are read-only baseline이다. Authoring CSV/meta `50/50`, Authoring manifest `4ffef6dbbea5151889d1c9114a500eba6cb54828ba47c9de508bad95dddc4ac3`, generated CSV `0`, Scene/Prefab/ProjectSettings/asmdef changes `0`을 유지한다.

MAP07_12가 PASS하면 MAP07_12만 COMPLETE, Current Task NONE으로 finalize한다. `MAP07_13_MAP07_STARTER_AND_EXIT_TESTS`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
