...
```text
TASKS/MAP01_06_IMPLEMENT_SCALAR_AND_LIST_PARSERS.md
```
...
| MAP01_06_IMPLEMENT_SCALAR_AND_LIST_PARSERS | CURRENT |
...
현재는 successful header/field validation과 PK index gate 뒤 `EffectiveValue`를 invariant scalar 또는 pipe-list typed value로 파싱하고 모든 오류를 exact source field 위치로 보고한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_06_IMPLEMENT_SCALAR_AND_LIST_PARSERS` 하나다.

FK resolution, domain definition, StaticDataRegistry는 구현하지 않는다. 기존 CSV·reader·header/field validator·PK index·schema catalog/importer·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_06이 PASS해도 `MAP01_07_IMPLEMENT_WORLD_ROUTE_DEFINITIONS`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_06을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
