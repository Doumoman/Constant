...
```text
TASKS/MAP05_11_MAP05_BATCH_AND_EXIT_TESTS.md
```
...
```text
MAP05_10_CREATE_MANDATORY_ROUTE_OVERLAY
```

## Last Result

```text
REPORTS/MAP05_10_CREATE_MANDATORY_ROUTE_OVERLAY_RESULT.md
STATUS: PASS
```
...
현재는 MAP05_01~10 mandatory route phase 전체를 batch/determinism/visual/exit audit로 검증한다. Type4의 네 수평 조합(UD/LUD/RUD/LRUD)은 모두 합법이며 U+D는 필수, L/R은 실제 graph adjacency 보존이다. Graph/CSV/SectorCell은 수정하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP05_11_MAP05_BATCH_AND_EXIT_TESTS` 하나다.

실제 production graph/CSV/SectorCell/Authoring CSV/meta와 asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. approved Runtime EditMode `Generation`에 `Map05ExitTests.cs` 1개와 matching meta 1개만 추가한다.

MAP05_11이 PASS하면 MAP05 EXIT을 APPROVED하고 MAP05_11만 COMPLETE, Current Task NONE으로 finalize한다. `MAP06_01_IMPLEMENT_OPTIONAL_REGION_MODELS`는 LOCKED로 유지한다.
