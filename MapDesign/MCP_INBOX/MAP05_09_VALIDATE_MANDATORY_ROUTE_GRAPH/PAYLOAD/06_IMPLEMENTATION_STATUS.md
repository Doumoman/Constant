...
```text
TASKS/MAP05_09_VALIDATE_MANDATORY_ROUTE_GRAPH.md
```
...
| MAP05_09_VALIDATE_MANDATORY_ROUTE_GRAPH | CURRENT |
...
```text
REPORTS/MAP05_08_BUILD_MANDATORY_ROUTE_GRAPH_RESULT.md
STATUS: PASS
```
...
- MAP05_08 tests: MandatoryRouteGraphBuilder `281/281`, required aggregate `1991/1991`, actually executed `1991/1991`, failed/skipped `0/0`
- MAP05_08 output: graph nodes/directed edges/undirected edges/route cells `47/96/48/47`, terminals reachable `7/7`, accepted loops represented `2/2`
- MAP05_08 final Assets meta `3229`, Authoring CSV/meta `50/50`, duplicate GUID `0`, task-marker changes `35`, existing production/unexpected `0/0`, compile/Console/warning `0/0/0`
- MAP04 EXIT: `APPROVED`; MAP05 entry: `ELIGIBLE FOR SEPARATE PATCH`

## Current Rule

현재는 MAP05_08의 mandatory route graph와 route-stamped GeneratedWorldData, generated edge CSV bytes를 검증한다. Type4의 네 수평 조합(UD/LUD/RUD/LRUD)은 모두 허용하며 U+D는 필수, L/R은 실제 graph adjacency 보존이다. Graph/CSV/SectorCell은 수정하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP05_09_VALIDATE_MANDATORY_ROUTE_GRAPH` 하나다.

실제 production graph/CSV/SectorCell/Authoring CSV/meta와 asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. approved Runtime/Test `Generation`에 validator production C# 8개, focused test 1개와 matching meta 9개를 추가하고, prior negative-audit test 8개만 제한적으로 수정한다.

MAP05_09가 PASS하면 MAP05_09만 COMPLETE, Current Task NONE으로 finalize한다. `MAP05_10_CREATE_MANDATORY_ROUTE_OVERLAY`는 LOCKED로 유지한다.
