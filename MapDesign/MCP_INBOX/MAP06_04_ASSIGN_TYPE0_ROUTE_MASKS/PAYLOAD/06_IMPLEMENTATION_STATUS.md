...
```text
TASKS/MAP06_04_ASSIGN_TYPE0_ROUTE_MASKS.md
```
...
| MAP06_04_ASSIGN_TYPE0_ROUTE_MASKS | CURRENT |
...
```text
MAP06_03_IMPLEMENT_OPTIONAL_REGION_GROWER
```

## Last Result

```text
REPORTS/MAP06_03_IMPLEMENT_OPTIONAL_REGION_GROWER_RESULT.md
STATUS: PASS
...
현재는 MAP06_04_ASSIGN_TYPE0_ROUTE_MASKS 하나만 CURRENT다. MAP06_03은 PASS 및 COMPLETE 확정 기준으로만 상태 전환됐다. MAP06_05 이후 Task body는 읽거나 생성하거나 실행하지 않는다.

이번 Task는 MAP06_03 optional-region topology와 MAP01 typed route definitions를 입력으로 사용해 각 optional cell의 same-region adjacency와 유일한 mandatory bridge에 exact match하는 registered Type0 mask ID를 배정하는 것까지만 수행한다. exact 12개 `ROUTE_T0_*` row만 허용하고 임의 bool 조합, extra open side, L/R simultaneous Type0를 금지한다.

MAP06_04에서 새 `Type0Route*` production symbol이 생기므로, phase-boundary negative assertions는 MAP06_04 symbol을 허용하고 MAP06_05+ future symbols만 금지하도록 필요한 기존 boundary test 파일만 교정할 수 있다. 그 외 MAP05/MAP06_01~03 production graph/mask/model/grower, `OptionalRegionCell`, `SectorCell`, Authoring/generated CSV/meta와 asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다.
...
MAP06_04가 PASS하면 MAP06_04만 COMPLETE, Current Task NONE으로 finalize한다. `MAP06_05_ASSIGN_ACCESS_RULES_AND_CLUES`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
