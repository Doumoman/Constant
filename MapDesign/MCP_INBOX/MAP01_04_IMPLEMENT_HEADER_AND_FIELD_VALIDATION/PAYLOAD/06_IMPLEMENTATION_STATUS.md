...
```text
TASKS/MAP01_04_IMPLEMENT_HEADER_AND_FIELD_VALIDATION.md
```
...
| MAP01_04_IMPLEMENT_HEADER_AND_FIELD_VALIDATION | CURRENT |
...
현재는 RFC4180 reader 결과를 schema catalog와 대조해 header missing/unexpected/duplicate/order, field count, required/default 규칙만 exact source location으로 검증한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_04_IMPLEMENT_HEADER_AND_FIELD_VALIDATION` 하나다.

typed scalar/list parsing, PK index, FK resolution, domain definition, StaticDataRegistry는 구현하지 않는다. 기존 CSV·reader·schema catalog/importer·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_04가 PASS해도 `MAP01_05_IMPLEMENT_PRIMARY_KEY_INDEX`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_04를 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
