...
```text
TASKS/MAP03_08_IMPLEMENT_VILLAGE_RESERVATION.md
```
...
| MAP03_08_IMPLEMENT_VILLAGE_RESERVATION | CURRENT |
...
현재는 MAP03의 여덟 번째 Task에서 MAP03_07의 approved six-site plan과 four disjoint Core capacity witnesses를 보존하면서 primary Village 하나를 예약한다. exact Start 거리 `2-3:20 / 4-6:50 / 7-10:30` bucket, active allowed layout weight, `1x1 / 2x1 / 1x2` sector footprint, existing footprint/entry/Core-witness 충돌을 deterministic continued `RNG_WORLD_SITE`로 처리한다. bucket 고갈 시 fallback/local repair 없이 whole `PASS_SITE` retry-required로 거부한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP03_08_IMPLEMENT_VILLAGE_RESERVATION` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime/Test `Generation` 폴더에 신규 production C# 8개, focused test C# 1개와 matching meta 9개만 추가한다. Village 내부 MicroChunk/layout cell/facility와 final reservation publication은 만들지 않는다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP03_08이 PASS하면 STATUS FINALIZE는 MAP03_08만 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다. `MAP03_09_IMPLEMENT_SITE_RESERVATION_VALIDATOR`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
