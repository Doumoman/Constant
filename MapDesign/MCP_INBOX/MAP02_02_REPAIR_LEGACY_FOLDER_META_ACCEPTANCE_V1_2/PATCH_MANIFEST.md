...
```yaml
patch_id: MAP02_02_REPAIR_LEGACY_FOLDER_META_ACCEPTANCE_V1_2
patch_version: 1.2
requires_status:
  current_task: TASKS/MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS.md
  task_states:
    ...
    MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS: CURRENT
...
requires_result:
  path: MapDesign/MCP/REPORTS/MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS_RESULT.md
...
requires_project_baseline:
  ...
  biome_boundary_definition_tests: 36/36
  special_village_definition_tests: 48/48
  ...
  targeted_editmode: 1026/1026
  full_editmode: 1046/1046
  compile_errors: 0
  relevant_new_warnings: 0
  authoring_csv_total: 50
  csv_meta_total: 50
  legacy_editor_folder_meta_total: 6
  final_assets_meta_total: 2954
forbids_started_task_prefixes:
  ...
  - MAP14
  - MAP15
sets_current_task: TASKS/MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS.md
supplements_current_task: TASKS/MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS_REMEDIATION_V1_2.md
copy_operations:
  - source: PAYLOAD/TASKS/MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS_REMEDIATION_V1_2.md
    destination: MapDesign/MCP/TASKS/MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS_REMEDIATION_V1_2.md
    mode: create
forbidden_operations:
  ...
  - delete_or_rewrite_accepted_legacy_folder_meta
...
- 적용 전/후 Current Task는 `TASKS/MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS.md`다.
- exact state는 `28 COMPLETE / MAP02_02 CURRENT / 176 LOCKED`이며 Master/Status/current Task는 교체하지 않는다.
- MAP02_02 최신 Result는 exact `STATUS: BLOCKED`이지만 read-scope incident acceptance, implementation, known vector, focused 103/103, targeted 1026/1026, full EditMode 1046/1046, compile/Console은 모두 PASS다.
- 유일 차단 사유는 필수 Unity refresh가 기존 MAP01_15 Editor 경로의 folder meta exact 6개를 자동 복원해 v1.1의 Assets drift 0 조건과 충돌한 것이다.
- patch apply는 v1.2 remediation addendum 1개만 create하고 Assets/C#/tests/CSV/meta/Master/Status/Result를 변경하지 않는다.
- 실행 단계에서는 위 exact 6개 folder meta를 삭제·재작성하지 않고 Unity 관리 metadata로 수용해 형식/GUID/경로/변경 범위를 감사한다.
- MAP02_03 이후는 계속 LOCKED이며 자동 시작하지 않는다.
