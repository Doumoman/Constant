...
```text
TASKS/MAP01_11_IMPLEMENT_FOREIGN_KEY_RESOLVER.md
```
...
현재는 49개 Authoring static CSV의 successful parsed records와 schema catalog을 기준으로 단일·list FK를 deterministic 2차/3차 pass로 해석한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_11_IMPLEMENT_FOREIGN_KEY_RESOLVER` 하나다.

StaticDataRegistry, domain validation, polymorphic implicit reference resolution은 구현하지 않는다. 기존 CSV·reader·validator·PK·parser·definitions·schema catalog/importer·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_11이 PASS해도 `MAP01_12_IMPLEMENT_STATIC_DATA_REGISTRY`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_11을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
