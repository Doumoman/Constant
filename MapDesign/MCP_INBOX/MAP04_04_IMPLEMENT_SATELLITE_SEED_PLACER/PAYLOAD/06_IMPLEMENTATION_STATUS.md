...
```text
TASKS/MAP04_04_IMPLEMENT_SATELLITE_SEED_PLACER.md
```
...
| MAP04_04_IMPLEMENT_SATELLITE_SEED_PLACER | CURRENT |
...
```text
MAP04_03_IMPLEMENT_CORE_PATCH_GROWER
```

## Last Result

```text
REPORTS/MAP04_03_IMPLEMENT_CORE_PATCH_GROWER_RESULT.md
STATUS: PASS
```
...
현재는 MAP04_03의 immutable Core-grown partial P02 snapshot 위에 active four Satellite rule의 seed count를 fresh current-attempt `RNG_BIOME_PATCH`로 먼저 추첨하고, unassigned·unreserved sector에서 같은 biome 기존 patch/seed와 최소 거리를 지키는 one-cell Satellite seed patch를 atomic 배치한다. Core 20 sectors와 site bindings는 그대로 보존하며 나머지 전역 성장은 수행하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP04_04_IMPLEMENT_SATELLITE_SEED_PLACER` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime/Test `Generation` 폴더에 Satellite seed placer production C# 7개, focused test C# 1개와 matching meta 8개만 추가한다. MAP04_01~03 existing files는 수정하지 않는다. Satellite patch 성장, full-map multi-seed cost/ownership, Intrusion, cleanup, serializer/export, validator, overlay, pass/root adapter는 만들지 않는다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP04_04가 PASS하면 STATUS FINALIZE는 MAP04_04만 COMPLETE로 바꾸며 Current Task를 NONE으로 만든 뒤 종료한다. `MAP04_05_IMPLEMENT_MULTI_SEED_BIOME_GROWER`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
