...
```text
TASKS/MAP01_12_IMPLEMENT_STATIC_DATA_REGISTRY.md
```
...
| MAP01_12_IMPLEMENT_STATIC_DATA_REGISTRY | CURRENT |
...
현재는 world/route, biome/boundary, special/village, microchunk/population/item definition set과 successful FK result를 하나의 immutable `StaticDataRegistry` snapshot으로 조립한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_12_IMPLEMENT_STATIC_DATA_REGISTRY` 하나다.

Content hash, global singleton install, atomic replacement/publish, domain validation은 구현하지 않는다. 기존 CSV·loader·definitions·FK resolver·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_12가 PASS해도 `MAP01_13_IMPLEMENT_CONTENT_VERSION_HASH`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_12를 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
