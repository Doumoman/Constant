...
```text
TASKS/MAP05_01_BUILD_MANDATORY_TERMINALS.md
```
...
| MAP05_01_BUILD_MANDATORY_TERMINALS | CURRENT |
...
```text
MAP04_11_MAP04_BATCH_AND_EXIT_TESTS
```

## Last Result

```text
REPORTS/MAP04_11_MAP04_BATCH_AND_EXIT_TESTS_RESULT.md
STATUS: PASS
```
...
현재는 approved P01 Site Reservation과 P02 Biome publication에서 StartAnchor 1개와 six required site entry 6개를 exact immutable mandatory terminal set으로 만든다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP05_01_BUILD_MANDATORY_TERMINALS` 하나다.

실제 Authoring/generated CSV와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. approved Runtime/Test `Generation`에 production C# 8개, focused test 1개와 matching meta 9개만 추가한다. route mask/tree/router/graph는 시작하지 않는다.

MAP05_01이 PASS하면 MAP05_01만 COMPLETE, Current Task NONE으로 finalize한다. `MAP05_02_IMPLEMENT_ROUTE_MASK_LOOKUP`은 LOCKED로 유지한다.
