...
```text
TASKS/MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS.md
```
...
현재는 test-owned temporary copy에서 CSV 실패 fixture를 생성해 duplicate PK, enum/number, missing FK, BOM, ordering과 previous Registry 보존을 검증한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS` 하나다.

실제 Authoring CSV/meta, Runtime/Editor production pipeline, asmdef, Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. fixture는 test temp directory에서만 생성하고 종료 시 own temp만 정리한다.

MAP01_16이 PASS해도 `MAP01_17_MAP01_EXIT_AUDIT`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_16을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
