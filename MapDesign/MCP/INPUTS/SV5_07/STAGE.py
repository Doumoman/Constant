#!/usr/bin/env python3
"""SV5_07 package verification and single-MD staging; no Apply, Unity, Finalize or commit."""
import argparse
import collections
import hashlib
import json
...
PREFIX = 'MapDesign/MCP/INPUTS/SV5_07'
...
def package(expected):
    ...
    need(m['task'] == TASK and m['format'] == 'sv5_07_package_v1', 'Wrong package')
    ...
    need(lock['format'] == 'sv5_07_source_lock_v1', 'Wrong source lock format')
...
def check_state(root, lock, post):
    ...
    need(len(registered) == 1, 'SV5_07 must already be registered exactly once in Master')
    phase = rows.get(TASK)
    if not post:
        need(phase == 'LOCKED' and current == 'NONE', 'Stage requires SV5_07 LOCKED / Current NONE')
        need(counts == {'COMPLETE': 249, 'LOCKED': 41}, 'Preflight status counts differ')
        need(sha(data) == lock['status_before_sha256'], 'Preflight Status bytes changed')
    else:
        need(phase in ('CURRENT', 'COMPLETE'), 'Post-readonly requires SV5_07 CURRENT or COMPLETE')
        need(current == ('TASKS/' + TASK + '.md' if phase == 'CURRENT' else 'NONE'), 'Current Task mismatch')
        expected = {'COMPLETE': 249, 'CURRENT': 1, 'LOCKED': 40} if phase == 'CURRENT' else {'COMPLETE': 250, 'LOCKED': 40}
...
def check_git(root, lock, post):
    ...
    if not post:
        ...
        need(not git(root, 'status', '--porcelain', '--untracked-files=all', '--', *owned),
             'Existing dirty changes overlap SV5_07 implementation/evidence paths')
...
def local(root, lock, manifest, post=False):
    ...
    need(HERE == safe(root, PREFIX).resolve(), 'Run the helper in this project INPUTS/SV5_07')
