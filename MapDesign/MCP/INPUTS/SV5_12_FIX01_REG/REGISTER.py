...
TASK = "SV5_12_FIX01"
PATCH = "SV5_12_FIX01_REG"
BASE = "3f05858cb80d6437e89123fb6b6dd30a395996fc"
STATUS_REL = "MapDesign/MCP/06_IMPLEMENTATION_STATUS.md"
MASTER_REL = "MapDesign/MCP/MASTER_IMPLEMENTATION_TASK_LIST.md"
INBOX_REL = "MapDesign/MCP_INBOX/SV5_12_FIX01.md"
TASK_REL = "MapDesign/MCP/TASKS/SV5_12_FIX01.md"
ARCHIVE_REL = "MapDesign/MCP_ARCHIVE/SV5_12_FIX01.md"
RESULT_REL = "MapDesign/MCP/REPORTS/SV5_12_FIX01_RESULT.md"
REG_DIR_REL = "MapDesign/MCP/GENERATED/SV5_12_FIX01_REG"
...
def verify_package(package: Path, expected_manifest_sha: str, errors: list[str]) -> dict:
    ...
    try:
        ...
        return {}
    if manifest.get("schema") != "SV5_12_FIX01_REG_FILES/v1":
    ...
    required = {
        ...
        "TREE_CANOPY_PROFILE.json", "SV5_12_FIX01.md", "REGISTER.py",
...
def verify_before(project: Path, package: Path, lock: dict, errors: list[str]) -> dict:
    ...
    expected = {"COMPLETE": 259, "CURRENT": 0, "LOCKED": 35}
    if counts != expected:
    ...
    if status.count("| SV5_12_TREE_GRAB | COMPLETE |") != 1:
        fail(errors, "SV5_12 COMPLETE row mismatch")
    if status.count("| SV5_13_JUMP_CONTRACT | LOCKED |") != 1:
        fail(errors, "SV5_13 LOCKED row mismatch")
    if TASK in status or TASK in master:
        fail(errors, "SV5_12_FIX01 is already registered")
    if master.count("| 12 | SV5_12_TREE_GRAB | LOCKED |") != 1:
        fail(errors, "SV5_12 Master anchor mismatch")
    if master.count("| 13 | SV5_13_JUMP_CONTRACT | LOCKED |") != 1:
        fail(errors, "SV5_13 Master anchor mismatch")
...
def atomic_write(path: Path, data: bytes) -> None:
    ...
    temp = path.with_name(path.name + ".SV5_12_FIX01.tmp")
    if temp.exists():
...
def apply_registration(project: Path, package: Path, manifest_sha: str, before: dict) -> dict:
    ...
    new_master = replace_exact(
        ...
        "| 12 | SV5_12_TREE_GRAB | LOCKED |\n| 12.F1 | SV5_12_FIX01 | LOCKED |\n| 13 | SV5_13_JUMP_CONTRACT | LOCKED |",
        "Master",
    )
    task_bytes = (package / "SV5_12_FIX01.md").read_bytes()
    ...
    try:
        ...
        receipt = {
            "schema": "SV5_12_FIX01_REG_RECEIPT/v1",
            "patch_id": PATCH,
            ...
            "state_after": {"COMPLETE": 259, "CURRENT": 0, "LOCKED": 36},
            "current_task": "NONE",
...
def verify_registered(project: Path, package: Path, applied: bool, errors: list[str]) -> dict:
    ...
    if master.count("| 12.F1 | SV5_12_FIX01 | LOCKED |") != 1:
        fail(errors, "registered Task Master row mismatch")
    if status.count("| SV5_12_TREE_GRAB | COMPLETE |") != 1 or status.count("| SV5_13_JUMP_CONTRACT | LOCKED |") != 1:
        fail(errors, "neighbor state changed")
    expected_counts = {"COMPLETE": 259, "CURRENT": 1 if applied else 0,
    ...
    if applied:
        ...
        if native_candidates(project) != ["SV5_12_FIX01.md"]:
