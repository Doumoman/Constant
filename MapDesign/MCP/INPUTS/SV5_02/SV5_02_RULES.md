...
4. 업로드 Result는 Finalize 이후 commit SHA를 담지 않는다. 실제 최종 Status/Current와 SV5_01을 담은 commit을 현지에서 확인하고 BINDING에 기록한다.
...
- GENERATED/SV5_01/BASELINE.json, PLAN_LINK.json, BINDING.json.
- MCP/INPUTS/SV5의 SPACE_V5_RULES.md, SPACE_V5_MEMORY.md, SPACE_V5_TASKS.md와 필요한 reference/v4 명세.
- MCP/INPUTS/SV5_02/SOURCE_LOCK.json, candidate/의 3개 파일, PROTOCOL_APPEND.md, 이 원본 명세.
- 규칙을 읽는 실제 문서 진입점 한 곳. 기존 프로토콜 연결을 확인하며 모든 소스 API 조사는 SV5_03에 남긴다.

경로는 보고서와 같은 MapDesign 모듈 루트 기준이다. 실제 모듈 루트가 다르면 이를 바인딩해 기록한다.
MCP/INPUTS/SV5 원본과 첫 입력 manifest는 수정하지 않는다. 새 입력은 SV5_02 폴더에만 추가한다.
...
- GENERATED/SV5_02/ 이하 RULE_REGISTRATION.json, BINDING.json, validation.json과 정상 Result 경로.
