...
BASE = "289015d201186fe79da733a5fe99fdb6844047fb"
STATUS_REL = "MapDesign/MCP/06_IMPLEMENTATION_STATUS.md"
MASTER_REL = "MapDesign/MCP/MASTER_IMPLEMENTATION_TASK_LIST.md"
INBOX_REL = "MapDesign/MCP_INBOX/SV5_16_JUMP_OUTLINE.md"
TASK_REL = "MapDesign/MCP/TASKS/SV5_16_JUMP_OUTLINE.md"
ARCHIVE_REL = "MapDesign/MCP_ARCHIVE/SV5_16_JUMP_OUTLINE.md"
RESULT_REL = "MapDesign/MCP/REPORTS/SV5_16_JUMP_OUTLINE_RESULT.md"
GENERATED_REL = "MapDesign/MCP/GENERATED/SV5_16_JUMP_OUTLINE"
...
def verify_package(package: Path, expected_sha: str, errors: list[str]) -> None:
    ...
    try:
        ...
        return
    add(errors, manifest.get("schema") == "SV5_16_JUMP_OUTLINE_FILES/v1",
    ...
    required = {"README.md", "CONTRACT.md", "JUMP_OUTLINE_PROFILE.json", "SOURCE_LOCK.json",
                "SV5_16_JUMP_OUTLINE.md", "STAGE.py", "tools/check_jump_outline.py"}
...
def verify_state(project: Path, package: Path, phase: str, errors: list[str]) -> dict:
    ...
    expected = {"COMPLETE": 263, "CURRENT": 1 if applied else 0,
                "LOCKED": 31 if applied else 32}
    ...
    add(errors, status.count("| SV5_15_JUMP_GRAB | COMPLETE |") == 1,
        "SV5_15_JUMP_GRAB predecessor row mismatch")
    add(errors, status.count(f"| {TASK} | {'CURRENT' if applied else 'LOCKED'} |") == 1,
        "SV5_16 row mismatch")
    add(errors, status.count("| SV5_17_JUMP_RECIPES | LOCKED |") == 1,
        "SV5_17 must remain LOCKED")
    add(errors, master.count("| 16 | SV5_16_JUMP_OUTLINE | LOCKED |") == 1,
        "SV5_16 Master membership mismatch")
    task_bytes = (package / "SV5_16_JUMP_OUTLINE.md").read_bytes()
    if phase == "before":
        ...
        add(errors, candidates(project) == ["SV5_16_JUMP_OUTLINE.md"],
...
def owned_overlap(project: Path, lock: dict, errors: list[str]) -> None:
    ...
    paths += [INBOX_REL, TASK_REL, ARCHIVE_REL, RESULT_REL, GENERATED_REL,
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpOutlineGeometry.cs",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpOutlineGeometry.cs.meta",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpOutlineExport.cs",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpOutlineExport.cs.meta",
              "Assets/_Game/Tests/EditMode/Map/SV5/Sv5JumpOutlineTests.cs",
              "Assets/_Game/Tests/EditMode/Map/SV5/Sv5JumpOutlineTests.cs.meta",
              "MapDesign/MCP/SV5/24_JUMP_OUTLINE_V5.md"]
...
def atomic_write(path: Path, data: bytes) -> None:
    ...
    temp = path.with_name(path.name + ".SV5_16.tmp")
    if temp.exists():
...
def main() -> int:
    ...
    if args.stage:
        ...
        atomic_write(path, (package / "SV5_16_JUMP_OUTLINE.md").read_bytes())
        post_errors: list[str] = []
        verify_state(project, package, "staged", post_errors)
        if post_errors:
            if path.exists() and path.read_bytes() == (package / "SV5_16_JUMP_OUTLINE.md").read_bytes():
                path.unlink()
