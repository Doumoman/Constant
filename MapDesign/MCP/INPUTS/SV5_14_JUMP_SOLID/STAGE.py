...
BASE = "33a635905345ff2a42facc3add5c6609eea9357c"
STATUS_REL = "MapDesign/MCP/06_IMPLEMENTATION_STATUS.md"
MASTER_REL = "MapDesign/MCP/MASTER_IMPLEMENTATION_TASK_LIST.md"
INBOX_REL = "MapDesign/MCP_INBOX/SV5_14_JUMP_SOLID.md"
TASK_REL = "MapDesign/MCP/TASKS/SV5_14_JUMP_SOLID.md"
ARCHIVE_REL = "MapDesign/MCP_ARCHIVE/SV5_14_JUMP_SOLID.md"
RESULT_REL = "MapDesign/MCP/REPORTS/SV5_14_JUMP_SOLID_RESULT.md"
GENERATED_REL = "MapDesign/MCP/GENERATED/SV5_14_JUMP_SOLID"
...
def verify_package(package: Path, expected_sha: str, errors: list[str]) -> None:
    ...
    try:
        ...
        return
    add(errors, manifest.get("schema") == "SV5_14_JUMP_SOLID_FILES/v1",
    ...
    required = {"README.md", "CONTRACT.md", "JUMP_SOLID_PROFILE.json", "SOURCE_LOCK.json",
                "SV5_14_JUMP_SOLID.md", "STAGE.py", "tools/check_jump_solid.py"}
...
def verify_state(project: Path, package: Path, phase: str, errors: list[str]) -> dict:
    ...
    expected = {"COMPLETE": 261, "CURRENT": 1 if applied else 0,
                "LOCKED": 33 if applied else 34}
    ...
    add(errors, status.count("| SV5_13_JUMP_CONTRACT | COMPLETE |") == 1,
        "SV5_13_JUMP_CONTRACT predecessor row mismatch")
    add(errors, status.count(f"| {TASK} | {'CURRENT' if applied else 'LOCKED'} |") == 1,
        "SV5_14 row mismatch")
    add(errors, status.count("| SV5_15_JUMP_GRAB | LOCKED |") == 1,
        "SV5_15 must remain LOCKED")
    add(errors, master.count("| 14 | SV5_14_JUMP_SOLID | LOCKED |") == 1,
        "SV5_14 Master membership mismatch")
    task_bytes = (package / "SV5_14_JUMP_SOLID.md").read_bytes()
    if phase == "before":
        ...
        add(errors, candidates(project) == ["SV5_14_JUMP_SOLID.md"],
...
def owned_overlap(project: Path, lock: dict, errors: list[str]) -> None:
    ...
    paths += [INBOX_REL, TASK_REL, ARCHIVE_REL, RESULT_REL, GENERATED_REL,
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpSolidGeometry.cs",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpSolidGeometry.cs.meta",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpSolidExport.cs",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpSolidExport.cs.meta",
              "Assets/_Game/Tests/EditMode/Map/SV5/Sv5JumpSolidTests.cs",
              "Assets/_Game/Tests/EditMode/Map/SV5/Sv5JumpSolidTests.cs.meta",
              "MapDesign/MCP/SV5/22_JUMP_SOLID_V5.md"]
...
def atomic_write(path: Path, data: bytes) -> None:
    ...
    temp = path.with_name(path.name + ".SV5_14.tmp")
    if temp.exists():
...
def main() -> int:
    ...
    if args.stage:
        ...
        atomic_write(path, (package / "SV5_14_JUMP_SOLID.md").read_bytes())
        post_errors: list[str] = []
        verify_state(project, package, "staged", post_errors)
        if post_errors:
            if path.exists() and path.read_bytes() == (package / "SV5_14_JUMP_SOLID.md").read_bytes():
                path.unlink()
