#!/usr/bin/env python3
"""SV5_08_FIX02 package verification and single-MD staging; no Apply, Unity, Finalize or commit."""
import argparse
import collections
import hashlib
import json
...
PREFIX = 'MapDesign/MCP/INPUTS/SV5_08_FIX02'
...
def package(expected):
    ...
    need(m['task'] == TASK and m['format'] == 'sv5_08_fix02_package_v1', 'Wrong package')
    ...
    need(lock['format'] == 'sv5_08_fix02_source_lock_v1', 'Wrong source lock format')
...
def status_state(data):
    ...
    need(len(rows) in (291,292) and len(dict(rows)) == len(rows), 'Status row count/duplicate mismatch')
...
def check_state(root, lock, post):
    ...
    need(len(registered) == 1, 'SV5_08_FIX02 must already be registered exactly once in Master')
    need(len(rows)==292, 'Registered status must have292 rows')
    phase = rows.get(TASK)
    if not post:
        need(phase == 'LOCKED' and current == 'NONE', 'Stage requires SV5_08_FIX02 LOCKED / Current NONE')
        need(counts == {'COMPLETE': 252, 'LOCKED': 40}, 'Preflight status counts differ')
        need(sha(data) == lock['status_before_sha256'], 'Preflight Status bytes changed')
    else:
        need(phase in ('CURRENT', 'COMPLETE'), 'Post-readonly requires SV5_08_FIX02 CURRENT or COMPLETE')
        need(current == ('TASKS/' + TASK + '.md' if phase == 'CURRENT' else 'NONE'), 'Current Task mismatch')
        expected = {'COMPLETE': 252, 'CURRENT': 1, 'LOCKED': 39} if phase == 'CURRENT' else {'COMPLETE': 253, 'LOCKED': 39}
...
def check_sources(root, lock, post, phase):
    ...
    need(re.search(r'^TASK_ID: ' + PREV + r'\r?$', previous, re.M) and not re.search(r'^TASK: ' + PREV + r'\r?$', previous, re.M) and re.search(r'^STATUS: PASS\r?$', previous, re.M),
         'This audit repair requires the exact predecessor TASK_ID-only defect')
    if phase == 'COMPLETE':
        ...
        need(re.search(r'^TASK_ID: ' + TASK + r'\r?$', report, re.M), 'COMPLETE Result also requires TASK_ID')
        audit = read_json(safe(root, 'MapDesign/MCP/GENERATED/SV5_08_FIX02/finalize_audit.json'))
        need(audit.get('status') == 'PASS_AUDITED_CORRECTION' and audit.get('prior_result_modified') is False,
             'COMPLETE requires the exact passing finalize audit')
    return checked
...
def check_git(root, lock, post):
    ...
    if not post:
        ...
        need(not git(root, 'status', '--porcelain', '--untracked-files=all', '--', *owned),
             'Existing dirty changes overlap SV5_08_FIX02 implementation/evidence paths')
...
def local(root, lock, manifest, post=False):
    ...
    need(HERE == safe(root, PREFIX).resolve(), 'Run the helper in this project INPUTS/SV5_08_FIX02')
...
def pre_registration(root, lock, manifest):
    ...
    need(len(rows)==291 and TASK not in rows and current=='NONE' and counts=={'COMPLETE':252,'LOCKED':39}, 'Registration before state mismatch')
