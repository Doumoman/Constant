...
BASE = "b99f9b9214583d653c273efaa53e69ec0e4abbe4"
STATUS_REL = "MapDesign/MCP/06_IMPLEMENTATION_STATUS.md"
MASTER_REL = "MapDesign/MCP/MASTER_IMPLEMENTATION_TASK_LIST.md"
INBOX_REL = "MapDesign/MCP_INBOX/SV5_17_JUMP_RECIPES.md"
TASK_REL = "MapDesign/MCP/TASKS/SV5_17_JUMP_RECIPES.md"
ARCHIVE_REL = "MapDesign/MCP_ARCHIVE/SV5_17_JUMP_RECIPES.md"
RESULT_REL = "MapDesign/MCP/REPORTS/SV5_17_JUMP_RECIPES_RESULT.md"
GENERATED_REL = "MapDesign/MCP/GENERATED/SV5_17_JUMP_RECIPES"
...
def verify_package(package: Path, expected_sha: str, errors: list[str]) -> None:
    ...
    try:
        ...
        return
    add(errors, manifest.get("schema") == "SV5_17_JUMP_RECIPES_FILES/v1",
    ...
    required = {"README.md", "CONTRACT.md", "JUMP_RECIPE_PROFILE.json",
                "REFERENCE_012_PROFILE.json", "SOURCE_LOCK.json",
                "SV5_17_JUMP_RECIPES.md", "STAGE.py", "tools/check_jump_recipes.py"}
...
def verify_state(project: Path, package: Path, phase: str, errors: list[str]) -> dict:
    ...
    expected = {"COMPLETE": 264, "CURRENT": 1 if applied else 0,
                "LOCKED": 30 if applied else 31}
    ...
    add(errors, status.count("| SV5_16_JUMP_OUTLINE | COMPLETE |") == 1,
        "SV5_16 predecessor row mismatch")
    add(errors, status.count(f"| {TASK} | {'CURRENT' if applied else 'LOCKED'} |") == 1,
        "SV5_17 row mismatch")
    add(errors, status.count("| SV5_18_JUMP_CLEARANCE | LOCKED |") == 1,
        "SV5_18 must remain LOCKED")
    add(errors, master.count("| 17 | SV5_17_JUMP_RECIPES | LOCKED |") == 1,
        "SV5_17 Master membership mismatch")
    task_bytes = (package / "SV5_17_JUMP_RECIPES.md").read_bytes()
    if phase == "before":
        ...
        add(errors, candidates(project) == ["SV5_17_JUMP_RECIPES.md"],
...
def owned_overlap(project: Path, lock: dict, errors: list[str]) -> None:
    ...
    paths += [INBOX_REL, TASK_REL, ARCHIVE_REL, RESULT_REL, GENERATED_REL,
              ...
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpRecipeCatalog.cs.meta",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpRecipeExport.cs",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpRecipeExport.cs.meta",
              "Assets/_Game/Tests/EditMode/Map/SV5/Sv5JumpRecipeTests.cs",
              "Assets/_Game/Tests/EditMode/Map/SV5/Sv5JumpRecipeTests.cs.meta",
              "MapDesign/MCP/SV5/25_JUMP_RECIPES_V5.md"]
...
def atomic_write(path: Path, data: bytes) -> None:
    ...
    temp = path.with_name(path.name + ".SV5_17.tmp")
    if temp.exists():
...
def main() -> int:
    ...
    if args.stage:
        ...
        atomic_write(path, (package / "SV5_17_JUMP_RECIPES.md").read_bytes())
        post_errors: list[str] = []
        verify_state(project, package, "staged", post_errors)
        if post_errors:
            if path.exists() and path.read_bytes() == (package / "SV5_17_JUMP_RECIPES.md").read_bytes():
                path.unlink()
