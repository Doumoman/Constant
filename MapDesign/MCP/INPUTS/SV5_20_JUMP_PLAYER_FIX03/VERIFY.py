...
def same(path: Path, spec: dict) -> bool:
    return (path.is_file() and path.stat().st_size == spec["bytes"]
            and digest(path.read_bytes()) == spec["sha256"])
...
def verify_package(package: Path, manifest_sha: str, errors: list[str]) -> None:
    ...
    if not path.is_file() or not re.fullmatch(r"[0-9a-f]{64}", manifest_sha):
        errors.append("FILES.json missing or invalid manifest argument")
        return
    if digest(path.read_bytes()) != manifest_sha:
    ...
    try:
        ...
        return
    if (manifest.get("schema") != "SV5_20_JUMP_PLAYER_FIX03_FILES/v1"
    ...
    required = {"README.md", "AMENDMENT.md", "GEOMETRY_PATCH.json",
                "SCHEDULER_PROFILE.json", "SOURCE_LOCK.json", "VERIFY.py",
                "tools/check_jump_player_fix03.py"}
    errors.extend("required package entry not manifested: " + rel
                  for rel in sorted(required - seen))


def verify_base_file(project: Path, entry: dict, errors: list[str]) -> None:
    rel = entry["path"]
    try:
        ...
        errors.append("base file unavailable: " + rel + ":" + str(exc))
        return
    if (oid != entry["git_blob_oid"] or digest(raw) != entry["sha256"]
            or len(raw) != entry["bytes"] or work != raw):
        errors.append("immutable predecessor mismatch: " + rel)


def verify_dynamic_readonly(project: Path, rel: str, errors: list[str]) -> None:
    try:
        oid = git(project, "rev-parse", f"{BASE}:{rel}")
        raw = subprocess.run(["git", "cat-file", "blob", oid], cwd=project,
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE,
...
def verify_failure(project: Path, spec: dict, errors: list[str]) -> list[str]:
    found: list[str] = []
    for rel in spec["paths"]:
        path = project / rel
        if same(path, spec):
            text = path.read_text(encoding="utf-8-sig", errors="replace")
            if spec["required_failure_token"] in text:
                found.append(rel)
    if not found:
        errors.append("byte-exact failure evidence missing: " + spec["label"])
    return found
...
def main() -> int:
    ...
    status_path = project / "MapDesign/MCP/06_IMPLEMENTATION_STATUS.md"
    master_path = project / "MapDesign/MCP/MASTER_IMPLEMENTATION_TASK_LIST.md"
    ...
    if current(status) != TASK or status.count("| SV5_20_JUMP_PLAYER | CURRENT |") != 1:
        errors.append("SV5_20 Current state mismatch")
    ...
    for name in ("original_package_manifest", "fix01_package_manifest",
                 "fix02_package_manifest", "installed_task", "archive_task"):
        spec = lock.get(name, {})
        if not spec or not same(project / spec.get("path", "__missing__"), spec):
            errors.append(name + " byte mismatch")
    installed, archived = lock.get("installed_task", {}), lock.get("archive_task", {})
    if installed and archived:
        ip, ap = project / installed["path"], project / archived["path"]
        if ip.is_file() and ap.is_file() and ip.read_bytes() != ap.read_bytes():
            errors.append("installed Task and Archive differ")

    for entry in lock.get("base_files", []):
        verify_base_file(project, entry, errors)
    for rel in lock.get("base_commit_readonly_paths", []):
        verify_dynamic_readonly(project, rel, errors)
    evidence = {spec["label"]: verify_failure(project, spec, errors)
                for spec in lock.get("failure_evidence", [])}

    if run(project, "git", "diff", "--cached", "--quiet", check=False).returncode != 0:
        errors.append("staged changes are not allowed before Finalize")

    if args.post_readonly:
        out = project / "MapDesign/MCP/GENERATED/SV5_20_JUMP_PLAYER"
        for name in (
            "player_geometry_fix03.json", "player_geometry_patch.csv",
            "player_composed_occupancy.csv", "player_effective_links.csv",
            "player_scheduler_audit.json", "player_main_link_diagnostic_results.xml",
            "independent_jump_player_fix03_audit.json",
            "targeted_playmode_before_fix03.xml",
        ):
            if not (out / name).is_file():
                errors.append("missing FIX03 output: " + name)
        spec = next((x for x in lock.get("failure_evidence", [])
                     if x.get("label") == "BEFORE_FIX03_SIX_SYMMETRIC_FAILURES"), None)
        if spec and not same(out / "targeted_playmode_before_fix03.xml", spec):
            errors.append("before-FIX03 evidence not byte-exact")

    if errors:
        print(json.dumps({"status": "BLOCKED",
                          "phase": "post" if args.post_readonly else "check",
                          "errors": errors}, ensure_ascii=False, indent=2))
        return 1
    print(json.dumps({
        "status": "PASS_FIX03_POST_READONLY" if args.post_readonly
                  else "PASS_FIX03_ACTIVE_TASK_PREFLIGHT",
        "task": TASK,
        "base_commit": BASE,
