#!/usr/bin/env python3
"""SV5_08 package verification and single-MD staging; no Apply, Unity, Finalize or commit."""
import argparse
import collections
import hashlib
import json
...
TASK = 'SV5_08_INFILL'
PREV = 'SV5_07_DIVERSITY'
NEXT = 'SV5_09_LOOPS'
HERE = Path(__file__).resolve().parent
PREFIX = 'MapDesign/MCP/INPUTS/SV5_08'
...
def package(expected):
    ...
    need(m['task'] == TASK and m['format'] == 'sv5_08_package_v1', 'Wrong package')
    ...
    need(lock['format'] == 'sv5_08_source_lock_v1', 'Wrong source lock format')
...
def status_state(data):
    ...
    need(len(rows) == 290 and len(dict(rows)) == 290, 'Status row count/duplicate mismatch')
...
def check_state(root, lock, post):
    ...
    need(len(registered) == 1, 'SV5_08 must already be registered exactly once in Master')
    phase = rows.get(TASK)
    if not post:
        need(phase == 'LOCKED' and current == 'NONE', 'Stage requires SV5_08 LOCKED / Current NONE')
        need(counts == {'COMPLETE': 250, 'LOCKED': 40}, 'Preflight status counts differ')
        need(sha(data) == lock['status_before_sha256'], 'Preflight Status bytes changed')
    else:
        need(phase in ('CURRENT', 'COMPLETE'), 'Post-readonly requires SV5_08 CURRENT or COMPLETE')
        need(current == ('TASKS/' + TASK + '.md' if phase == 'CURRENT' else 'NONE'), 'Current Task mismatch')
        expected = {'COMPLETE': 250, 'CURRENT': 1, 'LOCKED': 39} if phase == 'CURRENT' else {'COMPLETE': 251, 'LOCKED': 39}
...
def check_git(root, lock, post):
    ...
    if not post:
        ...
        need(not git(root, 'status', '--porcelain', '--untracked-files=all', '--', *owned),
             'Existing dirty changes overlap SV5_08 implementation/evidence paths')
...
def local(root, lock, manifest, post=False):
    ...
    need(HERE == safe(root, PREFIX).resolve(), 'Run the helper in this project INPUTS/SV5_08')
