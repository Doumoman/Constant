...
INBOX_REL = "MapDesign/MCP_INBOX/SV5_20_JUMP_PLAYER.md"
TASK_REL = "MapDesign/MCP/TASKS/SV5_20_JUMP_PLAYER.md"
ARCHIVE_REL = "MapDesign/MCP_ARCHIVE/SV5_20_JUMP_PLAYER.md"
RESULT_REL = "MapDesign/MCP/REPORTS/SV5_20_JUMP_PLAYER_RESULT.md"
GENERATED_REL = "MapDesign/MCP/GENERATED/SV5_20_JUMP_PLAYER"
...
def verify_package(package: Path, expected_sha: str, errors: list[str]) -> None:
    ...
    try:
        ...
        return
    add(errors, manifest.get("schema") == "SV5_20_JUMP_PLAYER_FILES/v1",
    ...
    required = {"README.md", "CONTRACT.md", "PLAYER_PROFILE.json",
                "SOURCE_LOCK.json", "SV5_20_JUMP_PLAYER.md", "STAGE.py",
                "FIX04_PLAYER_BINDING_AMENDMENT.json",
                "FIX04_PLAYER_BINDING_AMENDMENT.md",
                "FIX05_RMAP_SCENE_ISOLATION_AMENDMENT.json",
                "FIX05_RMAP_SCENE_ISOLATION_AMENDMENT.md",
                "tools/check_jump_player.py",
                "tools/write_player_binding_amendment.py"}
...
def verify_git(project: Path, lock: dict, errors: list[str]) -> None:
    ...
    for rel in lock.get("base_commit_readonly_paths", []):
        try:
            cat_blob(project, BASE, rel)
        except Exception as exc:
            errors.append(f"base Player input unavailable: {rel}: {exc}")
    for entry in lock.get("approved_runtime_bindings", []):
        rel = entry["path"]
        try:
            oid, raw = cat_blob(project, BASE, rel)
            work = (project / rel).read_bytes()
        except Exception as exc:
            errors.append(f"approved runtime binding unavailable: {rel}: {exc}")
            continue
        add(errors, oid == entry.get("base_git_blob_oid"),
            "approved runtime base OID mismatch: " + rel)
        add(errors, digest(raw) == entry.get("base_sha256") and
            len(raw) == entry.get("base_bytes"),
            "approved runtime base bytes mismatch: " + rel)
        add(errors, digest(work) == entry.get("approved_sha256") and
            len(work) == entry.get("approved_bytes"),
            "approved runtime current bytes mismatch: " + rel)
        add(errors, entry.get("role") == "APPROVED_FIX04_RUNTIME_CORRECTION",
            "approved runtime role mismatch: " + rel)
...
def verify_state(project: Path, package: Path, phase: str,
    ...
    expected = {"COMPLETE": 267, "CURRENT": 1 if applied else 0,
                "LOCKED": 27 if applied else 28}
    ...
    add(errors, status.count("| SV5_19_JUMP_RECOVERY | COMPLETE |") == 1,
        "SV5_19 predecessor row mismatch")
    add(errors, status.count(f"| {TASK} | {'CURRENT' if applied else 'LOCKED'} |") == 1,
        "SV5_20 row mismatch")
    add(errors, status.count("| SV5_21_LIBRARY | LOCKED |") == 1,
        "SV5_21 must remain LOCKED")
    add(errors, master.count("| 20 | SV5_20_JUMP_PLAYER | LOCKED |") == 1,
        "SV5_20 Master membership mismatch")
    task_bytes = (package / "SV5_20_JUMP_PLAYER.md").read_bytes()
    if phase == "before":
        ...
        add(errors, candidates(project) == ["SV5_20_JUMP_PLAYER.md"],
...
def task_owned(lock: dict) -> list[str]:
    return ([entry["path"] for entry in lock.get("files", [])] +
            list(lock.get("base_commit_readonly_paths", [])) +
            [entry["path"] for entry in lock.get("approved_runtime_bindings", [])] + [
            entry["path"] for entry in lock.get("approved_test_harness_paths", [])] + [
        INBOX_REL, TASK_REL, ARCHIVE_REL, RESULT_REL, GENERATED_REL,
        "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpPlayerVerification.cs",
        "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpPlayerVerification.cs.meta",
        "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpPlayerExport.cs",
        "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpPlayerExport.cs.meta",
        "Assets/_Game/Tests/EditMode/Map/SV5/Sv5JumpPlayerProofTests.cs",
        "Assets/_Game/Tests/EditMode/Map/SV5/Sv5JumpPlayerProofTests.cs.meta",
        "Assets/_Game/Tests/PlayMode/Map/SV5/Sv5JumpPlayerPlayModeTests.cs",
        "Assets/_Game/Tests/PlayMode/Map/SV5/Sv5JumpPlayerPlayModeTests.cs.meta",
...
def verify_immutable(project: Path, lock: dict, errors: list[str]) -> None:
    ...
    paths += list(lock.get("base_commit_readonly_paths", []))
    proc = run(project, "git", "status", "--porcelain=v1", "--untracked-files=all",
               "--", *paths, check=False)
    add(errors, proc.returncode == 0 and not proc.stdout.strip(),
        "immutable predecessor/Player path changed:\n" + proc.stdout.rstrip())


def atomic_write(path: Path, data: bytes) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    temp = path.with_name(path.name + ".SV5_20.tmp")
    if temp.exists():
...
def main() -> int:
    ...
    if args.stage:
        ...
        data = (package / "SV5_20_JUMP_PLAYER.md").read_bytes()
