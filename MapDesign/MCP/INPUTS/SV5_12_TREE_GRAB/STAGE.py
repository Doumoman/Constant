...
BASE = "aeedc340670843d0bbeb43d3aad853a7953ba3b9"
STATUS_REL = "MapDesign/MCP/06_IMPLEMENTATION_STATUS.md"
MASTER_REL = "MapDesign/MCP/MASTER_IMPLEMENTATION_TASK_LIST.md"
INBOX_REL = "MapDesign/MCP_INBOX/SV5_12_TREE_GRAB.md"
TASK_REL = "MapDesign/MCP/TASKS/SV5_12_TREE_GRAB.md"
ARCHIVE_REL = "MapDesign/MCP_ARCHIVE/SV5_12_TREE_GRAB.md"
RESULT_REL = "MapDesign/MCP/REPORTS/SV5_12_TREE_GRAB_RESULT.md"
GENERATED_REL = "MapDesign/MCP/GENERATED/SV5_12_TREE_GRAB"
...
def verify_package(package: Path, expected_sha: str, errors: list[str]) -> dict:
    ...
    try:
        ...
        return {}
    add(errors, manifest.get("schema") == "SV5_12_TREE_GRAB_FILES/v1", "manifest schema mismatch")
    seen: set[str] = set()
    ...
    required = {"README.md", "CONTRACT.md", "TREE_GRAB_PROFILE.json", "SOURCE_LOCK.json",
                "SV5_12_TREE_GRAB.md", "STAGE.py", "tools/check_tree_grab.py"}
...
def verify_git(project: Path, lock: dict, errors: list[str]) -> None:
    ...
    for entry in lock.get("files", []):
        ...
        try:
            ...
            continue
        if entry.get("git_blob_oid"):
            add(errors, oid == entry["git_blob_oid"], f"Git blob OID mismatch: {rel}")
        add(errors, digest(raw) == entry["sha256"], f"Git blob SHA mismatch: {rel}")
        if entry.get("bytes") is not None:
            add(errors, len(raw) == entry["bytes"], f"Git blob size mismatch: {rel}")


def counts(text: str) -> dict[str, int]:
    result = {"COMPLETE": 0, "CURRENT": 0, "LOCKED": 0}
...
def owned_overlap(project: Path, lock: dict, errors: list[str]) -> None:
    ...
    paths += [INBOX_REL, TASK_REL, ARCHIVE_REL, RESULT_REL, GENERATED_REL,
              ...
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5TreeGrab.cs.meta",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5TreeGrabExport.cs",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5TreeGrabExport.cs.meta",
              "Assets/_Game/Tests/EditMode/Map/SV5/Sv5TreeGrabTests.cs",
              "Assets/_Game/Tests/EditMode/Map/SV5/Sv5TreeGrabTests.cs.meta",
...
def verify_state(project: Path, package: Path, phase: str, errors: list[str]) -> dict:
    ...
    expected_counts = {"COMPLETE": 258, "CURRENT": 1 if applied else 0,
                       "LOCKED": 35 if applied else 36}
    add(errors, counts(status) == expected_counts, f"Status counts mismatch: {counts(status)}")
    add(errors, current(status) == (TASK if applied else "NONE"),
        f"Current Task mismatch: {current(status)}")
    add(errors, status.count("| SV5_11_FIX01 | COMPLETE |") == 1,
        "SV5_11_FIX01 predecessor row mismatch")
    add(errors, status.count(f"| {TASK} | {'CURRENT' if applied else 'LOCKED'} |") == 1,
        "SV5_12 row mismatch")
    add(errors, status.count("| SV5_13_JUMP_CONTRACT | LOCKED |") == 1,
        "SV5_13 must remain LOCKED")
    add(errors, master.count("| 12 | SV5_12_TREE_GRAB | LOCKED |") == 1,
        "SV5_12 Master membership mismatch")
    task_bytes = (package / "SV5_12_TREE_GRAB.md").read_bytes()
    if phase == "before":
        ...
        add(errors, candidates(project) == ["SV5_12_TREE_GRAB.md"], "inbox candidate set is not exactly one")
...
def atomic_write(path: Path, data: bytes) -> None:
    ...
    temp = path.with_name(path.name + ".SV5_12.tmp")
    if temp.exists():
...
def main() -> int:
    ...
    if args.stage:
        ...
        atomic_write(path, (package / "SV5_12_TREE_GRAB.md").read_bytes())
        post_errors: list[str] = []
        verify_state(project, package, "staged", post_errors)
        if post_errors:
            if path.exists() and path.read_bytes() == (package / "SV5_12_TREE_GRAB.md").read_bytes():
                path.unlink()
