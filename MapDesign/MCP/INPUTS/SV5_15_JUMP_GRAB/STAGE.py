...
BASE = "ad952b3929259c1968ffd62ef60c3b99b9bed42e"
STATUS_REL = "MapDesign/MCP/06_IMPLEMENTATION_STATUS.md"
MASTER_REL = "MapDesign/MCP/MASTER_IMPLEMENTATION_TASK_LIST.md"
INBOX_REL = "MapDesign/MCP_INBOX/SV5_15_JUMP_GRAB.md"
TASK_REL = "MapDesign/MCP/TASKS/SV5_15_JUMP_GRAB.md"
ARCHIVE_REL = "MapDesign/MCP_ARCHIVE/SV5_15_JUMP_GRAB.md"
RESULT_REL = "MapDesign/MCP/REPORTS/SV5_15_JUMP_GRAB_RESULT.md"
GENERATED_REL = "MapDesign/MCP/GENERATED/SV5_15_JUMP_GRAB"
...
def verify_package(package: Path, expected_sha: str, errors: list[str]) -> None:
    ...
    try:
        ...
        return
    add(errors, manifest.get("schema") == "SV5_15_JUMP_GRAB_FILES/v1",
    ...
    required = {"README.md", "CONTRACT.md", "JUMP_GRAB_PROFILE.json", "SOURCE_LOCK.json",
                "SV5_15_JUMP_GRAB.md", "STAGE.py", "tools/check_jump_grab.py"}
...
def verify_state(project: Path, package: Path, phase: str, errors: list[str]) -> dict:
    ...
    expected = {"COMPLETE": 262, "CURRENT": 1 if applied else 0,
                "LOCKED": 32 if applied else 33}
    ...
    add(errors, status.count("| SV5_14_JUMP_SOLID | COMPLETE |") == 1,
        "SV5_14_JUMP_SOLID predecessor row mismatch")
    add(errors, status.count(f"| {TASK} | {'CURRENT' if applied else 'LOCKED'} |") == 1,
        "SV5_15 row mismatch")
    add(errors, status.count("| SV5_16_JUMP_OUTLINE | LOCKED |") == 1,
        "SV5_16 must remain LOCKED")
    add(errors, master.count("| 15 | SV5_15_JUMP_GRAB | LOCKED |") == 1,
        "SV5_15 Master membership mismatch")
    task_bytes = (package / "SV5_15_JUMP_GRAB.md").read_bytes()
    if phase == "before":
        ...
        add(errors, candidates(project) == ["SV5_15_JUMP_GRAB.md"],
...
def owned_overlap(project: Path, lock: dict, errors: list[str]) -> None:
    ...
    paths += [INBOX_REL, TASK_REL, ARCHIVE_REL, RESULT_REL, GENERATED_REL,
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpGrabGeometry.cs",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpGrabGeometry.cs.meta",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpGrabExport.cs",
              "Assets/_Game/Map/Runtime/WorldGeneration/SectorPlanning/Sv5JumpGrabExport.cs.meta",
              "Assets/_Game/Tests/EditMode/Map/SV5/Sv5JumpGrabTests.cs",
              "Assets/_Game/Tests/EditMode/Map/SV5/Sv5JumpGrabTests.cs.meta",
              "MapDesign/MCP/SV5/23_JUMP_GRAB_V5.md"]
...
def atomic_write(path: Path, data: bytes) -> None:
    ...
    temp = path.with_name(path.name + ".SV5_15.tmp")
    if temp.exists():
...
def main() -> int:
    ...
    if args.stage:
        ...
        atomic_write(path, (package / "SV5_15_JUMP_GRAB.md").read_bytes())
        post_errors: list[str] = []
        verify_state(project, package, "staged", post_errors)
        if post_errors:
            if path.exists() and path.read_bytes() == (package / "SV5_15_JUMP_GRAB.md").read_bytes():
                path.unlink()
