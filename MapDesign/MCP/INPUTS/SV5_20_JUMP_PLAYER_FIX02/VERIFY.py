...
def same(path: Path, spec: dict) -> bool:
    return path.is_file() and path.stat().st_size == spec["bytes"] and digest(path.read_bytes()) == spec["sha256"]
...
def verify_package(package: Path, manifest_sha: str, errors: list[str]) -> None:
    ...
    try:
        ...
        return
    if manifest.get("schema") != "SV5_20_JUMP_PLAYER_FIX02_FILES/v1" or manifest.get("base_commit") != BASE:
    ...
    required = {"README.md", "AMENDMENT.md", "GEOMETRY_PATCH.json",
                ...
                "tools/check_jump_player_fix02.py"}
    errors.extend("required package entry not manifested: " + rel for rel in sorted(required - seen))
...
def main() -> int:
    ...
    if args.post_readonly:
        required_outputs = [
            "player_geometry_fix02.json", "player_geometry_patch.csv",
        ...
        for name in required_outputs:
            if not (out / name).is_file():
                errors.append("missing FIX02 output: " + name)
    ...
    if errors:
        ...
        return 1
    print(json.dumps({
        "status": "PASS_FIX02_POST_READONLY" if args.post_readonly else "PASS_FIX02_ACTIVE_TASK_PREFLIGHT",
        "task": TASK,
        "base_commit": BASE,
