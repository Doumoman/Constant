#!/usr/bin/env python3
"""Independent SV5_11 hub-shell checker with semantic task-local retired-grid checks."""
import argparse
import csv
import hashlib
import json
...
EXPORT_NAMES = ("sector_index.csv", "sector_pairs.csv")
SCHEMA_PATTERNS = (
    r"(?i)(^|_)sector(_|$)",
    r"(?i)sectorid",
    r"(?i)distinct_sectors",
    r"(?i)home_sector",
)
NEW_RUNTIME = (
...
def identifier_matches(csharp, label):
    code = strip_csharp_noncode(csharp)
    return [label + ":" + pattern for pattern in IDENTIFIER_PATTERNS if re.search(pattern, code)]


def runtime_export_matches(csharp, label):
    lowered = csharp.lower()
    return [label + ":" + name for name in EXPORT_NAMES if name in lowered]
...
def generated_schema_matches(generated_root):
    matches = []
    ...
    for profile in ("default", "repeat"):
        ...
        for path in root.rglob("*"):
            ...
            if path.name.lower() in EXPORT_NAMES:
                matches.append(rel + ":forbidden filename")
            if path.suffix.lower() == ".csv":
                with path.open("r", encoding="utf-8-sig", newline="") as handle:
                    fields = next(csv.reader(handle), [])
                for field in fields:
                    matches.extend(rel + ":header:" + pattern for pattern in SCHEMA_PATTERNS if re.search(pattern, field))
            elif path.suffix.lower() == ".json":
                for key in json_keys(load_json(path)):
                    matches.extend(rel + ":key:" + pattern for pattern in SCHEMA_PATTERNS if re.search(pattern, key))
    return matches


def check_retired_grid_dependencies(source_root, generated_root, allow_missing_new=False):
    head = run_git(source_root, "rev-parse", "HEAD").strip()
    need(head == BASE_COMMIT, "HEAD changed before SV5_11 Finalize: " + head)
    matches = []
    present_new = 0
    for rel in NEW_RUNTIME + NEW_TESTS:
        ...
        matches.extend(identifier_matches(text, rel))
        if rel in NEW_RUNTIME:
            matches.extend(runtime_export_matches(text, rel))
    ...
    matches.extend(identifier_matches(additions, "SV5_11_ADDED_RUNTIME_LINES"))
    matches.extend(runtime_export_matches(additions, "SV5_11_ADDED_RUNTIME_LINES"))
    matches.extend(generated_schema_matches(generated_root))
    need(not matches, "retired grid dependency introduced by SV5_11: " + ", ".join(matches[:8]))
    return present_new
...
def main():
    ...
    try:
        generated_root = args.default.parent if args.default is not None else None
        present_new = check_retired_grid_dependencies(
            args.source_root.resolve(),
            generated_root,
            allow_missing_new=args.preflight_only,
        )
        if args.preflight_only:
            print("PASS_CHECKER_SCOPE_PREFLIGHT_V2")
            print(json.dumps({"scope": "SV5_11_SEMANTIC_TASK_LOCAL", "present_new_owned_sources": present_new}, indent=2))
            return
        ...
        payload = {
            ...
            "checker_scope": "SV5_11_SEMANTIC_TASK_LOCAL",
