#!/usr/bin/env python3
"""Read-only verifier for SV5_11 CHECKER_SCOPE_FIX02."""
import argparse
import collections
import hashlib
import json
...
PREFIX = "MapDesign/MCP/INPUTS/SV5_11_HUB_SHELL/CORRECTIONS/CHECKER_SCOPE_FIX02"
INPUT = "MapDesign/MCP/INPUTS/SV5_11_HUB_SHELL"
...
def main():
    ...
    try:
        ...
        need(sha(manifest_bytes) == args.expected_manifest_sha256, "FIX02 manifest SHA mismatch")
        manifest = json.loads(manifest_bytes.decode("utf-8-sig"))
        need(manifest["schema"] == "SV5_11_CHECKER_SCOPE_FIX_PACKAGE_V2", "wrong correction package")
        for entry in manifest["files"]:
            data = safe(HERE, entry["path"]).read_bytes()
            need(len(data) == entry["bytes"] and sha(data) == entry["sha256"], "FIX02 bytes mismatch: " + entry["path"])

        head = git(root, "rev-parse", "HEAD").decode().strip()
        need(head == BASE, "HEAD changed: " + head)
        original_manifest = safe(root, INPUT + "/FILES.json").read_bytes()
        ...
        fix01_manifest = safe(root, INPUT + "/CORRECTIONS/CHECKER_SCOPE_FIX01/FILES.json").read_bytes()
        need(sha(fix01_manifest) == "82a6af754d1d305696f93c80c4cbf5b9e2bd3d553c09c67ab5f0fd58755d19cd", "FIX01 manifest changed")
        fix01_checker = safe(root, INPUT + "/CORRECTIONS/CHECKER_SCOPE_FIX01/check_hub_shell.py").read_bytes()
        need(sha(fix01_checker) == "f888065191268b63e5464244fd82ce7665f5b326d49e77247dd84e971ba15b16", "FIX01 checker changed")
        ...
        print("PASS_CHECKER_SCOPE_FIX02")
