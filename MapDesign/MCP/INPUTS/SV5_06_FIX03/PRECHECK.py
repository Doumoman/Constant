...
TASK='SV5_06_FIX03'; PREV='SV5_06_FIX02'
PREFIX='MapDesign/MCP/INPUTS/SV5_06_FIX03'
...
def phase_counts(spec):
 return {name:spec[name.lower()] for name in ['COMPLETE','CURRENT','LOCKED'] if spec[name.lower()]}
...
def state(root,reg,mode):
 ...
 if mode=='pre-registration':
  need(phase=='UNREGISTERED' and current=='NONE' and len(rows)==reg['before']['rows'] and counts==phase_counts(reg['before']),'Expected unregistered / Current NONE')
  need(digest(b)==reg['status']['before_sha256'] and digest(master)==reg['master']['before_sha256'],'Registration before bytes mismatch')
 else:
  need(len(rows)==reg['after_registration']['rows'] and digest(master)==reg['master']['after_sha256'],'Exact registered Master/row count required')
  n=len(re.findall(r'^\|[^\r\n]*\b'+TASK+r'\b[^\r\n]*\|\s*$',master.decode('utf-8-sig'),re.M))
  need(n==1,'Task must be registered once in Master')
  if mode in ['pre-stage','stage']:
   need(phase=='LOCKED' and current=='NONE' and counts==phase_counts(reg['after_registration']),'Expected registered LOCKED / NONE')
   ...
   expected=phase_counts(reg['after_apply'] if phase=='CURRENT' else reg['after_finalize'])
...
def local(root,lock,reg,mode):
 ...
 if not post:
  for rel in lock['expected_absent_before']:
   need(not safe(root,rel).exists(),'Unexpected pre-existing task output: '+rel)
 if mode=='pre-registration':
  need(not git(root,'diff','--cached','--name-only','--',STATUS,MASTER),'Existing staged registry change overlaps registration')
 if post:
 ...
 if post and phase=='COMPLETE':
  own=safe(root,'MapDesign/MCP/REPORTS/'+TASK+'_RESULT.md')
  need(own.is_file(),'Finalized task must have its own Result')
  result_text=own.read_text(encoding='utf-8-sig')
  need(re.search(r'^TASK: '+TASK+r'$',result_text,re.M) and re.search(r'^STATUS: PASS$',result_text,re.M),'Finalized Result must match own TASK and PASS')
