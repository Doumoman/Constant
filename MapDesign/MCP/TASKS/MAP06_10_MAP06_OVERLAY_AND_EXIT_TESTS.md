...
13. `REPORTS/MAP06_09_IMPLEMENT_OPTIONAL_REGION_VALIDATOR_RESULT.md`
...
STATUS: BLOCKED
MAP06_10: NOT COMPLETE
MAP06 PHASE EXIT: NOT APPROVED
MAP07_01_IMPLEMENT_MICROCHUNK_DEFINITION: LOCKED / DO NOT START
SHA-256: d02204b7515e4818052f6e5e8dad0fc0740803f3af5f0753f652b5c715e3119e
Blocked current Task SHA-256 before repair: 205ce60e1e591036a80bc7dc10a939ea95d0237d09babe106e86c09b78e70605
```
...
이 repair patch가 적용된 뒤에만 MAP06_10을 재실행한다. MAP07 이후 Task body는 읽거나 시작하지 않는다.

## Repair v1.1 — Editor Preview Directory Allowlist

Original Task v1.0 listed this exact Editor preview target:

```text
Assets/_Game/Editor/MapAuthoring/Preview/OptionalRegionOverlaySceneDrawer.cs
```

The approved project did not already contain `Assets/_Game/Editor/MapAuthoring/Preview/` or a predecessor production drawer at `Assets/_Game/Editor/MapAuthoring/Preview/MandatoryRouteOverlaySceneDrawer.cs`. Creating the new canonical drawer therefore requires exactly one new Unity folder meta:

```text
Assets/_Game/Editor/MapAuthoring/Preview.meta
```

This repair keeps the canonical output path, explicitly allows creating that one folder and `.meta`, removes the nonexistent predecessor drawer from the required read allowlist, and updates Assets meta from `3311 -> 3323`. No other directory/folder meta may be created.
...
No predecessor production drawer is required for this repair because `Assets/_Game/Editor/MapAuthoring/Preview/MandatoryRouteOverlaySceneDrawer.cs` was proven absent by the BLOCKED Result. Use existing runtime diagnostics and tests as the allowed implementation reference.
...
### 신규 Editor preview directory/folder meta — exact 1
...
신규 C# 11개와 matching `.cs.meta` 11개, 신규 Editor preview folder `.meta` 1개, 위 boundary test C# 최대 15개 수정, Result 1개만 허용한다. MAP05/MAP06_01~09 production source, world/site/biome/mandatory/Type0/access/reward/return/inactive/validation artifacts, Authoring/generated CSV/meta, asmdef, Scene, Prefab, Packages, ProjectSettings는 수정하지 않는다. 위 exact Editor preview directory/folder meta 외 신규 directory/folder meta/asmdef/asmref를 만들지 않는다.
