...
- line ending/UTF-8/BOM은 원본에서 보존하고, missing-BOM case만 leading UTF-8 BOM `EF BB BF`를 제거한다.
...
7. `MISSING_UTF8_BOM` — BOM-required static source 한 개에서 leading UTF-8 BOM을 제거한다.
...
- duplicate/enum/number/FK/header cases와 BOM 제거 case는 `published=false`, exact stage/code/source tuple, error count와 navigation line을 검증한다. BOM 제거 case의 exact pipeline code는 `MISSING_UTF8_BOM`이다.
...
- duplicate PK, invalid ENUM/INT/FLOAT, missing single/list FK, missing UTF-8 BOM/header diagnostics
...
## BLOCKED Remediation v1.1 — BOM Fixture Direction

v1.0 실행 전 진단에서 actual Authoring CSV 50/50이 모두 leading UTF-8 BOM을 가지며 production pipeline이 이를 필수로 요구함을 확인했다. v1.0의 `UNEXPECTED_UTF8_BOM` 요구는 폐기하고 위 `MISSING_UTF8_BOM` 계약으로 교체한다.

- production RFC4180 reader/pipeline/error code는 변경하지 않는다.
- double BOM, second BOM code point, header corruption으로 대체하지 않는다.
- factory는 temp copy의 exact 첫 `EF BB BF` 3 bytes만 제거하고 나머지 bytes를 보존한다.
- expected result는 `HadUtf8Bom=false`, pipeline issue code `MISSING_UTF8_BOM`, `published=false`, previous Registry identity/version/hash unchanged다.
- actual Authoring CSV/meta 50/50은 before/after byte/hash unchanged여야 한다.
- 나머지 9 fixture와 전체 verification/Result/finalize 계약은 v1.0 그대로 실행한다.
- Result에는 `REMEDIATION v1.1` 절로 BOM authority, exact mutation bytes, diagnostic tuple과 test evidence를 기록한다.
