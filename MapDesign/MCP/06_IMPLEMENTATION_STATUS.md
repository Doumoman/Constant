...
```text
MAP06_10_MAP06_OVERLAY_AND_EXIT_TESTS
```

## Last Result

```text
REPORTS/MAP06_10_MAP06_OVERLAY_AND_EXIT_TESTS_RESULT.md
STATUS: PASS
...
- MAP06_10 optional overlay: cells `169`, exclusive kinds `44/8/39/26/52`, connections attachment/return `12/19`, legend `15`, canonical digest `9cbd3833bf5e8b771f46cc3442c1c60d63493d8ffb7e8aa3c9a701f6a097fefd`
- MAP06_10 tests: overlay/drawer/exit `180/40/180`, MAP06_01~09 `2346/2346`, MAP05 aggregate `1959/1959`, actually executed required `4705/4705`, failed/skipped `0/0`
- MAP06_10 asset/static baseline: new C#/meta `11/11`, Preview folder meta `1/1`, Assets meta `3323`, Authoring CSV/meta `50/50`, duplicate GUID `0`, Scene/Prefab/CSV/asmdef changes `0`
- MAP06 PHASE EXIT: `APPROVED`; MAP07_01 remains `LOCKED / DO NOT START`

- MAP04 EXIT: `APPROVED`; MAP05 entry: `ELIGIBLE FOR SEPARATE PATCH`

## Current Rule

현재 Current Task는 `NONE`이다. MAP06_10은 PASS 및 COMPLETE로 finalize됐고 MAP06 phase exit는 APPROVED다. MAP07 이후 Task body는 읽거나 생성하거나 실행하지 않는다.

MAP06 optional overlay와 phase exit tests가 완료됐다. 다음 작업은 새 MCP_INBOX patch가 열기 전까지 시작하지 않는다.

Overlay는 access color, depth, attachment/contact, return witness, reward tier, inactive kind, validation issue 상태를 표시한다. Generated CSV writer, boundary profile/recipe/microchunk/tile/socket/edge, MAP07 microchunk authoring은 만들지 않았다. MAP06_10 symbols만 허용하고 MAP07+ future symbols는 계속 금지한다.

Mandatory route graph 기준은 고정이다: graph nodes/directed/undirected/route cells `47/96/48/47`, mask counts `20/4/4/17/0/0/2`, validation `12/12/0/0/0`. MAP05 Type4 규칙은 U+D mandatory, L/R actual adjacency preserved, UD/LUD/RUD/LRUD all legal이다. Type0/access/reward/return assignments와 Authoring/generated CSV도 변경하지 않는다.

`MAP07_01_IMPLEMENT_MICROCHUNK_DEFINITION`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
