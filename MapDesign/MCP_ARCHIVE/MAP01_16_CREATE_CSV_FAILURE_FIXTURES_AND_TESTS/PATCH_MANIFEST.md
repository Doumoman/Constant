...
```yaml
patch_id: MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS
patch_version: 1.0
requires_status:
  current_task: NONE
  task_states:
...
- 205개 상태 행에서 MAP00_01~10과 MAP01_01~15는 `COMPLETE`, MAP01_16 이후는 `LOCKED`다.
- MAP01_15 Result는 exact `STATUS: PASS`, window 48/48, targeted 764/764, full EditMode 784/784, compile/warning 0/0, actual published version 1이다.
- Authoring CSV/meta 50/50과 production pipeline은 MAP01_15에서 보존됐다.
- 새 Master는 MAP01_15 COMPLETE, MAP01_16 exact next를 기록한다.
- 새 status는 `25 COMPLETE / MAP01_16 CURRENT / 179 LOCKED`다.
