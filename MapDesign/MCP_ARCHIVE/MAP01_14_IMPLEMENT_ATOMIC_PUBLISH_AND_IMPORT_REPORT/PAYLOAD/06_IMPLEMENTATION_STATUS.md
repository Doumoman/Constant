...
```text
TASKS/MAP01_14_IMPLEMENT_ATOMIC_PUBLISH_AND_IMPORT_REPORT.md
```
...
| MAP01_14_IMPLEMENT_ATOMIC_PUBLISH_AND_IMPORT_REPORT | CURRENT |
...
```text
REPORTS/MAP01_13_IMPLEMENT_CONTENT_VERSION_HASH_RESULT.md
STATUS: PASS
```
...
현재는 candidate Registry+hash를 하나의 immutable snapshot으로 원자적 publish하고, issue가 하나라도 있으면 last-good snapshot을 보존하며 전체 issue를 `CsvImportReport.json` contract로 직렬화한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_14_IMPLEMENT_ATOMIC_PUBLISH_AND_IMPORT_REPORT` 하나다.

Editor import window, file picker/watcher, CSV pipeline orchestration, domain validation은 구현하지 않는다. 기존 CSV·loader·definitions·FK/Registry/hash·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_14가 PASS해도 `MAP01_15_CREATE_CSV_IMPORT_WINDOW`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_14를 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
