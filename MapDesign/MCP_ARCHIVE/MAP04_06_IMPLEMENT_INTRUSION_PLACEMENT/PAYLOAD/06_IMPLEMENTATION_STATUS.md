...
```text
TASKS/MAP04_06_IMPLEMENT_INTRUSION_PLACEMENT.md
```
...
| MAP04_06_IMPLEMENT_INTRUSION_PLACEMENT | CURRENT |
...
```text
MAP04_05_IMPLEMENT_MULTI_SEED_BIOME_GROWER
```

## Last Result

```text
REPORTS/MAP04_05_IMPLEMENT_MULTI_SEED_BIOME_GROWER_RESULT.md
STATUS: PASS
```
...
현재는 MAP04_05의 viable immutable grown P02 snapshot에서 active Root/Mill Intrusion count를 continued current-attempt `RNG_BIOME_PATCH`로 count-first 추첨한다. active boundary pair가 `BOUND_TUNNEL/TUNNEL_INTRUSION`을 허용하는 actual inter-biome cardinal boundary에서만, protected seed/site cell이 아니고 donor patch의 minimum·connectivity를 유지하는 sector를 one-cell Intrusion patch로 atomic transfer한다. source result/publication/P01/P02는 mutate하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP04_06_IMPLEMENT_INTRUSION_PLACEMENT` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime/Test `Generation` 폴더에 Intrusion placement production C# 7개, focused test C# 1개와 matching meta 8개만 추가한다. MAP04_01~05 existing files는 수정하지 않는다. cleanup, serializer/export, final validator, overlay, pass/root adapter는 만들지 않는다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP04_06이 PASS하면 STATUS FINALIZE는 MAP04_06만 COMPLETE로 바꾸며 Current Task를 NONE으로 만든 뒤 종료한다. `MAP04_07_IMPLEMENT_PATCH_CLEANUP`은 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
