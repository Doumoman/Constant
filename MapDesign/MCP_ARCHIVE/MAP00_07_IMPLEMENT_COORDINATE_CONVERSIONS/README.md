# MCP PATCH — MAP00_07 IMPLEMENT COORDINATE CONVERSIONS v1.0

## 목적

광역 WorldGeneration의 네 좌표 공간을 `WorldCoordinateUtility` 단일 진입점에서 검증하고 상호 변환한다.

이번 패치는 Runtime 좌표 변환 C# 1개와 변환 API 계약 EditMode test 1개만 만든다. 모든 169×16 경계를 훑는 exhaustive test, debug view, CSV 또는 생성 알고리즘은 만들지 않는다.
...
```text
MapDesign/MCP_INBOX/MAP00_07_IMPLEMENT_COORDINATE_CONVERSIONS/PATCH_MANIFEST.md
```
...
```text
PATCH APPLY
→ MAP00_07 TASK EXECUTION
→ REPORTS/MAP00_07_IMPLEMENT_COORDINATE_CONVERSIONS_RESULT.md 생성
→ STATUS FINALIZE
→ MAP00_07 COMPLETE
→ Current Task NONE
→ STOP
