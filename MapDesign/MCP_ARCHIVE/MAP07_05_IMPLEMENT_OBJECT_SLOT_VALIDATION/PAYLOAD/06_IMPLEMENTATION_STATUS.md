...
```text
TASKS/MAP07_05_IMPLEMENT_OBJECT_SLOT_VALIDATION.md
```
...
| MAP07_05_IMPLEMENT_OBJECT_SLOT_VALIDATION | CURRENT |
...
```text
REPORTS/MAP07_04_IMPLEMENT_SOCKET_EDGE_VALIDATION_RESULT.md
STATUS: PASS
...
현재는 MAP07_05_IMPLEMENT_OBJECT_SLOT_VALIDATION 하나만 CURRENT다. MAP07_04는 STATUS `PASS`, SHA-256 `90bb39103282ad08d031ee710802abdeba0adc4799c754ba73eaede4a2b7ade5`로 검수됐고 microchunk socket-edge validation layer가 완료됐다.

이번 Task는 object slot의 anchor coordinate, category, pool compatibility, required/orientation/visible flag, required marker, blocking layer 배치 금지, forbidden-radius 안전 반경을 검증하는 deterministic object-slot validator까지만 수행한다. `allowed_pool_id`는 validator에 공급된 pool catalog와 category compatibility를 대조하되 spawn population이나 item selection을 수행하지 않는다.

Standalone 96-cell validator pass, reachability probe, authoring grid/window, socket/slot editor UI, CSV import/export, preview/report는 MAP07_06~MAP07_13의 소유다. MAP07_05에서 구현하거나 선행 생성하지 않는다.
...
MAP07_05가 PASS하면 MAP07_05만 COMPLETE, Current Task NONE으로 finalize한다. `MAP07_06_IMPLEMENT_96_CELL_VALIDATOR`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
