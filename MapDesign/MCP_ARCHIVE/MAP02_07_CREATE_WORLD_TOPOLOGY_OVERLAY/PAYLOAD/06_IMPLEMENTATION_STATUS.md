...
```text
TASKS/MAP02_07_CREATE_WORLD_TOPOLOGY_OVERLAY.md
```
...
| MAP02_07_CREATE_WORLD_TOPOLOGY_OVERLAY | CURRENT |
...
```text
MAP02_06_IMPLEMENT_SEED_MANIFEST_AND_REPLAY_RECORDER
```

## Last Result

```text
REPORTS/MAP02_06_IMPLEMENT_SEED_MANIFEST_AND_REPLAY_RECORDER_RESULT.md
STATUS: PASS
```
...
현재는 MAP02의 일곱 번째 Task에서 immutable `GridInitializationResult`를 169-cell topology overlay snapshot으로 투영한다. x는 왼쪽→오른쪽, y는 아래→위 방향을 유지하며 모든 cell에 좌표와 Role glyph를 표시하고, hover에는 월드 타일 범위와 exact L/R/U/D 이웃 index를 표시한다. 같은 renderer를 Game View와 Scene View가 공유한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP02_07_CREATE_WORLD_TOPOLOGY_OVERLAY` 하나다.

실제 Authoring CSV/meta, 기존 production·tests, asmdef, Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. MAP02_07에서는 existing approved Runtime `Diagnostics`, Editor `Preview`, Runtime test `Generation`, Editor test `Preview` 폴더에 C# `4/1/1/1`만 추가한다. matching meta 7개 외 meta를 바꾸지 않으며 기존 legacy Editor folder meta 6개는 baseline으로 보존한다. 신규 directory/folder meta는 만들지 않는다.

MAP02_07이 PASS해도 `MAP02_08_MAP02_EXIT_TESTS`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP02_07을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
