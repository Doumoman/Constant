...
```text
TASKS/MAP07_01_IMPLEMENT_MICROCHUNK_DEFINITION.md
```
...
| MAP07_01_IMPLEMENT_MICROCHUNK_DEFINITION | CURRENT |
...
현재는 MAP07_01_IMPLEMENT_MICROCHUNK_DEFINITION 하나만 CURRENT다. MAP06_10은 STATUS `PASS`, SHA-256 `690a7cef9dbf1d22416e38b3675d76b0ef758062de2425e8e4841381f0d9bdeb`로 검수됐고 `MAP06 PHASE EXIT: APPROVED`가 확정됐다.

이번 Task는 12x8 microchunk를 표현하는 immutable runtime definition model과 구조 테스트까지만 수행한다. Microchunk local coordinate, ID, catalog metadata, 8개 tile layer cell record, socket definition, object slot definition, complete 96-cell collection 계약을 만든다.

Tile layer collision rule, transform execution, socket edge compatibility, object slot semantic validation, 96-cell validator pass, reachability probe, authoring window, socket/slot editor, CSV import/export, preview/report는 MAP07_02~MAP07_13의 소유다. MAP07_01에서 구현하거나 선행 생성하지 않는다.

Mandatory route graph와 MAP06 optional-region source chain은 read-only baseline이다. Authoring CSV/meta `50/50`, Authoring manifest `4ffef6dbbea5151889d1c9114a500eba6cb54828ba47c9de508bad95dddc4ac3`, generated CSV `0`, boundary/recipe/sector assembly artifacts `0`을 유지한다.

MAP07_01이 PASS하면 MAP07_01만 COMPLETE, Current Task NONE으로 finalize한다. `MAP07_02_IMPLEMENT_TILE_LAYER_RULES`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
