...
```text
TASKS/MAP05_08_BUILD_MANDATORY_ROUTE_GRAPH.md
```
...
| MAP05_08_BUILD_MANDATORY_ROUTE_GRAPH | CURRENT |
...
```text
MAP05_07_ADD_MANDATORY_ROUTE_LOOPS
```

## Last Result

```text
REPORTS/MAP05_07_ADD_MANDATORY_ROUTE_LOOPS_RESULT.md
STATUS: PASS
```
...
현재는 MAP05_07의 loop plan까지 포함한 P03 입력에서 최종 mandatory route graph를 deterministic하게 만든다. Type4의 네 수평 조합(U+D/L+U+D/R+U+D/L+R+U+D)은 모두 허용하며 U+D는 필수, L/R은 실제 graph adjacency 보존이다. Authoring CSV schema/body는 수정하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP05_08_BUILD_MANDATORY_ROUTE_GRAPH` 하나다.

실제 Authoring CSV/meta와 asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. approved Runtime/Test `Generation`에 graph production C# 13개, focused test 1개와 matching meta 14개를 추가하고, route stamp에 필요한 existing production 최대 3개와 prior negative-audit test 7개만 제한적으로 수정한다. generated CSV는 deterministic byte[]로만 만든다.

MAP05_08이 PASS하면 MAP05_08만 COMPLETE, Current Task NONE으로 finalize한다. `MAP05_09_IMPLEMENT_MANDATORY_ROUTE_VALIDATOR`는 LOCKED로 유지한다.
