...
```text
TASKS/MAP03_01_IMPLEMENT_SITE_RESERVATION_MODELS.md
```
...
| MAP03_01_IMPLEMENT_SITE_RESERVATION_MODELS | CURRENT |
...
현재는 MAP03의 첫 번째 Task에서 P01 Site Reservation이 사용할 immutable 데이터 계약만 만든다. footprint, final-oriented local cell, entry anchor, typed reservation ID, per-sector reservation, `CoreBiomeSeed`, complete reservation snapshot을 구현하되 후보 열거·transform 배치·거리·비용·backtracking·capacity flood·village 선택·PASS_SITE 실행은 아직 구현하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP03_01_IMPLEMENT_SITE_RESERVATION_MODELS` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime/Test `Generation` 폴더에 신규 production C# 8개, focused test C# 1개와 matching meta 9개만 추가한다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP03_01이 PASS하면 STATUS FINALIZE는 MAP03_01만 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다. `MAP03_02_ENUMERATE_START_AND_SITE_CANDIDATES`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
