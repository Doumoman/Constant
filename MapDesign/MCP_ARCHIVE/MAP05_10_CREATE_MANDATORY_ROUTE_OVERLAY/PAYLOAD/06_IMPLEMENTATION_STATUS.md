...
```text
TASKS/MAP05_10_CREATE_MANDATORY_ROUTE_OVERLAY.md
```
...
| MAP05_10_CREATE_MANDATORY_ROUTE_OVERLAY | CURRENT |
...
```text
MAP05_09_VALIDATE_MANDATORY_ROUTE_GRAPH
```

## Last Result

```text
REPORTS/MAP05_09_VALIDATE_MANDATORY_ROUTE_GRAPH_RESULT.md
STATUS: PASS
```
...
현재는 MAP05_09에서 검증 완료된 mandatory route graph와 validation report를 Game/Scene overlay로 표시한다. Type4의 네 수평 조합(UD/LUD/RUD/LRUD)은 모두 표시 대상이며 U+D는 필수, L/R은 실제 graph adjacency 보존이다. Graph/CSV/SectorCell은 수정하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP05_10_CREATE_MANDATORY_ROUTE_OVERLAY` 하나다.

실제 production graph/CSV/SectorCell/Authoring CSV/meta와 asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. approved runtime `Diagnostics`와 editor `Preview`에 overlay production C# 5개, focused tests 2개와 matching meta 7개만 추가한다.

MAP05_10이 PASS하면 MAP05_10만 COMPLETE, Current Task NONE으로 finalize한다. `MAP05_11_MAP05_BATCH_AND_EXIT_TESTS`는 LOCKED로 유지한다.
