...
```text
TASKS/MAP03_03_IMPLEMENT_FOOTPRINT_PLACEMENT_SOLVER.md
```
...
| MAP03_03_IMPLEMENT_FOOTPRINT_PLACEMENT_SOLVER | CURRENT |
...
현재는 MAP03의 세 번째 Task에서 raw origin과 source footprint/entry definitions를 `R0/MIRROR_X/MIRROR_Y/R180`로 변환하고 world-bound·footprint overlap·entry 접근 충돌을 판정한다. Start는 synthetic 1×1 R0, special sites는 네 transform을 평가하며 거리·비용·RNG·선택·backtracking은 수행하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP03_03_IMPLEMENT_FOOTPRINT_PLACEMENT_SOLVER` 하나다.
...
MAP03_03이 PASS하면 STATUS FINALIZE는 MAP03_03만 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다. `MAP03_04_IMPLEMENT_SITE_DISTANCE_INDEX`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
