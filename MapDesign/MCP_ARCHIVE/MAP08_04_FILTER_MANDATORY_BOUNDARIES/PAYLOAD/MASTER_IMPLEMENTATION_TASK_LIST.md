...
```text
완료: MAP00_01 ~ MAP00_10, MAP01_01 ~ MAP01_17, MAP02_01 ~ MAP02_08, MAP03_01 ~ MAP03_11, MAP04_01 ~ MAP04_11, MAP05_01 ~ MAP05_11, MAP06_01 ~ MAP06_10, MAP07_01 ~ MAP07_13, MAP08_01 ~ MAP08_03
실제 다음: MAP08_04_FILTER_MANDATORY_BOUNDARIES
...
MAP08 Phase: 3 COMPLETE / MAP08_04 CURRENT / 10 LOCKED
Auto Start: NO
```
...
| MAP08 | 14 | 3 COMPLETE / 1 CURRENT / 10 LOCKED |
| MAP09 | 13 | LOCKED |
| MAP10 | 12 | LOCKED |
| MAP11 | 14 | LOCKED |
| MAP12 | 14 | LOCKED |
...
| MAP14 | 13 | LOCKED |
| MAP15 | 18 | LOCKED |
| **합계** | **205** | **94 COMPLETE / 1 CURRENT / 110 LOCKED** |
...
```text
NEXT  : MAP08_04_FILTER_MANDATORY_BOUNDARIES
THEN  : MAP08_05_IMPLEMENT_BOUNDARY_WARNING_CONTRACT (locked until MAP08_04 PASS)
LOCKED: MAP08_05 이후 전부
NO RUN: MAP08_03 및 이전 완료 Task 패키지
```

`MAP08_03_IMPLEMENT_BOUNDARY_CHUNK_RESOLVER` Result는 STATUS `PASS`, SHA-256 `43a6d29466996164af4cc8e2d09dd6478a013f95c0b40ad15f132b3bead01445`로 검수했다. Resolver request fields, forward/reverse direction, reverse transform policy, positive-weight deterministic selection, zero-weight fallback, no-candidate failure가 통과했다. MAP07 required total `5422/5422`, MAP06 required total `2746/2746`, MAP05 required total `1959/1959`, all required executions `11787/11787`, compile/Console/warning `0/0/0`, Assets meta `3439`, Authoring CSV/meta `50/50`, generated CSV `0`, Scene/Prefab/asmdef/ProjectSettings changes `0/0/0/0`이다.

`MAP08_04_FILTER_MANDATORY_BOUNDARIES`는 이 패치에서 CURRENT로 열린다. 이 Task의 범위는 mandatory route boundary 요청에서 `tool_requirement=NONE`이고 `mandatory_route_allowed=true`인 후보만 resolver 입력으로 남기는 deterministic filter 구현이다. `MAP08_05_IMPLEMENT_BOUNDARY_WARNING_CONTRACT`와 이후는 별도 PASS Result 전까지 열지 않는다.
