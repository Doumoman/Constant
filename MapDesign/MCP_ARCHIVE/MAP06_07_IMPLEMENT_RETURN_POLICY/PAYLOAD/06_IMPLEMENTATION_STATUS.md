...
```text
TASKS/MAP06_07_IMPLEMENT_RETURN_POLICY.md
```
...
| MAP06_07_IMPLEMENT_RETURN_POLICY | CURRENT |
...
```text
MAP06_06_CALCULATE_OPTIONAL_REWARD_TIER
```

## Last Result

```text
REPORTS/MAP06_06_CALCULATE_OPTIONAL_REWARD_TIER_RESULT.md
STATUS: PASS
...
현재는 MAP06_07_IMPLEMENT_RETURN_POLICY 하나만 CURRENT다. MAP06_06은 PASS 및 COMPLETE 확정 기준으로만 상태 전환됐다. MAP06_08 이후 Task body는 읽거나 생성하거나 실행하지 않는다.

이번 Task는 MAP06_04 Type0, MAP06_05 access/clue, MAP06_06 reward-tier source-chain을 결합해 모든 optional cell이 reciprocal internal BaseEdge로 attachment에 복귀 가능한지 증명하고 existing `OptionalReturnPolicy.BacktrackToAttachment`와 canonical critical witness를 기록하는 것까지만 수행한다.

approved fixture는 모든 `RequiresReturnConnection=false`이므로 synthetic ReturnGate/SafeExit/device/socket/edge/recipe를 만들지 않는다. 같은 opened/discovered optional attachment boundary를 역방향으로 사용하며 attachment base mask는 closed를 유지한다. MAP06_07 symbols만 허용하고 MAP06_08+ future symbols는 계속 금지한다.

Mandatory route graph 기준은 고정이다: graph nodes/directed/undirected/route cells `47/96/48/47`, mask counts `20/4/4/17/0/0/2`, validation `12/12/0/0/0`. MAP05 Type4 규칙은 U+D mandatory, L/R actual adjacency preserved, UD/LUD/RUD/LRUD all legal이다. Type0/access/reward assignments와 Authoring/generated CSV도 변경하지 않는다.

MAP06_07이 PASS하면 MAP06_07만 COMPLETE, Current Task NONE으로 finalize한다. `MAP06_08_ASSIGN_INACTIVE_BUFFERS`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
