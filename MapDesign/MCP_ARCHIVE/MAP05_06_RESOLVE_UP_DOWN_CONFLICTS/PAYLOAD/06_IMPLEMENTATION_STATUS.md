...
```text
TASKS/MAP05_06_RESOLVE_UP_DOWN_CONFLICTS.md
```
...
| MAP05_06_RESOLVE_UP_DOWN_CONFLICTS | CURRENT |
...
현재는 MAP05_05 vertical gateway plan에서 Type4로 표현할 수 없는 U/D 충돌만 별도 resolution pair로 deterministic하게 분리한다. Type4의 네 수평 조합(U+D/L+U+D/R+U+D/L+R+U+D)은 모두 허용하며 U+D는 필수, L/R은 실제 상태 보존이다. 기존 MAP05_02 lookup과 MAP05_05 plan은 변경하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP05_06_RESOLVE_UP_DOWN_CONFLICTS` 하나다.

실제 Authoring/generated CSV와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. approved Runtime/Test `Generation`에 production C# 8개, focused test 1개와 matching meta 9개를 추가하고, prior negative-audit test 4개만 MAP05_06 output을 허용하도록 전환한다. Type4는 U+D mandatory/L/R independent semantic으로 보존하며 loop/graph/validator/overlay는 시작하지 않는다.

MAP05_06가 PASS하면 MAP05_06만 COMPLETE, Current Task NONE으로 finalize한다. `MAP05_07_ADD_MANDATORY_ROUTE_LOOPS`는 LOCKED로 유지한다.
