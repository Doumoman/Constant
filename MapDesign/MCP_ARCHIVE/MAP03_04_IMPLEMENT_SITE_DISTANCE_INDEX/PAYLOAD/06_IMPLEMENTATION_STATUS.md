...
```text
TASKS/MAP03_04_IMPLEMENT_SITE_DISTANCE_INDEX.md
```
...
| MAP03_04_IMPLEMENT_SITE_DISTANCE_INDEX | CURRENT |
...
현재는 MAP03의 네 번째 Task에서 immutable footprint placements의 P00 4-neighbor sector-graph 최소 거리를 footprint-aware하게 index한다. exact required-site policy는 typed definition의 Start 거리와 special-site 상호 거리 값을 사용하며 비용·RNG·후보 선택·backtracking·Core 용량·Village bucket은 수행하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP03_04_IMPLEMENT_SITE_DISTANCE_INDEX` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime/Test `Generation` 폴더에 신규 production C# 7개, focused test C# 1개와 matching meta 8개만 추가한다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP03_04가 PASS하면 STATUS FINALIZE는 MAP03_04만 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다. `MAP03_05_IMPLEMENT_SITE_CANDIDATE_COST`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
