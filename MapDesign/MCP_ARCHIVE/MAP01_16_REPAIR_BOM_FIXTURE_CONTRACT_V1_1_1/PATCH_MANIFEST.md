...
```yaml
patch_id: MAP01_16_REPAIR_BOM_FIXTURE_CONTRACT_V1_1_1
patch_version: 1.1.1
requires_status:
  current_task: TASKS/MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS.md
  task_states:
    ...
    MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS: CURRENT
...
requires_result:
  path: MapDesign/MCP/REPORTS/MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS_RESULT.md
...
requires_project_baseline:
  ...
  microchunk_population_item_definition_tests: 77/77
  ...
  targeted_editmode: 764/764
  full_editmode: 784/784
  compile_errors: 0
...
forbids_started_task_prefixes:
  ...
  - MAP14
  - MAP15
sets_current_task: TASKS/MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS.md
copy_operations:
  - source: PAYLOAD/TASKS/MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS.md
    destination: MapDesign/MCP/TASKS/MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS.md
    mode: replace_if_sha256_matches
    required_existing_sha256: 1e295a82ff4b7d622921f7bbf25f4580fe1e64cb55669d4201da68125f516f25
...
- 적용 전/후 Current Task는 `TASKS/MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS.md`다.
- exact state는 `25 COMPLETE / MAP01_16 CURRENT / 179 LOCKED`이며 Master/Status는 변경하지 않는다.
- MAP01_16 Result는 exact `STATUS: BLOCKED`; production/test 신규 파일은 0이고 실제 CSV 50/50 모두 UTF-8 BOM이다.
- 실제 설치된 Current Task SHA-256 `1e295a82ff4b7d622921f7bbf25f4580fe1e64cb55669d4201da68125f516f25`가 manifest와 일치할 때만 v1.1 payload로 교체한다. 다르면 `BLOCKED`다.
- patch apply 중 Assets, CSV/meta, C#, tests, asmdef, Master, Status, Result를 변경하지 않는다.
- MAP01_17은 계속 LOCKED이며 자동 시작하지 않는다.
- Current Task와 Task 파일이 정확히 일치한다.
- PATCH APPLY 단계에서는 Assets, CSV, C#, test, asmdef를 변경하지 않는다.

Task destination은 exact current v1.0 SHA-256과 일치할 때만 교체한다.
