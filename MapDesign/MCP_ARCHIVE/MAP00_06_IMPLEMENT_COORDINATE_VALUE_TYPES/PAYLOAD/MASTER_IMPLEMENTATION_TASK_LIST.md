...
```text
완료: MAP00_01 ~ MAP00_05
실제 다음: MAP00_06_IMPLEMENT_COORDINATE_VALUE_TYPES
MAP00 Phase: 아직 미완료
MAP00_05 Result: PASS 검수 완료
MAP00_06 패치: PACKAGED / RESULT 대기
MAP01_01 기존 패키지: HOLD / 실행 금지
Auto Start: NO
```

MAP00_01~05는 프로젝트 감사, 폴더·assembly 경계 계획, 36개 폴더 생성, 아키텍처 테스트 생성, `WorldGenConstants` 구현까지 완료했다. 그러나 원본 MAP00 완료 조건에 포함된 다음 항목은 아직 구현되지 않았다.

- `WorldTileCoord`, `SectorCoord`, `MicroChunkCoord`, `LocalTileCoord`
- World/Sector/MicroChunk/Local 좌표 변환
- 경계값·왕복 변환 EditMode 테스트
- 좌표 디버그 표시

따라서 MAP01로 진입하기 전에 MAP00_06~10을 먼저 완료한다. 이미 만든 `MAP01_01_INSTALL_CSV_AUTHORING_BASELINE` ZIP은 실행하지 않고 보류하며, MAP00_10 PASS 후 최신 프로젝트 상태에 맞춰 다시 검증·발행한다.
...
| Phase | Task 수 | 현재 상태 |
|---|---:|---|
| MAP00 | 10 | 5 COMPLETE / 5 REMAINING |
| MAP01 | 17 | LOCKED |
| MAP02 | 8 | LOCKED |
| MAP03 | 11 | LOCKED |
| MAP04 | 11 | LOCKED |
...
| MAP14 | 13 | LOCKED |
| MAP15 | 18 | LOCKED |
| **합계** | **205** | **5 COMPLETE / 200 REMAINING** |
...
- [ ] `MAP00_06_IMPLEMENT_COORDINATE_VALUE_TYPES` — WorldTile/Sector/MicroChunk/LocalTile 좌표 readonly 값 타입과 equality/hash/string을 구현한다.
- [ ] `MAP00_07_IMPLEMENT_COORDINATE_CONVERSIONS` — 좌표 왕복 변환, 범위 검사, `TryCreate`, 월드 밖 입력 거부를 구현한다.
- [ ] `MAP00_08_CREATE_COORDINATE_TESTS` — 네 모서리, 모든 169×16 청크 경계, invalid 좌표, 왕복 변환을 EditMode로 검증한다.
- [ ] `MAP00_09_CREATE_COORDINATE_DEBUG_VIEW` — Scene/Editor에서 마우스 위치의 World/Sector/MicroChunk/Local 좌표를 동시에 표시한다.
- [ ] `MAP00_10_MAP00_EXIT_AUDIT` — compile/test/assembly/magic-number/Legacy dependency를 최종 감사하고 MAP01 진입을 승인한다.
...
- [ ] `MAP01_01_INSTALL_CSV_AUTHORING_BASELINE` — 정적 CSV 49개와 데이터 사전 1개를 확정 Authoring 경로에 바이트 그대로 설치한다.
- [ ] `MAP01_02_IMPLEMENT_CSV_SCHEMA_CATALOG` — `CSV_DATA_DICTIONARY.csv`에서 파일·열·타입·필수·PK·FK·default 계약을 읽는다.
- [ ] `MAP01_03_IMPLEMENT_RFC4180_READER` — quoted field, comma, CRLF/LF, escaped quote, multiline, UTF-8 BOM을 위치 정보와 함께 읽는다.
- [ ] `MAP01_04_IMPLEMENT_HEADER_AND_FIELD_VALIDATION` — 헤더 누락·추가·순서 불일치와 required/default 규칙을 파일·행·열로 보고한다.
- [ ] `MAP01_05_IMPLEMENT_PRIMARY_KEY_INDEX` — 모든 파일의 단일/복합 PK를 1차 수집하고 중복 행 양쪽 위치를 보고한다.
- [ ] `MAP01_06_IMPLEMENT_SCALAR_AND_LIST_PARSERS` — invariant int/ulong/float/bool/hex/enum과 pipe-list trim·빈 항목 금지를 구현한다.
- [ ] `MAP01_07_IMPLEMENT_WORLD_ROUTE_DEFINITIONS` — world, generation, RNG, route mask, sector recipe, edge signature 정의 객체를 만든다.
- [ ] `MAP01_08_IMPLEMENT_BIOME_BOUNDARY_DEFINITIONS` — biome, patch, boundary profile/pair/catalog 정의 객체를 만든다.
- [ ] `MAP01_09_IMPLEMENT_SPECIAL_VILLAGE_DEFINITIONS` — special map, footprint, entry, reward, event, village, shop 정의 객체를 만든다.
- [ ] `MAP01_10_IMPLEMENT_MICROCHUNK_POPULATION_ITEM_DEFINITIONS` — microchunk, tile code, slot, population, resource, map element, battery, tool, prefab 정의 객체를 만든다.
- [ ] `MAP01_11_IMPLEMENT_FOREIGN_KEY_RESOLVER` — 2차/3차 pass에서 단일·list FK를 해결하고 끊긴 참조의 원본/대상을 보고한다.
- [ ] `MAP01_12_IMPLEMENT_STATIC_DATA_REGISTRY` — ID Dictionary와 필요한 역색인을 immutable/read-only 형태로 publish한다.
- [ ] `MAP01_13_IMPLEMENT_CONTENT_VERSION_HASH` — 파일·행 순서에 독립적인 정규화 SHA-256 `ContentVersionHash`를 만든다.
- [ ] `MAP01_14_IMPLEMENT_ATOMIC_PUBLISH_AND_IMPORT_REPORT` — 오류 1개라도 있으면 Registry를 교체하지 않고 전체 오류를 `CsvImportReport.json`에 기록한다.
- [ ] `MAP01_15_CREATE_CSV_IMPORT_WINDOW` — 전체 재임포트, 파일별 행/오류/해시, 오류 위치·FK 대상 이동 UI를 만든다.
- [ ] `MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS` — 중복 ID, enum/숫자, missing FK, BOM, 순서 변경, previous Registry 보존을 테스트한다.
- [ ] `MAP01_17_MAP01_EXIT_AUDIT` — 49개 starter 전체 import, 필수 World/Biome/RouteMask/Battery ID, ERROR 0을 최종 승인한다.
...
- [ ] `MAP02_01_IMPLEMENT_GENERATED_WORLD_DATA` — 169개 `SectorCell`, 역할·biome·patch·route·site 필드와 직렬화 형식을 고정한다.
- [ ] `MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS` — site/biome/route/type0/recipe/population별 독립 RNG stream과 salt를 구현한다.
...
```text
NEXT  : MAP00_06_IMPLEMENT_COORDINATE_VALUE_TYPES
THEN  : MAP00_07_IMPLEMENT_COORDINATE_CONVERSIONS
THEN  : MAP00_08_CREATE_COORDINATE_TESTS
LOCKED: MAP00_09 이후 전부
HOLD  : 기존 MAP01_01_INSTALL_CSV_AUTHORING_BASELINE 패키지
```

`MAP00_05_DEFINE_WORLDGEN_CONSTANTS` Result는 PASS로 검수했다. `MAP00_06_IMPLEMENT_COORDINATE_VALUE_TYPES` 패치는 발행되었으며 Result를 기다린다. PASS 검수 전에는 MAP00_07을 만들거나 실행하지 않는다.
