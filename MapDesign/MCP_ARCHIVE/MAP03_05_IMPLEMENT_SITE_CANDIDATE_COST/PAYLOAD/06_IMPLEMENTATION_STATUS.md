...
```text
TASKS/MAP03_05_IMPLEMENT_SITE_CANDIDATE_COST.md
```
...
| MAP03_05_IMPLEMENT_SITE_CANDIDATE_COST | CURRENT |
...
현재는 MAP03의 다섯 번째 Task에서 성공 footprint option 하나의 altitude, edge clearance, partial distance deficit, caller-supplied future Core capacity estimate, three-Core 4×4 clustering을 deterministic integer cost로 계산한다. 점수만 만들며 RNG·후보 선택·reservation 생성·backtracking·실제 Core flood는 수행하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP03_05_IMPLEMENT_SITE_CANDIDATE_COST` 하나다.
...
MAP03_05가 PASS하면 STATUS FINALIZE는 MAP03_05만 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다. `MAP03_06_IMPLEMENT_RESERVATION_BACKTRACKING`은 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
