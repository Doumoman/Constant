...
```text
TASKS/MAP06_01_IMPLEMENT_OPTIONAL_REGION_MODELS.md
```
...
```text
MAP05_11_MAP05_BATCH_AND_EXIT_TESTS
```

## Last Result

```text
REPORTS/MAP05_11_MAP05_BATCH_AND_EXIT_TESTS_RESULT.md
STATUS: PASS
```
...
현재는 MAP06_01_IMPLEMENT_OPTIONAL_REGION_MODELS 하나만 CURRENT다. MAP05 EXIT은 APPROVED됐고 MAP06 ENTRY는 separate patch로만 열렸다. MAP06_02 이후 Task body는 읽거나 생성하거나 실행하지 않는다.

이번 Task는 Type0 optional region의 immutable value/aggregate model만 만든다. optional attachment enumeration, grower, Type0 route mask assignment, access/clue assignment, reward tier calculation algorithm, return device placement, inactive buffer assignment, validator, overlay는 구현하지 않는다.

Mandatory route graph/CSV/SectorCell/Authoring CSV/meta와 asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. MAP05 Type4 규칙은 보존한다: U+D mandatory, L/R actual adjacency preserved, UD/LUD/RUD/LRUD all legal.

MAP06_01이 PASS하면 MAP06_01만 COMPLETE, Current Task NONE으로 finalize한다. `MAP06_02_ENUMERATE_OPTIONAL_ATTACHMENTS`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
