...
- MAP07 phase exit: `APPROVED`; MAP08_01 and all later tasks remain locked and were not read or started

## Current Rule

현재 Current Task는 `NONE`이다. MAP07_13은 STATUS `PASS`, SHA-256 `263a2bbf291e4df25dbe6bc101986e11ebf39bc0fc3d0074759fb7450b6df77e`로 검수됐고 MAP07 starter and exit tests가 완료됐다.

MAP07 phase는 `COMPLETE / EXIT APPROVED`다. 신규 Current Task를 열지 않는다.

MAP08 boundary pair/content, sector assembly, world-level traversal, generated CSV writer는 MAP08 이후 소유다. 이 MAP7 finalize 패키지에서 구현하거나 선행 생성하지 않는다.
...
`MAP08_01_DEFINE_MOONPALACE_BIOME_PAIRS`는 LOCKED로 유지하고 별도 MAP08 patch 없이는 시작하지 않는다.
