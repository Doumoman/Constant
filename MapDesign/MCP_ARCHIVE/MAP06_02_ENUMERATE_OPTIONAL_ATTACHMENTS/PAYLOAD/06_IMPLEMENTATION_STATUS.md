...
```text
TASKS/MAP06_02_ENUMERATE_OPTIONAL_ATTACHMENTS.md
```
...
| MAP06_02_ENUMERATE_OPTIONAL_ATTACHMENTS | CURRENT |
...
```text
MAP06_01_IMPLEMENT_OPTIONAL_REGION_MODELS
```

## Last Result

```text
REPORTS/MAP06_01_IMPLEMENT_OPTIONAL_REGION_MODELS_RESULT.md
STATUS: PASS
...
현재는 MAP06_02_ENUMERATE_OPTIONAL_ATTACHMENTS 하나만 CURRENT다. MAP06_01은 PASS 및 COMPLETE 확정 기준으로만 상태 전환됐다. MAP06_03 이후 Task body는 읽거나 생성하거나 실행하지 않는다.

이번 Task는 MAP05 mandatory route graph와 MAP06_01 OptionalRegion model 계약을 입력으로 사용해 optional attachment candidate를 deterministic하게 열거하는 것까지만 수행한다. optional region grower, Type0 route mask assignment, access/clue assignment, reward tier calculation algorithm, return device placement, inactive buffer assignment, validator, overlay, generated CSV writer는 구현하지 않는다.

MAP06_02에서 새 `OptionalAttachment*`/`OptionalAttachmentEnumerator` production symbol이 생기므로, MAP05 phase-boundary negative assertions는 MAP06_02 symbol을 허용하고 MAP06_03+ future symbols만 금지하도록 필요한 기존 boundary test 파일만 교정할 수 있다. 그 외 MAP05 production graph/CSV/SectorCell/Authoring CSV/meta와 asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다.
...
MAP06_02가 PASS하면 MAP06_02만 COMPLETE, Current Task NONE으로 finalize한다. `MAP06_03_IMPLEMENT_OPTIONAL_REGION_GROWER`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
