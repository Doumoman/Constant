...
```text
TASKS/MAP04_09_IMPLEMENT_BIOME_PATCH_VALIDATOR.md
```
...
| MAP04_09_IMPLEMENT_BIOME_PATCH_VALIDATOR | CURRENT |
...
```text
MAP04_08_EXPORT_BIOME_PATCH_RESULTS
```

## Last Result

```text
REPORTS/MAP04_08_EXPORT_BIOME_PATCH_RESULTS_RESULT.md
STATUS: PASS
```
...
현재는 MAP04_08 export publication과 typed biome/patch/boundary definitions를 입력으로 patch 크기·연결·반복·share·site/reservation·Intrusion·world/CSV 재현성을 rule-by-rule 검증한다. validation은 source를 mutate하지 않고 RNG/filesystem을 사용하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP04_09_IMPLEMENT_BIOME_PATCH_VALIDATOR` 하나다.

실제 Authoring/generated CSV와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. approved Runtime/Test `Generation` 폴더에 validator production C# 7개, focused test C# 1개와 matching meta 8개만 추가한다. MAP04_01~08 existing files는 수정하지 않는다. overlay, root/retry adapter, file writer는 만들지 않는다.

MAP04_09가 PASS하면 MAP04_09만 COMPLETE, Current Task NONE으로 finalize한다. `MAP04_10_CREATE_BIOME_PATCH_OVERLAY`는 LOCKED로 유지한다.
