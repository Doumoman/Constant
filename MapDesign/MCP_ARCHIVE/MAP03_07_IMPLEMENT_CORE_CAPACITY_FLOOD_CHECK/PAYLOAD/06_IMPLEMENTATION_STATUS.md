...
```text
TASKS/MAP03_07_IMPLEMENT_CORE_CAPACITY_FLOOD_CHECK.md
```
...
| MAP03_07_IMPLEMENT_CORE_CAPACITY_FLOOD_CHECK | CURRENT |
...
현재는 MAP03의 일곱 번째 Task에서 MAP03_06의 잠정 six-site selection plan 중 Forge와 CoreResource 3개의 footprint를 seed로 삼아 cardinal flood capacity를 검사한다. footprint, exact buffer ring, Core rule 최소 크기를 모두 담는 서로 겹치지 않는 deterministic witness를 만들고 부족하면 whole `PASS_SITE` retry-required로 거부한다. RNG, option 재선택/backtracking, biome painting, Village, final reservation snapshot/pass adapter는 수행하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP03_07_IMPLEMENT_CORE_CAPACITY_FLOOD_CHECK` 하나다.
...
MAP03_07이 PASS하면 STATUS FINALIZE는 MAP03_07만 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다. `MAP03_08_IMPLEMENT_VILLAGE_RESERVATION`은 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
