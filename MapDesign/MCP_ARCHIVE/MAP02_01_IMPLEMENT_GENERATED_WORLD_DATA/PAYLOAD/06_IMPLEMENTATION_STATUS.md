...
```text
TASKS/MAP02_01_IMPLEMENT_GENERATED_WORLD_DATA.md
```
...
| MAP02_01_IMPLEMENT_GENERATED_WORLD_DATA | CURRENT |
...
```text
REPORTS/MAP01_17_MAP01_EXIT_AUDIT_RESULT.md
STATUS: PASS
```
...
현재는 MAP02의 첫 Task에서 169개 `SectorCell`, generated role/biome/patch/route/site 상태, `generated_world_sectors.csv` 직렬화 계약을 구현한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP02_01_IMPLEMENT_GENERATED_WORLD_DATA` 하나다.

실제 Authoring CSV/meta, MAP00/01 production·tests, asmdef, Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 승인된 Generation Runtime C# 4, EditMode test C# 1, matching meta와 Result만 추가한다.

MAP02_01이 PASS해도 `MAP02_02_IMPLEMENT_DETERMINISTIC_RNG_STREAMS`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP02_01을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
