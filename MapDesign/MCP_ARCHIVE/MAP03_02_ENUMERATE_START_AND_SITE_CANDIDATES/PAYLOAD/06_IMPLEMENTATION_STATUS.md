...
```text
TASKS/MAP03_02_ENUMERATE_START_AND_SITE_CANDIDATES.md
```
...
| MAP03_02_ENUMERATE_START_AND_SITE_CANDIDATES | CURRENT |
...
현재는 MAP03의 두 번째 Task에서 P01의 raw origin 후보만 deterministic하게 열거한다. Start는 generation profile의 outer edge ring `0..1`에서 exact 88개, Boss·Forge·핵심 자원 3개는 각 169개 origin으로 총 845개, 전체 933개 후보를 immutable catalog로 만든다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP03_02_ENUMERATE_START_AND_SITE_CANDIDATES` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime/Test `Generation` 폴더에 신규 production C# 6개, focused test C# 1개와 matching meta 7개만 추가한다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP03_02가 PASS하면 STATUS FINALIZE는 MAP03_02만 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다. `MAP03_03_IMPLEMENT_FOOTPRINT_PLACEMENT_SOLVER`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
