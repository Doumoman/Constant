...
```text
TASKS/MAP01_03_IMPLEMENT_RFC4180_READER.md
```
...
| MAP01_03_IMPLEMENT_RFC4180_READER | CURRENT |
...
현재는 UTF-8 CSV의 comma, quoted field, escaped quote, CRLF/LF, multiline, BOM과 exact source location을 읽는 generic RFC4180 syntax reader만 구현한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_03_IMPLEMENT_RFC4180_READER` 하나다.

schema header/required/default validation, PK index, scalar/list parsing, FK resolution, StaticDataRegistry는 구현하지 않는다. 기존 CSV·schema model/builder·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_03이 PASS해도 `MAP01_04_IMPLEMENT_HEADER_AND_FIELD_VALIDATION`을 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_03을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
