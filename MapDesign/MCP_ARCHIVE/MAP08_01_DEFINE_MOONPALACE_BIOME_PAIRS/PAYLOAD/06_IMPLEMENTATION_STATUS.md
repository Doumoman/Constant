...
- MAP07 phase exit: `APPROVED`; MAP08_01 is now the only CURRENT task. MAP08_02 and all later tasks remain locked and must not be read or started.

## Current Rule

현재 Current Task는 `MAP08_01_DEFINE_MOONPALACE_BIOME_PAIRS`다. MAP07_13은 STATUS `PASS`, SHA-256 `263a2bbf291e4df25dbe6bc101986e11ebf39bc0fc3d0074759fb7450b6df77e`로 검수됐고 MAP07 starter and exit tests가 완료됐다.

MAP07 phase는 `COMPLETE / EXIT APPROVED`다. MAP08 phase는 `MAP08_01 CURRENT / MAP08_02~MAP08_14 LOCKED` 상태로 시작한다.

MAP08_01의 소유 범위는 월궁 biome 4종의 canonical ID/name/order와 6개 unordered biome pair, 각 pair의 H/V 방향 가능성, mandatory 경계 후보 존재 조건, `tool_requirement=NONE` 보장, warning marker 최소 2종 요구를 정의하는 immutable contract와 검증 테스트다.

MAP08_02 boundary candidate index, MAP08_03 resolver, MAP08_04 mandatory-boundary filter, MAP08_06~11 실제 boundary content authoring, generated CSV writer, sector assembly, tilemap baking은 아직 소유하지 않는다.
...
`MAP08_02_IMPLEMENT_BOUNDARY_CANDIDATE_INDEX`와 이후 전체는 LOCKED로 유지하고 별도 PASS Result와 다음 patch 없이는 시작하지 않는다.
