...
MAP06_07 PASS/finalize 뒤 approved 13×13 world, P01 site reservations, P02 biome publication, MAP05 mandatory graph, MAP06_04 Type0 assignments, MAP06_07 return-policy result를 immutable source-chain으로 결합한다. ReservedSite, Mandatory, Type0의 서로 배타적인 protected ownership을 먼저 확정하고, 어느 protected set에도 속하지 않은 모든 sector를 explicit `SectorRole.InactiveBuffer`로 원자적으로 발행한다.
...
approved fixture의 exclusive accounting은 `169 = 8 ReservedSite + 47 Mandatory + 39 Type0 + 75 InactiveBuffer`다. decorative/interior exact split은 source topology에서 독립 oracle로 계산해 test와 Result에 기록하되 production service에 특정 split이나 canonical digest를 하드코딩하지 않는다.

MAP06_09 validator와 MAP06_10 overlay/exit는 구현하지 않는다. MAP06_08 production symbols가 새로 생기므로 기존 phase-boundary negative assertions는 MAP06_08 symbols를 허용하고 MAP06_09+ future symbols만 금지하도록 필요한 boundary test만 교정한다.
...
11. 이 Task
12. `REPORTS/MAP06_07_IMPLEMENT_RETURN_POLICY_RESULT.md`
...
이 별도 patch가 적용된 뒤에만 MAP06_08을 실행한다. MAP06_09 이후 Task body는 읽거나 시작하지 않는다.
...
Protected union: 8 + 47 + 39 = 94
Expected remaining InactiveBuffer assignments: 75
...
신규 C# 7개와 matching `.cs.meta` 7개, 위 boundary test C# 최대 13개 수정, Result 1개만 허용한다. MAP05/MAP06_01~07 production source, world/site/biome/mandatory/Type0/access/reward/return artifacts, Authoring/generated CSV/meta, asmdef, Scene, Prefab, Packages, ProjectSettings는 수정하지 않는다. 신규 directory/folder meta/asmdef/asmref를 만들지 않는다.
...
8. site, mandatory route cell, Type0 cell sector sets는 pairwise disjoint다.
9. approved fixture exact counts/digests는 test/Result에서 검증하고 production 일반 입력 수량으로 하드코딩하지 않는다.
...
1. 169-length ownership table을 만들고 site footprint sector를 `ReservedSite`, mandatory graph route cell을 `Mandatory`, Type0 assignment cell을 `Type0`로 표시한다.
2. duplicate sector identity와 protected ownership overlap을 stable error로 수집한다.
3. error가 없을 때 sector index `0..168`을 순회한다.
4. protected가 아닌 sector마다 exact one `InactiveBufferAssignment`를 만든다.
5. fixed cardinal neighbor order는 `L, R, U, D`다. world 밖은 neighbor로 추가하지 않는다.
6. protected cardinal neighbor가 하나 이상이면 `DecorativeBoundary`, 없으면 `InteriorInactive`다.
7. protected neighbor indices와 inactive neighbor indices는 fixed direction order에서 copied read-only list로 보존한다.
8. mandatory graph open edge와 Type0 base open edge가 inactive sector를 향하지 않는지 검사한다. 하나라도 있으면 `OpenEdgeToInactive` atomic failure다.
9. `ReservedSite + Mandatory + Type0 + InactiveBuffer == 169`, union uniqueness `169`, Unassigned `0`을 확인한 뒤에만 result를 publish한다.

approved fixture exact accounting:

```text
world = 169
protected ReservedSite/Mandatory/Type0 = 8/47/39
protected union = 94
...
새 `InactiveBufferAssignerTests`는 최소 `280` actual PASS cases를 가져야 한다.

- enum/settings/assignment/result immutability `>=30`
- 169-cell row-major world and source validation `>=34`
- protected ownership/disjointness/exclusive accounting `>=38`
...
Actually executed required total >=3983 PASS
failed/skipped 0/0
...
Result must include patch/receipt, prior Result and Task SHA gates, created/changed files, source-chain digests, settings, world/site/biome/mandatory/Type0/return source accounting, protected set intersections, exact inactive/decorative/interior counts, per-sector canonical classification or digest-backed fixture table, neighbor/edge counters, exclusive full-world accounting, open-edge-to-inactive zero, atomic failure and mutation evidence, MAP05/Type4 and MAP06_04~07 preservation, exact test jobs, Unity gate, asset/meta/CSV/GUID/change-scope gate, and NEXT that finalizes only MAP06_08 while keeping MAP06_09 locked.
...
- [ ] ReservedSite/Mandatory/Type0 protected sets are pairwise disjoint.
...
- [ ] Full accounting is `169 = 8 + 47 + 39 + 75`, Unassigned/overlap/duplicate `0/0/0`.
