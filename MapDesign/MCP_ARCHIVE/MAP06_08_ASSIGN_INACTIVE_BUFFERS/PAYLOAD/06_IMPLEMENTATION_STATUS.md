...
```text
TASKS/MAP06_08_ASSIGN_INACTIVE_BUFFERS.md
```
...
현재는 MAP06_08_ASSIGN_INACTIVE_BUFFERS 하나만 CURRENT다. MAP06_07은 PASS 및 COMPLETE 확정 기준으로만 상태 전환됐다. MAP06_09 이후 Task body는 읽거나 생성하거나 실행하지 않는다.

이번 Task는 P00 169-cell world, P01 site reservation, P02 biome publication, MAP05 mandatory graph, MAP06 Type0/return source-chain을 결합해 protected ownership `ReservedSite/Mandatory/Type0 = 8/47/39`를 검증하고 남은 `75` cells를 immutable `InactiveBuffer` assignment로 발행하는 것까지만 수행한다.

protected cardinal neighbor가 있는 inactive cell은 logical `DecorativeBoundary`, 나머지는 `InteriorInactive`로 분류하되 둘 다 existing `SectorRole.InactiveBuffer`다. boundary profile/recipe/microchunk/tile/socket/edge/generated CSV는 만들지 않는다. MAP06_08 symbols만 허용하고 MAP06_09+ future symbols는 계속 금지한다.
...
MAP06_08이 PASS하면 MAP06_08만 COMPLETE, Current Task NONE으로 finalize한다. `MAP06_09_IMPLEMENT_OPTIONAL_REGION_VALIDATOR`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
