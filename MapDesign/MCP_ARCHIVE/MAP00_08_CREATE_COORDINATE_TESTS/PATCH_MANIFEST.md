...
```yaml
patch_id: MAP00_08_CREATE_COORDINATE_TESTS
patch_version: 1.0
requires_status:
  current_task: NONE
  task_states:
...
requires_result:
  path: MapDesign/MCP/REPORTS/MAP00_07_IMPLEMENT_COORDINATE_CONVERSIONS_RESULT.md
  exact_status: PASS
forbids_started_task_prefixes:
  - MAP01
  - MAP02
sets_current_task: TASKS/MAP00_08_CREATE_COORDINATE_TESTS.md
copy_operations:
  ...
  - source: PAYLOAD/06_IMPLEMENTATION_STATUS.md
    ...
    mode: replace
  - source: PAYLOAD/TASKS/MAP00_08_CREATE_COORDINATE_TESTS.md
    destination: MapDesign/MCP/TASKS/MAP00_08_CREATE_COORDINATE_TESTS.md
    mode: create
...
- patch folder, `patch_id`, Task ID는 모두 `MAP00_08_CREATE_COORDINATE_TESTS`로 일치한다.
- 기존 `MASTER_IMPLEMENTATION_TASK_LIST.md`가 205개 Task를 포함하고 MAP00_07을 정확한 직전 단계로 기록하는지 먼저 확인한다.
- 이 패치의 Master 사본은 MAP00_07을 COMPLETE, MAP00_08을 NEXT로 반영한 승인 갱신본이므로 `replace`한다.
- `06_IMPLEMENTATION_STATUS.md`의 Current Task가 `TASKS/MAP00_08_CREATE_COORDINATE_TESTS.md`다.
- 상태 표에서 `MAP00_08_CREATE_COORDINATE_TESTS`가 `CURRENT`다.
