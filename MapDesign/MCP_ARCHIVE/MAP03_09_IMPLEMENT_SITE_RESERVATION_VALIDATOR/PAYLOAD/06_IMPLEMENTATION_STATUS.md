...
```text
TASKS/MAP03_09_IMPLEMENT_SITE_RESERVATION_VALIDATOR.md
```
...
| MAP03_09_IMPLEMENT_SITE_RESERVATION_VALIDATOR | CURRENT |
...
현재는 MAP03의 아홉 번째 Task에서 MAP03_08의 approved seven-site selection을 required count, world boundary, footprint/entry overlap, non-Village/Village distance, entry direction, Core capacity exact six rules로 재검증한다. 모두 PASS일 때만 deterministic seven reservation IDs/orders, 169 sector rows, six required entry anchors, four CoreBiomeSeeds를 가진 final immutable P01 snapshot을 atomic publish한다. 실패 시 local repair/RNG 없이 whole `PASS_SITE` retry-required로 거부한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP03_09_IMPLEMENT_SITE_RESERVATION_VALIDATOR` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime/Test `Generation` 폴더에 신규 production C# 8개, focused test C# 1개와 matching meta 9개만 추가한다. generated CSV/file I/O, pass/root adapter, overlay, batch, biome growth는 만들지 않는다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP03_09가 PASS하면 STATUS FINALIZE는 MAP03_09만 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다. `MAP03_10_CREATE_SITE_RESERVATION_OVERLAY`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
