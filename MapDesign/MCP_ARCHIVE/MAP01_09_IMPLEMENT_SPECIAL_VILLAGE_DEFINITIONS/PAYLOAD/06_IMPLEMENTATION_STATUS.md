...
```text
TASKS/MAP01_09_IMPLEMENT_SPECIAL_VILLAGE_DEFINITIONS.md
```
...
현재는 successful typed parse 결과에서 special map/footprint/entry/reward/event와 village/shop exact 12개 CSV를 immutable typed definition set으로 변환한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_09_IMPLEMENT_SPECIAL_VILLAGE_DEFINITIONS` 하나다.

FK resolution, microchunk/population/item definition, special/village domain validation, StaticDataRegistry는 구현하지 않는다. 기존 CSV·reader·validator·PK·parser·world/route/biome/boundary definitions·schema catalog/importer·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_09이 PASS해도 `MAP01_10_IMPLEMENT_MICROCHUNK_POPULATION_ITEM_DEFINITIONS`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_09을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
