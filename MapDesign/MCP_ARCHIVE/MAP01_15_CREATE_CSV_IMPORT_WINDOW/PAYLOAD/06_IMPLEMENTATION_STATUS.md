...
```text
TASKS/MAP01_15_CREATE_CSV_IMPORT_WINDOW.md
```
...
```text
REPORTS/MAP01_14_IMPLEMENT_ATOMIC_PUBLISH_AND_IMPORT_REPORT_RESULT.md
STATUS: PASS
```
...
현재는 fixed Authoring CSV 50개를 전체 재임포트하는 Editor pipeline/window에서 파일별 행·오류·진단 hash, 전역 ContentVersionHash, publish/report를 표시한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_15_CREATE_CSV_IMPORT_WINDOW` 하나다.

Failure fixture expansion, auto watcher, runtime UI, domain validation은 구현하지 않는다. 기존 CSV·Runtime pipeline·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_15가 PASS해도 `MAP01_16_CREATE_CSV_FAILURE_FIXTURES_AND_TESTS`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_15를 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
