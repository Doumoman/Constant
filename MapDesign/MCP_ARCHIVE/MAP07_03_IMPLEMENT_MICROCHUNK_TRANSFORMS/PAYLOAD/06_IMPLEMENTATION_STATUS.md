...
```text
TASKS/MAP07_03_IMPLEMENT_MICROCHUNK_TRANSFORMS.md
```
...
현재는 MAP07_03_IMPLEMENT_MICROCHUNK_TRANSFORMS 하나만 CURRENT다. MAP07_02는 STATUS `PASS`, SHA-256 `98240add84d955ffdc50c3e22e18eb3a0255d9a1d397e9d6c2039e2488dafc4e`로 검수됐고 tile-layer compatibility rule matrix가 완료됐다.

이번 Task는 `MicrochunkDefinition`의 cells, sockets, object slots에 `R0`, `MIRROR_X`, `MIRROR_Y`, `R180` transform을 적용하는 deterministic runtime transformer와 transform tests까지만 수행한다. `R90`/`R270`과 arbitrary rotation은 12x8 aspect-ratio 때문에 명시적으로 거부한다.

Socket edge compatibility, object slot semantic validation, standalone 96-cell validator pass, reachability probe, authoring grid/window, CSV import/export, preview/report는 MAP07_04~MAP07_13의 소유다. MAP07_03에서 구현하거나 선행 생성하지 않는다.
...
MAP07_03이 PASS하면 MAP07_03만 COMPLETE, Current Task NONE으로 finalize한다. `MAP07_04_IMPLEMENT_SOCKET_EDGE_VALIDATION`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
