...
## BLOCKED Remediation v1.3 — World Route Schema Contracts

v1.2 실행 후 새로 노출된 exact World/Route direct mismatch 5개만 해소한다. v1.1 parser/builder와 v1.2 Special Map BOOL 수정은 보존한다.

### Current Report Gate

`CsvImportReport.json` exact tuple: schema 1, ERROR 9, WARNING 0, `published=false`, version 0, candidate/current hash null. SHA-256은 manifest 값과 일치해야 한다. 직접 mismatch 5개와 그로 인한 derived 4개 외 issue가 있으면 적용하지 말고 BLOCKED한다.

### Authoritative Five Rows

```text
edge_signatures.csv.tags = ID_LIST (optional)
sector_recipe_cells.csv.required_route_roles = ID_LIST (optional)
sector_recipe_cells.csv.required_usage_class = ENUM_LIST (optional, allowed_values empty)
sector_recipe_cells.csv.transform_policy = ENUM_LIST (required, R0|MIRROR_X|MIRROR_Y|R180)
socket_band_definitions.csv.recommended_center = FLOAT (required)
```

dictionary와 fixed CSV/meta는 read-only다. 위 다섯 row 외 수동 도메인 분석이나 schema 변경은 금지한다.

### v1.3 WRITE ALLOWLIST
...
builder correction 후 materialization의 typed API/accessor가 authoritative type과 실제로 불일치할 때에만 아래 existing definition files도 exact 관련 property 범위에서 수정할 수 있다:
...
다른 production/test/asmdef는 변경 금지. compile reference inventory는 읽기만 허용하며 추가 consumer 수정이 필요하면 BLOCKED한다.

### Exact Corrections

- builder hard-coded expected type을 위 다섯 authoritative type으로 교정한다. inventory/order/required/default/PK/FK/다른 field 계약은 유지한다.
- `required_usage_class`의 empty vocabulary는 v1.1 unconstrained ENUM_LIST 규칙을 그대로 사용한다.
- `transform_policy`의 non-empty four-token vocabulary는 ordinal membership validation을 유지한다.
- ID_LIST/ENUM_LIST public output은 기존 immutable ordered string list 의미를 유지한다. type 이름만 맞추려고 scalar 또는 joined string으로 변환하지 않는다.
- `SocketBandDefinition.RecommendedCenter`가 아직 integer/string API 또는 accessor라면 exact `float` property/constructor와 `WorldRouteDefinitionValueReader.Float` materialization으로 교정한다.
- 이미 compatible한 definition property/accessor는 변경하지 않는다.
- issue suppression, field-specific bypass, `ToString`, in-memory schema rewrite 금지.

### Regression / Full Verification

- 다섯 field 각각 authoritative exact-type acceptance와 one near-miss rejection을 table-driven으로 검증한다.
- `recommended_center` fractional value 보존, transform four-token validation, empty-vocabulary usage-class list, list order/duplicates/immutability를 검증한다.
- v1.3 focused 최소 `12/12 PASS`; 누적 repair 최소 `39/39 PASS`.
- world-route 전체 기존 `59/59` 및 신규 cases 전부 PASS.
- window focused `48/48 PASS`; targeted total 최소 `758 PASS`; full EditMode 최소 `778 PASS`.
- actual full reimport: exact 50 file rows/49 parsed sources, error/warning `0/0`, `published=true`, current version 1, candidate/current hash non-null and equal.
- Unity refresh/compile error/relevant warning `0/0`, visual published state PASS, CSV/meta 50/50 hashes unchanged.

### v1.3 Completion

기존 Result에 `REMEDIATION v1.3`을 추가하여 five-row authority, exact changed files/SHA, tests, final report tuple/hash, visual evidence를 기록한다. 모든 조건 충족 시에만 MAP01_15 COMPLETE/Current Task NONE으로 finalize한다. 실패하면 MAP01_15 CURRENT/BLOCKED 유지. MAP01_16은 LOCKED이며 자동 시작하지 않는다.

---

## BLOCKED Remediation v1.4 — Stale interaction_tags Test

production, actual full reimport, publish, window, repair-focused 검증은 모두 PASS다. targeted/full EditMode의 동일한 sole stale assertion 한 줄만 교정하는 final same-task patch다.

### Exact Precondition

- current report: schema 1, ERROR/WARNING 0/0, `published=true`, current version 1.
- candidate/current ContentVersionHash: exact `1c41b14c2734200999e779ad1317c5bc2ef5208da1c3b4bc30347e47182cfeaf`.
- window `48/48`, cumulative repair `47/47`, world-route `73/73`, atomic/hash/Registry/FK `210/210` PASS.
- sole targeted/full failure: `MicrochunkPopulationItemDefinitionBuilderTests.CoreContractCase(13)`.

precondition이 다르거나 두 번째 실패가 존재하면 적용하지 말고 BLOCKED한다.
...
production C#, 다른 test, dictionary, CSV/meta, asmdef, Scene/Prefab은 변경 금지다.

### Required One-Line Contract Correction

`map_element_definitions.csv.interaction_tags`는 v1.1부터 authoritative `ID_LIST`다. `CoreContractCase(13)`의 stale expected sample만 아래처럼 교정한다:

```text
before expected: ENUM_A|ENUM_B|ENUM_A
after expected:  LIST_A|LIST_B|LIST_A
```

actual fixture/data, production materialization, generic comparison helper, 다른 case의 ENUM_LIST expectation은 변경하지 않는다. test skip/ignore/category exclusion, assertion 완화, issue suppression은 금지한다.

### Final Verification / Finalize

- 해당 focused case 및 MicrochunkPopulationItemDefinitionBuilder 전체 PASS.
- targeted Map assembly exact `764/764 PASS`.
- full project EditMode exact `784/784 PASS`.
- Unity compile error/relevant warning `0/0`.
- 기존 successful report tuple/hash, published Registry version/hash, visual PASS, CSV/meta hashes가 유지됨을 확인한다. content가 변하지 않았으므로 불필요한 CSV 재임포트로 version을 증가시키지 않는다.
- 기존 Result에 `REMEDIATION v1.4` 절을 추가해 one-line before/after, test job/count, preserved report tuple을 기록한다.
- 전부 충족하면 Result 최상단 `STATUS: PASS`, MAP01_15 COMPLETE, Current Task NONE으로 finalize한다.
- MAP01_16은 LOCKED로 유지하고 자동 시작하지 않는다. 실패 시 MAP01_15 CURRENT/BLOCKED를 유지한다.
