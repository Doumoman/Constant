...
```text
TASKS/MAP01_08_IMPLEMENT_BIOME_BOUNDARY_DEFINITIONS.md
```
...
- MAP01_07 Authoring CSV/meta preservation: `50/50`, modified `0`

## Current Rule

현재는 successful typed parse 결과에서 biome type, patch rule, boundary profile/pair, boundary chunk catalog exact 5개 CSV를 immutable typed definition set으로 변환한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP01_08_IMPLEMENT_BIOME_BOUNDARY_DEFINITIONS` 하나다.

FK resolution, special/village/microchunk/population/item definition, biome/boundary domain validation, StaticDataRegistry는 구현하지 않는다. 기존 CSV·reader·validator·PK·parser·world/route definitions·schema catalog/importer·asmdef·Scene·Prefab·Package·ProjectSettings는 수정하지 않는다.

MAP01_08이 PASS해도 `MAP01_09_IMPLEMENT_SPECIAL_VILLAGE_DEFINITIONS`를 자동 시작하지 않는다. STATUS FINALIZE는 MAP01_08을 COMPLETE로 바꾸고 Current Task를 NONE으로 만든 뒤 종료한다.
