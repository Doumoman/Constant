...
```text
TASKS/MAP04_01_IMPLEMENT_BIOME_PATCH_MODELS.md
```
...
| MAP04_01_IMPLEMENT_BIOME_PATCH_MODELS | CURRENT |
...
```text
MAP03_11_MAP03_BATCH_AND_EXIT_TESTS
```

## Last Result

```text
REPORTS/MAP03_11_MAP03_BATCH_AND_EXIT_TESTS_RESULT.md
STATUS: PASS
MAP03 EXIT: APPROVED
```
...
현재는 MAP03 exit 승인 상태에서 MAP04의 첫 Task로 P02 Biome Patch가 사용할 immutable typed model만 구현한다. `BiomePatchId`, Core/Satellite/Intrusion 역할, patch seed, 169-sector Primary/SecondaryBiome·Patch 소유권, Core site binding, patch aggregate, complete/partial snapshot의 자기 일관성·결정적 순서를 고정한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP04_01_IMPLEMENT_BIOME_PATCH_MODELS` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime/Test `Generation` 폴더에 신규 biome-patch model C# 7개, focused test C# 1개와 matching meta 8개만 추가한다. Core seed 초기화, growth/cost/RNG, Satellite/Intrusion 배치, cleanup, serializer/export, validator, overlay, pass/root adapter는 만들지 않는다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP04_01이 PASS하면 STATUS FINALIZE는 MAP04_01만 COMPLETE로 바꾸며 Current Task를 NONE으로 만든 뒤 종료한다. `MAP04_02_INITIALIZE_CORE_PATCH_SEEDS`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
