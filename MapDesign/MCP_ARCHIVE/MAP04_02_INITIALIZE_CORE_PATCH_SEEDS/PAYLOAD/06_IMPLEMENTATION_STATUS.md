...
```text
TASKS/MAP04_02_INITIALIZE_CORE_PATCH_SEEDS.md
```
...
| MAP04_02_INITIALIZE_CORE_PATCH_SEEDS | CURRENT |
...
```text
MAP04_01_IMPLEMENT_BIOME_PATCH_MODELS
```

## Last Result

```text
REPORTS/MAP04_01_IMPLEMENT_BIOME_PATCH_MODELS_RESULT.md
STATUS: PASS
```
...
현재는 MAP04_01의 immutable P02 model 위에서 MAP03 final publication의 exact four `CoreBiomeSeed`를 Core patch로 초기화한다. 각 source reservation footprint 전체를 동일 Core Patch의 seed·owned sector로 강제하고, 나머지 169-sector ownership은 unassigned인 deterministic partial snapshot을 atomic publish한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP04_02_INITIALIZE_CORE_PATCH_SEEDS` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime/Test `Generation` 폴더에 Core patch initializer production C# 6개, focused test C# 1개와 matching meta 7개만 추가한다. buffer/minimum growth, growth cost/RNG, Satellite/Intrusion 배치, cleanup, serializer/export, validator, overlay, pass/root adapter는 만들지 않는다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP04_02가 PASS하면 STATUS FINALIZE는 MAP04_02만 COMPLETE로 바꾸며 Current Task를 NONE으로 만든 뒤 종료한다. `MAP04_03_IMPLEMENT_CORE_PATCH_GROWER`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
