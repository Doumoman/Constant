...
```text
TASKS/MAP07_06_IMPLEMENT_96_CELL_VALIDATOR.md
```
...
| MAP07_06_IMPLEMENT_96_CELL_VALIDATOR | CURRENT |
...
```text
REPORTS/MAP07_05_IMPLEMENT_OBJECT_SLOT_VALIDATION_RESULT.md
STATUS: PASS
...
현재는 MAP07_06_IMPLEMENT_96_CELL_VALIDATOR 하나만 CURRENT다. MAP07_05는 STATUS `PASS`, SHA-256 `4d805c6ff1702e4e8ecea3be7a337584e4e2856b7d5106d51d1e42c31954029c`로 검수됐고 microchunk object-slot validation layer가 완료됐다.

이번 Task는 complete microchunk tile data가 정확히 `12x8 = 96` logical cell records를 갖는지 검증하는 deterministic coverage validator까지만 수행한다. `0..11 x 0..7` 좌표의 누락, 중복, 범위 초과, explicit `NONE` row 생략을 검출하되 tile-layer compatibility, socket-edge, object-slot, reachability 의미를 재검증하지 않는다.

Reachability probe, authoring grid/window, socket/slot editor UI, CSV import/export, preview/report는 MAP07_07~MAP07_13의 소유다. MAP07_06에서 구현하거나 선행 생성하지 않는다.
...
MAP07_06이 PASS하면 MAP07_06만 COMPLETE, Current Task NONE으로 finalize한다. `MAP07_07_IMPLEMENT_MICROCHUNK_REACHABILITY_PROBE`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
