...
```text
TASKS/MAP06_10_MAP06_OVERLAY_AND_EXIT_TESTS.md
```
...
현재는 MAP06_10_MAP06_OVERLAY_AND_EXIT_TESTS 하나만 CURRENT다. MAP06_09는 PASS 및 COMPLETE 확정 기준으로만 상태 전환됐다. MAP07 이후 Task body는 읽거나 생성하거나 실행하지 않는다.

이번 Task는 MAP06 source-chain과 validation report를 시각 점검 가능한 immutable overlay snapshot으로 표시하고 MAP06 phase exit tests를 완료하는 것까지만 수행한다.

Overlay는 access color, depth, attachment/contact, return witness, reward tier, inactive kind, validation issue 상태를 표시한다. Generated CSV writer, boundary profile/recipe/microchunk/tile/socket/edge, MAP07 microchunk authoring은 만들지 않는다. MAP06_10 symbols만 허용하고 MAP07+ future symbols는 계속 금지한다.
...
MAP06_10이 PASS하면 MAP06_10만 COMPLETE, Current Task NONE으로 finalize하고 `MAP06 PHASE EXIT: APPROVED`를 기록한다. `MAP07_01_IMPLEMENT_MICROCHUNK_DEFINITION`는 LOCKED로 유지하고 별도 patch 없이는 시작하지 않는다.
