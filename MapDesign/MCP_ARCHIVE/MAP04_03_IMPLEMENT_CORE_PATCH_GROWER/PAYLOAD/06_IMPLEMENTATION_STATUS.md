...
```text
TASKS/MAP04_03_IMPLEMENT_CORE_PATCH_GROWER.md
```
...
| MAP04_03_IMPLEMENT_CORE_PATCH_GROWER | CURRENT |
...
```text
MAP04_02_INITIALIZE_CORE_PATCH_SEEDS
```

## Last Result

```text
REPORTS/MAP04_02_INITIALIZE_CORE_PATCH_SEEDS_RESULT.md
STATUS: PASS
```
...
현재는 MAP04_02의 immutable partial P02 snapshot에서 exact four CorePatch를 각 source footprint의 cardinal buffer와 rule minimum까지 먼저 성장시킨다. 자기 source footprint는 보존하고 모든 다른 P01 reservation은 hard blocker로 취급하며, 성공 시 starter ownership을 `4 assigned / 165 unassigned`에서 exact `20 assigned / 149 unassigned`로 atomic publish한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP04_03_IMPLEMENT_CORE_PATCH_GROWER` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime/Test `Generation` 폴더에 Core patch grower production C# 6개, focused test C# 1개와 matching meta 7개만 추가한다. initializer 결과와 MAP04_01/02 existing files는 수정하지 않는다. Satellite/Intrusion, full-map multi-seed cost/noise/RNG, cleanup, serializer/export, validator, overlay, pass/root adapter는 만들지 않는다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP04_03이 PASS하면 STATUS FINALIZE는 MAP04_03만 COMPLETE로 바꾸며 Current Task를 NONE으로 만든 뒤 종료한다. `MAP04_04_IMPLEMENT_SATELLITE_SEED_PLACER`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
