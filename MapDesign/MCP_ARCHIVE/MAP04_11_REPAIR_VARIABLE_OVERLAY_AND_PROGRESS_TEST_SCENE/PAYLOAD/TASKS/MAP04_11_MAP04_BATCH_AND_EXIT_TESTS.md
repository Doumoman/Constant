# MAP04_11 — MAP04 Batch and Exit Tests (Repair v1.3 + Progress Scene)

```yaml
status_control:
  task_key: MAP04_11_MAP04_BATCH_AND_EXIT_TESTS
  result_file: REPORTS/MAP04_11_MAP04_BATCH_AND_EXIT_TESTS_RESULT.md
  repair_contract: VARIABLE_COUNT_OVERLAY_AND_MANUAL_PROGRESS_SCENE
```
...
```text
Current Task SHA:
9da9fde9b65ffee5bd9ffbb408a0732d89a65c8f9e8c4be947798780ce3394ec

Current FAIL Result SHA:
9b5a4e8746a453f76a2a959ecdae7be2029f503316ef25398551c4762c14b03e
...
- existing `Assets/_Game/Scenes` path-only inventory, build settings scene list, relevant asmdef
...
plus matching two new `.meta`, current Result, Phase C Status finalize only. `Map04ExitTests`, repaired cleanup/producer files, public models/API shape, asmdef, CSV, other Scene/Prefab, Packages, ProjectSettings는 수정하지 않는다.
...
overlay focused compile/tests가 PASS한 뒤 Unity Editor API로 scene을 생성한다. raw YAML 작성은 금지한다.
...
```text
Assets meta 3148 -> 3150
new Editor C#/scene/meta = 1/1/2
new folder/folder meta = 0
