...
```text
TASKS/MAP05_07_ADD_MANDATORY_ROUTE_LOOPS.md
```
...
| MAP05_07_ADD_MANDATORY_ROUTE_LOOPS | CURRENT |
...
현재는 MAP05_06의 tree/backbone/gateway/conflict 입력에서 필수망 사이 독립 loop를 최소 2개 deterministic하게 계획한다. Type4의 네 수평 조합(U+D/L+U+D/R+U+D/L+R+U+D)은 모두 허용하며 U+D는 필수, L/R은 실제 상태 보존이다. 기존 MAP05_02 lookup과 MAP05_06 plan은 변경하지 않는다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP05_07_ADD_MANDATORY_ROUTE_LOOPS` 하나다.

실제 Authoring/generated CSV와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. approved Runtime/Test `Generation`에 production C# 8개, focused test 1개와 matching meta 9개를 추가하고, prior negative-audit test 5개만 MAP05_07 output을 허용하도록 전환한다. Type4는 U+D mandatory/L/R independent semantic으로 보존하며 graph/validator/overlay는 시작하지 않는다.

MAP05_07가 PASS하면 MAP05_07만 COMPLETE, Current Task NONE으로 finalize한다. `MAP05_08_BUILD_MANDATORY_ROUTE_GRAPH`는 LOCKED로 유지한다.
