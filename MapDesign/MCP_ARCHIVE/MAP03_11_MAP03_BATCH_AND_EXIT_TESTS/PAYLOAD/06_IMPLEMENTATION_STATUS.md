...
```text
TASKS/MAP03_11_MAP03_BATCH_AND_EXIT_TESTS.md
```
...
| MAP03_11_MAP03_BATCH_AND_EXIT_TESTS | CURRENT |
...
```text
REPORTS/MAP03_10_CREATE_SITE_RESERVATION_OVERLAY_RESULT.md
STATUS: PASS
```
...
현재는 MAP03의 마지막 Task에서 MAP03_01~10 production을 수정하지 않고 phase-level batch와 exit tests를 수행한다. exact 100,000 world-seed Village bucket schedule의 20/50/30 분포, 10,000-seed full reservation attempt/retry audit, retry 원인 보존, final required reservation/entry/Core capacity/validation 결정론, current-project Game/Scene overlay를 하나의 evidence chain으로 재검증한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP03_11_MAP03_BATCH_AND_EXIT_TESTS` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. 기존 approved Runtime test `Generation` 폴더에 신규 exit test C# 1개와 matching meta 1개만 추가한다. batch runner/adapter/root retry production, generated output, biome growth는 만들지 않는다. 기존 legacy Editor folder meta 6개는 baseline으로 보존하고 신규 directory/folder meta는 만들지 않는다.

MAP03_11이 PASS하면 `MAP03 EXIT: APPROVED`를 기록하고 STATUS FINALIZE는 MAP03_11만 COMPLETE로 바꾸며 Current Task를 NONE으로 만든 뒤 종료한다. `MAP04_01_IMPLEMENT_BIOME_PATCH_MODELS`는 LOCKED로 유지하며 별도 최신 패치로만 열 수 있다.
