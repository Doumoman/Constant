...
```text
TASKS/MAP04_07_IMPLEMENT_PATCH_CLEANUP.md
```
...
| MAP04_07_IMPLEMENT_PATCH_CLEANUP | CURRENT |
...
```text
MAP04_06_IMPLEMENT_INTRUSION_PLACEMENT
```

## Last Result

```text
REPORTS/MAP04_06_IMPLEMENT_INTRUSION_PLACEMENT_RESULT.md
STATUS: PASS
```
...
현재는 MAP04_06의 immutable 17-patch snapshot에서 일반 Core/Satellite의 exact checkerboard cell과 1-cell neck만 deterministic cleanup한다. P01 reservation, Core binding, 모든 seed, Intrusion cell/anchor는 이동하지 않는다. source를 mutate하지 않고 safe move가 없으면 전체 rollback한다.

`MASTER_IMPLEMENTATION_TASK_LIST.md`의 205개 Task 순서를 상위 기준으로 사용한다. 현재 Task는 `MAP04_07_IMPLEMENT_PATCH_CLEANUP` 하나다.

실제 Authoring CSV/meta와 기존 production/tests/asmdef/Scene/Prefab/Package/ProjectSettings는 수정하지 않는다. approved Runtime/Test `Generation` 폴더에 cleanup production C# 6개, focused test C# 1개와 matching meta 7개만 추가한다. MAP04_01~06 existing files는 수정하지 않는다. export, final validator, overlay, pass/root adapter는 만들지 않는다.

MAP04_07이 PASS하면 MAP04_07만 COMPLETE, Current Task NONE으로 finalize한다. `MAP04_08_EXPORT_BIOME_PATCH_RESULTS`는 LOCKED로 유지한다.
