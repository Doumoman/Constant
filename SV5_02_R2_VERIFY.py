"""Read-only SV5_02 R2 package check, plus optional LOCAL PRE-APPLY checks.
...
def main():
    ...
    mf=root/'SV5_02_R2_FILES.json';need(mf.is_file(),'manifest missing')
    ...
    inp=root/'MapDesign/MCP/INPUTS/SV5_02_R2';lock=load(inp/'SOURCE_LOCK.json')
    required_generated = {'MCP/GENERATED/SV5_01/'+n for n in ('BASELINE.json','PLAN_LINK.json','BINDING.json')}
    actual_generated = {e['path'] for e in lock['files'] if 'GENERATED/SV5_01/' in e['path']}
    need(actual_generated == required_generated, 'R2 generated prerequisite paths must be MapDesign-relative MCP/GENERATED/SV5_01')
    need(lock.get('input_revision') == 'R2', 'wrong input revision')
    revision = load(inp/'REVISION.json')
    for e in revision['unchanged_payloads']:
        p = (inp/e['path_in_input']).resolve()
        need(p.is_relative_to(inp), 'unsafe unchanged payload path')
        if p.is_relative_to(inp):need(p.is_file() and sha(p)==e['sha256'], 'unchanged payload mismatch: '+e['path_in_input'])
    ...
    if args.local_precheck:
        ...
        if args.prior_result:
            ...
            need(not p.is_relative_to(inp) and not p.is_relative_to((mr/'MCP/INPUTS').resolve()) and not p.is_relative_to((root/'MapDesign/MCP/INPUTS').resolve()),'input reference copy cannot replace actual Result')
